...
double CORONAGRAPHS_PIXSCALE = CORONAGRAPHS_TDIAM / CORONAGRAPHS_PSCALE /
                               CORONAGRAPHS_ARRAYSIZE; // l/d per pixel
double COROTMP1;
double MASKSIZELD;
...
static int FPMASKSIZE_ERROR =
    0; /* 1 if focal plane mask error is included (for chromaticity for example) */
static double FPMASK_FACTOR =
    1.0; // if focal plane mask error included, multiplicative coefficient to the focal plane mask size - currently only for MULTI APLC
...
/* 8 order BL Lyot */
#define BL8MASK_NBSTEP                                                         \
    100000 /* number of points over which the BL mask is computed */
...
static double RRPM_P2 =
    -0.3207; /* term of order 2 in polynomial pupil apodization */
static double RRPM_P3 =
    0.00952; /* term of order 3 in polynomial pupil apodization */
static double RRPM_P4 =
    0.000658; /* term of order 4 in polynomial pupil apodization */
static double RRPM_P5 =
    0.001127; /* term of order 5 in polynomial pupil apodization */
...
/* in l/d */ /* 4.25 is the critical size to maintain 1e-10 contrast at all separations  default=4.6*/
...
static errno_t init_module_CLI()
{
    ...
    RegisterCLIcommand(
        ...
        "coronagraph_make_2Dprolate(double masksize, double centralObs, const "
        "char *outname)");

    RegisterCLIcommand("cormk2Dprolateld",
                       __FILE__,
                       coronagraph_make_2Dprolateld__cli,
                       "make 2D prolate, l/D unit mask size",
                       "<focal plane mask radius [l/D]> <central obstruction> "
                       "<image name> <array size>",
                       "cormk2Dprolateld 1.20 0.1 prolim 2048",
                       "coronagraph_make_2Dprolateld(double masksizeld, double "
                       "centralObs, const char *outname, long size)");

    RegisterCLIcommand("corup2Dprolate",
                       __FILE__,
                       ...
                       "coronagraph_update_2Dprolate(double masksizeld, double "
                       "centralObs, double zfactor)");

    RegisterCLIcommand("corprolcomp",
                       __FILE__,
    ...
    RegisterCLIcommand(
        ...
        "coronagraph_simulPSF(double xld, double yld, const char *psfname, "
        "long coronagraph_type, const "
        "char *options)");
...
//
errno_t coronagraph_make_2Dprolate(double      fpmradpix,
                                   double      beamradpix,
                                   double      centralObs,
                                   const char *outname,
                                   long        size,
                                   ...
                                   double     *transmval)
{
    ...
    double      peak;
    long        IDprol, ID;
    long double total, total0, total2;
    long        iter;
    long        NBiter = 40;
    double      transm;
    ...
    if(ID != -1)
    {
        NBiter = (long)(1.0 * data.core.variable[ID].value.f + 0.01);
    }
    ...
    IDfpmask =
        make_subpixdisk("FPmask", size, size, size / 2, size / 2, fpmradpix);
    for(long ii = 0; ii < size2; ii++)
    {
        data.core.image[IDfpmask].array.F[ii] =
            1.0 * data.core.image[IDfpmask].array.F[ii];
        //      if(data.core.image[IDfpmask].array.F[ii]<0.01)
        //	data.core.image[IDfpmask].array.F[ii] = 0.0;
    }
    ...
    IDpupa0 = image_ID("pupa0", data.core.image, data.core.NB_MAX_IMAGE);
    if(IDpupa0 != -1)
    {
    ...
    {
        IDpupa0 =
            make_disk("pupa0", size, size, size / 2, size / 2, beamradpix);
        if(CentralObstructionFlag == 1)
        {
            IDpupa0co = make_disk("pupa0co",
                                  size,
                                  size,
                                  size / 2,
                                  size / 2,
            ...
            {
                data.core.image[IDpupa0].array.F[ii] -=
                    data.core.image[IDpupa0co].array.F[ii];
            }
            delete_image_ID("pupa0co", DELETE_IMAGE_ERRMODE_WARNING);
        }

        IDpupmask = image_ID(pupmask_name, data.core.image, data.core.NB_MAX_IMAGE);
        if(IDpupmask != -1)
        {
            for(long ii = 0; ii < size * size; ii++)
            {
                data.core.image[IDpupa0].array.F[ii] *=
                    data.core.image[IDpupmask].array.F[ii];
            }
        }
    ...
    for(long ii = 0; ii < size2; ii++)
    {
        double v1 = data.core.image[IDpupa0].array.F[ii];
        total0 += v1 * v1;
    }
    ...
    for(long ii = 0; ii < size; ii++)
        ...
        {
            if(data.core.image[IDpupa0].array.F[jj * size + ii] > 0.01)
            {
                double x  = 1.0 * ii - size / 2;
                double y  = 1.0 * jj - size / 2;
                double r2 = (x * x + y * y) / beamradpix / beamradpix;
                data.core.image[IDprolr].array.F[jj * size + ii] =
                    exp(-r2 * 0.73 / 5.0 * fpmradpix);
                data.core.image[IDproli].array.F[jj * size + ii] = 0.0;
                data.core.image[IDprol].array.F[jj * size + ii] =
                    data.core.image[IDprolr].array.F[jj * size + ii];
            }
            else
            {
                data.core.image[IDprolr].array.F[jj * size + ii] = 0.0;
                data.core.image[IDproli].array.F[jj * size + ii] = 0.0;
                data.core.image[IDprol].array.F[jj * size + ii]  = 0.0;
            }
        }

    ID = image_ID("apostart", data.core.image, data.core.NB_MAX_IMAGE);
    if(ID != -1)
    {
        for(long ii = 0; ii < size2; ii++)
        {
            data.core.image[IDprolr].array.F[ii] = data.core.image[ID].array.F[ii];
            data.core.image[IDprol].array.F[ii]  = data.core.image[ID].array.F[ii];
        }
    }
    delete_image_ID("apostart", DELETE_IMAGE_ERRMODE_WARNING);
    ...
    for(iter = 0; iter < NBiter; iter++)
    {
        for(long ii = 0; ii < size2; ii++)
        {
            data.core.image[IDpupa0m].array.F[ii] = data.core.image[IDpupa0].array.F[ii] *
                                               data.core.image[IDprol].array.F[ii];
            data.core.image[IDpupp0m].array.F[ii] =
                data.core.image[IDpupp0].array.F[ii] +
        ...
        {
            total +=
                data.core.image[IDprol].array.F[ii] * data.core.image[IDprol].array.F[ii];
        }
        transm = total / total0;
        ...
        for(long ii = 0; ii < size2; ii++)
        {
            total += data.core.image[ID].array.F[ii] * data.core.image[ID].array.F[ii];
        }
        ...
        for(long ii = 0; ii < size2; ii++)
        {
            total2 += data.core.image[ID].array.F[ii] * data.core.image[ID].array.F[ii];
        }
        total2 /= 1.0 * size2 * total0;

        printf(
            "%ld/%ld   FPmask throughput = %g    prolate throughput = %.18g\n",
            iter,
            NBiter,
            (double)(total / total2),
            (double) transm);
        ...
        for(long ii = 0; ii < size; ii++)
            for(long jj = 0; jj < size; jj++)
            {
                data.core.image[IDr].array.F[jj * size + ii] = 0.0;
            }

        for(long ii = 1; ii < size; ii++)
            for(long jj = 1; jj < size; jj++)
            {
                data.core.image[IDr].array.F[jj * size + ii] =
                    data.core.image[IDrcp]
                    .array.F[(size - jj) * size + (size - ii)] /
                    size2;
            }
        peak = data.core.image[IDr].array.F[(size / 2) * size + size / 2];
        ...
        for(long ii = 0; ii < size; ii++)
            for(long jj = 0; jj < size; jj++)
            {
                data.core.image[IDi].array.F[jj * size + ii] = 0.0;
            }

        for(long ii = 1; ii < size; ii++)
            for(long jj = 1; jj < size; jj++)
            {
                data.core.image[IDi].array.F[jj * size + ii] =
                    data.core.image[IDicp]
                    .array.F[(size - jj) * size + (size - ii)] /
                    size2;
            }
        ...
        peak = 0.0;
        for(long ii = 0; ii < size2; ii++)
            if(data.core.image[IDpupa0].array.F[ii] > 0.01)
                if(fabs(data.core.image[IDr].array.F[ii]) > fabs(peak))
                {
                    peak = data.core.image[IDr].array.F[ii];
                }
        printf(" %f\n", peak);
        ...
        for(long ii = 0; ii < size2; ii++)
            if(data.core.image[IDpupa0].array.F[ii] > 0.01)
            {
                data.core.image[IDprolr].array.F[ii] =
                    1.0 * data.core.image[IDr].array.F[ii];
                data.core.image[IDproli].array.F[ii] =
                    1.0 * data.core.image[IDi].array.F[ii];
            }

        //      for(ii=0;ii<size2;ii++)
        //	data.core.image[IDprol].array.F[ii] = data.core.image[IDprolr].array.F[ii];
        for(long ii = 1; ii < size - 1; ii++)
            for(long jj = 1; jj < size - 1; jj++)
            {
                long ii1 = jj * size + ii;
                data.core.image[IDprol].array.F[jj * size + ii] =
                    sqrt(data.core.image[IDprolr].array.F[ii1] *
                         data.core.image[IDprolr].array.F[ii1] +
                         data.core.image[IDproli].array.F[ii1] *
                         data.core.image[IDproli].array.F[ii1]);
                data.core.image[IDprolp].array.F[jj * size + ii] =
                    -atan2(data.core.image[IDproli].array.F[ii1],
                           data.core.image[IDprolr].array.F[ii1]);
            }
        ...
        for(long ii = 0; ii < size2; ii++)
            if(data.core.image[IDpupa0].array.F[ii] > 0.01)
                if(fabs(data.core.image[IDprol].array.F[ii]) > fabs(peak))
                {
                    peak = data.core.image[IDprol].array.F[ii];
                }

        //      printf("peak = %f\n",peak);
        for(long ii = 0; ii < size2; ii++)
        {
            if(data.core.image[IDpupa0].array.F[ii] > 0.001)
            {
                data.core.image[IDprol].array.F[ii] /= peak;
            }
            else
            {
                data.core.image[IDprol].array.F[ii] = 0.0;
            }
        }
        ...
        if(0)  // TEST
        {
            ...
            delete_image_ID("pc1_a", DELETE_IMAGE_ERRMODE_WARNING);
            delete_image_ID("pc1_p", DELETE_IMAGE_ERRMODE_WARNING);
            save_fl_fits(outname, "proltmp.1.fits");
        }

        delete_image_ID("pc1", DELETE_IMAGE_ERRMODE_WARNING);
        delete_image_ID("fc1", DELETE_IMAGE_ERRMODE_WARNING);
        delete_image_ID("fa1", DELETE_IMAGE_ERRMODE_WARNING);
        delete_image_ID("fp1", DELETE_IMAGE_ERRMODE_WARNING);
        delete_image_ID("fa1m", DELETE_IMAGE_ERRMODE_WARNING);
        delete_image_ID("fc2", DELETE_IMAGE_ERRMODE_WARNING);
        delete_image_ID("pc2", DELETE_IMAGE_ERRMODE_WARNING);
        delete_image_ID("pr2", DELETE_IMAGE_ERRMODE_WARNING);
        delete_image_ID("pi2", DELETE_IMAGE_ERRMODE_WARNING);
    }
    ...
    for(long ii = 0; ii < size2; ii++)
    {
        data.core.image[IDfpmask].array.F[ii] =
            1.0 -
            data.core.image[IDfpmask].array.F[ii] * (1.0 + (1.0 - peak) / peak);
    }

    //  save_fl_fits("FPmask","FPmask.fits");

    delete_image_ID("FPmask", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pupp0", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pupa0", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pupa0m", DELETE_IMAGE_ERRMODE_WARNING);

    delete_image_ID("proli", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("prolr", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("prolp", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pupp0m", DELETE_IMAGE_ERRMODE_WARNING);
                                      ...
                                      double     *transmval)
{
    ...
    FILE       *fp;
    uint64_t    size2;
    imageID     IDfpmask, IDfpmaskc, IDfpmaskz;
    imageID     IDpupa0, IDpupp0, IDpupa0m, IDpupp0m;
    double      peak;
    imageID     IDprol, ID;
    ...
    double      transm;
    imageID     IDr, IDi;
    imageID     IDprolr, IDproli, IDprolp;
    imageID     IDpupmask;
    ...
    {
        DFTZFACTOR = data.core.variable[ID].value.f;
    }
    ...
    FUNC_CHECK_RETURN(create_2Dimage_ID("proli", size, size, &IDproli));

    FUNC_CHECK_RETURN(create_2Dimage_ID("prolr", size, size, &IDprolr));

    FUNC_CHECK_RETURN(create_2Dimage_ID("prolp", size, size, &IDprolp));
    ...
    for(long k = 0; k < fpmshape_n; k++)
    {
        ...
        {
            char vname[STRINGMAXLEN_IMGNAME];

            WRITE_IMAGENAME(vname, "FPMSHAPE_%ld_r", k);
            if((ID = variable_ID(vname)) != -1)
            {
                fpmshape_ra[k] = data.core.variable[ID].value.f;
            }

            WRITE_IMAGENAME(vname, "FPMSHAPE_%ld_k", k);
            if((ID = variable_ID(vname)) != -1)
            {
                fpmshape_ka[k] = data.core.variable[ID].value.f;
            }

            WRITE_IMAGENAME(vname, "FPMSHAPE_%ld_p", k);
            if((ID = variable_ID(vname)) != -1)
            {
                fpmshape_pa[k] = data.core.variable[ID].value.f;
            }
        }
    ...
    IDfpmask =
        make_subpixdisk("FPmask", size, size, size / 2, size / 2, fpmradpix);
    IDfpmaskz = make_subpixdisk_perturb("FPmaskz",
                                        size,
                                        size,
                                        size / 2,
                                        size / 2,
                                        fpmradpix * DFTZFACTOR,
                                        fpmshape_n,
                                        fpmshape_ra,
                                        fpmshape_ka,
                                        fpmshape_pa);

    FUNC_CHECK_RETURN(create_2DCimage_ID("_fpmz", size, size, &IDfpmaskc));

    for(uint64_t ii = 0; ii < size2; ii++)
    {
        data.core.image[IDfpmask].array.F[ii] =
            1.0 * data.core.image[IDfpmaskz].array.F[ii];
        data.core.image[IDfpmaskc].array.CF[ii].re =
            data.core.image[IDfpmaskz].array.F[ii];
        data.core.image[IDfpmaskc].array.CF[ii].im = 0.0;
        //      if(data.core.image[IDfpmask].array.F[ii]<0.01)
        //	data.core.image[IDfpmask].array.F[ii] = 0.0;
    }

    {
        ...
        WRITE_FULLFILENAME(fname, "%s/FPmask.tmp.fits", data.core.SAVEDIR);
    ...
    if(0)
    {
        FUNC_CHECK_RETURN(mk_amph_from_complex("_fpmz", "_fpmza", "_fpmzp", 0));

        FUNC_CHECK_RETURN(save_fl_fits("_fpmza", "_fpmza.fits"));

        FUNC_CHECK_RETURN(save_fl_fits("_fpmzp", "_fpmzp.fits"));

        FUNC_CHECK_RETURN(
            delete_image_ID("_fpmza", DELETE_IMAGE_ERRMODE_WARNING));

        FUNC_CHECK_RETURN(
            delete_image_ID("_fpmzp", DELETE_IMAGE_ERRMODE_WARNING));
    }
    // exit(0);

    FUNC_CHECK_RETURN(create_2Dimage_ID("pupp0", size, size, &IDpupp0));
    ...
    for(uint64_t ii = 0; ii < size2; ii++)
    {
        v1 = data.core.image[IDpupa0].array.F[ii];
        total0 += v1 * v1;
    }
    ...
    // total0 = arith_image_total("pupa0");
    FUNC_CHECK_RETURN(create_2Dimage_ID("pupa0m", size, size, &IDpupa0m));

    FUNC_CHECK_RETURN(create_2Dimage_ID("pupp0m", size, size, &IDpupp0m));

    FUNC_CHECK_RETURN(create_2Dimage_ID(outname, size, size, &IDprol));

    for(uint32_t ii = 0; ii < size; ii++)
        for(uint32_t jj = 0; jj < size; jj++)
        {
            if(data.core.image[IDpupa0].array.F[jj * size + ii] > 0.0001)
            {
                double x  = 1.0 * ii - size / 2;
                double y  = 1.0 * jj - size / 2;
                double r2 = (x * x + y * y) / beamradpix / beamradpix;
    ...
    for(iter = 0; iter < NBiter; iter++)
    {
        ...
        FUNC_CHECK_RETURN(mk_complex_from_amph("pupa0m", "pupp0", "pc1", 0));
        ...
        total = data.core.DOUBLEARRAY[0];
        total /= 1.0 * size2 * total0;

        // TOTAL LIGHT IN IMAGE -> total2
        ID     = image_ID("pc1", data.core.image, data.core.NB_MAX_IMAGE);
        total2 = 0.0;
        for(uint64_t ii = 0; ii < size2; ii++)
        {
            data.core.image[ID].array.CF[ii].re *= size;
            data.core.image[ID].array.CF[ii].im *= size;
            re = data.core.image[ID].array.CF[ii].re;
            im = data.core.image[ID].array.CF[ii].im;
            total2 += re * re + im * im;
        }
        total2 /= 1.0 * size2 * total0;
        ...
        FUNC_CHECK_RETURN(mk_reim_from_complex("pc3", "pr2", "pi2", 0));
        ...
        peak = 0.0;
        for(uint64_t ii = 0; ii < size2; ii++)
            if(data.core.image[IDpupa0].array.F[ii] > 0.0001)
                if(fabs(data.core.image[IDr].array.F[ii]) > fabs(peak))
                {
                    peak = data.core.image[IDr].array.F[ii];
                }
        ...
        fprintf(fp, "# col 4: Peak\n");
        fprintf(fp,
                "# ideal complex transmission for FPM is MASKCAMULT = %.18f\n",
                -(1.0 - peak) / peak);
        fclose(fp);
        printf("IDEAL COMPLEX TRANSMISSION = %g\n", -(1.0 - peak) / peak);
        ...
        if(0)  // TEST
        {
            FUNC_CHECK_RETURN(save_fl_fits("pr2", "_prol_pr2.fits"));

            FUNC_CHECK_RETURN(save_fl_fits("pi2", "_prol_pi2.fits"));
        }
        ...
        {
            data.core.image[IDprol].array.F[ii] =
                sqrt(data.core.image[IDprolr].array.F[ii] *
                     data.core.image[IDprolr].array.F[ii] +
                     data.core.image[IDproli].array.F[ii] *
                     data.core.image[IDproli].array.F[ii]);
        }
        ...
        if(0)  // TEST
        {
            ...
            delete_image_ID("pc1_a", DELETE_IMAGE_ERRMODE_WARNING);
            delete_image_ID("pc1_p", DELETE_IMAGE_ERRMODE_WARNING);
            delete_image_ID("pc3_a", DELETE_IMAGE_ERRMODE_WARNING);
            delete_image_ID("pc3_p", DELETE_IMAGE_ERRMODE_WARNING);
            save_fl_fits(outname, "proltmp.fits");
            //	  exit(0);
        }

        FUNC_CHECK_RETURN(delete_image_ID("pc1", DELETE_IMAGE_ERRMODE_WARNING));

        FUNC_CHECK_RETURN(delete_image_ID("pc3", DELETE_IMAGE_ERRMODE_WARNING));

        FUNC_CHECK_RETURN(delete_image_ID("pr2", DELETE_IMAGE_ERRMODE_WARNING));

        FUNC_CHECK_RETURN(delete_image_ID("pi2", DELETE_IMAGE_ERRMODE_WARNING));
    }
    ...
    for(uint64_t ii = 0; ii < size2; ii++)
    {
        data.core.image[IDfpmaskz].array.F[ii] =
            1.0 -
            data.core.image[IDfpmaskz].array.F[ii] * (1.0 + (1.0 - peak) / peak);
    }

    //  save_fl_fits("FPmaskz","FPmask.fits");
    FUNC_CHECK_RETURN(delete_image_ID("FPmaskz", DELETE_IMAGE_ERRMODE_WARNING));

    FUNC_CHECK_RETURN(delete_image_ID("FPmask", DELETE_IMAGE_ERRMODE_WARNING));

    FUNC_CHECK_RETURN(delete_image_ID("_fpmz", DELETE_IMAGE_ERRMODE_WARNING));

    FUNC_CHECK_RETURN(delete_image_ID("pupp0", DELETE_IMAGE_ERRMODE_WARNING));

    FUNC_CHECK_RETURN(delete_image_ID("pupa0", DELETE_IMAGE_ERRMODE_WARNING));

    FUNC_CHECK_RETURN(delete_image_ID("pupa0m", DELETE_IMAGE_ERRMODE_WARNING));

    FUNC_CHECK_RETURN(delete_image_ID("proli", DELETE_IMAGE_ERRMODE_WARNING));

    FUNC_CHECK_RETURN(delete_image_ID("prolr", DELETE_IMAGE_ERRMODE_WARNING));

    FUNC_CHECK_RETURN(delete_image_ID("prolp", DELETE_IMAGE_ERRMODE_WARNING));

    FUNC_CHECK_RETURN(delete_image_ID("pupp0m", DELETE_IMAGE_ERRMODE_WARNING));
                                     ...
                                     const char *pupmask_name)
{
    ...
    if(useDFT == 0)
    {
        if(coronagraph_make_2Dprolate(fpmradpix,
                                      beamradpix,
                                      centralObs,
                                      outname,
                                      size,
                                      ...
                                      NULL) != RETURN_SUCCESS)
        {
            FUNC_RETURN_FAILURE("Call to coronagraph_make_2Dprolate failed");
        }
    }
    else
    {
        if(coronagraph_make_2Dprolate_DFT(fpmradpix,
                                          beamradpix,
                                          centralObs,
                                          outname,
                                          size,
        ...
        {
            FUNC_RETURN_FAILURE(
                "Call to coronagraph_make_2Dprolate_DFT failed");
        }
    }
                                      ...
                                      const char *outname)
{
    ...
    if(coronagraph_make_2Dprolate_DFT(masksize,
                                      beamradpix,
                                      0.0,
                                      "outapo",
                                      size,
                                     ...
                                     double zfactor)
{
    ...
    for(MASKSIZELD = 0.8; MASKSIZELD < 5.01; MASKSIZELD += masksizestep)
    {
        ...
        {
            sprintf(fname,
                    "%s/APLCapo/raw/APLCapo_%.3f.%ld.ref.gz",
                    CORONAGRAPHSDATALOCAL,
                    MASKSIZELD,
                    size);
            sprintf(fnameinfo,
                    "%s/APLCapo/APLCapo_%.3f.%ld.ref.info",
                    CORONAGRAPHSDATALOCAL,
                    MASKSIZELD,
                    size);
        }
        ...
        {
            sprintf(fname,
                    "%s/APLCapo/raw/APLCapo_%.3f.%.3f.%ld.ref.gz",
                    CORONAGRAPHSDATALOCAL,
                    MASKSIZELD,
                    centralObs,
                    size);
            sprintf(fnameinfo,
                    "%s/APLCapo/APLCapo_%.3f.%.3f.%ld.ref.info",
                    CORONAGRAPHSDATALOCAL,
                    MASKSIZELD,
                    centralObs,
                    size);
        ...
        if(CentralObstructionFlag == 0)
        {
            sprintf(fnameprev,
                    "%s/APLCapo/raw/APLCapo_%.3f.%ld.ref.gz",
                    CORONAGRAPHSDATALOCAL,
                    MASKSIZELD - masksizestep,
                    size);
            sprintf(fnameinfoprev,
                    "%s/APLCapo/APLCapo_%.3f.%ld.ref.info",
                    CORONAGRAPHSDATALOCAL,
                    MASKSIZELD - masksizestep,
                    size);
        ...
        sprintf(fnamenew,
                "%s/APLCapo/APLCapo_%.3f.%.3f.%ld.ref.new1",
                CORONAGRAPHSDATALOCAL,
                MASKSIZELD,
                centralObs,
                size);
        ...
        if(file_exists(fnamenew) == 0)
        {
            ...
            sprintf(fnamenew,
                    ...
                    centralObs,
                    size);
            if(((file_exists(fnameprev) == 1) &&
                    (file_exists(fnameinfoprev) == 1)) &&
                    ((file_exists(fname) == 0) || (file_exists(fnameinfo) == 0)))
            {
                ...
                EXECUTE_SYSTEM_COMMAND("cp %s %s", fnameprev, fname);
                EXECUTE_SYSTEM_COMMAND("cp %s %s", fname1prev, fname1);
            ...
            {
                data.core.image[ID2].array.F[ii] = 0.0;
            }
            ...
            while(iter1 < NBiter1)
            {
                ...
                while((iter != 2) && (iter1 < NBiter1 + 1))
                {
                    ...
                    load_fits(fname, "apostart", 1, NULL);
                    ...
                    double transm;
                    if(coronagraph_make_2Dprolate_DFT(
                                MASKSIZELD / CORONAGRAPHS_PIXSCALE,
                                beamradpix,
                                centralObs,
                                "out",
                                size,
                                "NULLim",
                                &transm) != RETURN_SUCCESS)
                    {
                        FUNC_RETURN_FAILURE(
                            "Call to coronagraph_make_2Dprolate_DFT failed");
                    }
                    ...
                    ID  = image_ID("out", data.core.image, data.core.NB_MAX_IMAGE);
                    err = 0.0;
                    for(ii = 0; ii < size * size; ii++)
                    {
                        data.core.image[ID1].array.F[ii] =
                            data.core.image[ID2].array.F[ii];
                        data.core.image[ID2].array.F[ii] =
                            data.core.image[ID].array.F[ii];
                        tmp1 = data.core.image[ID1].array.F[ii] -
                               data.core.image[ID2].array.F[ii];
                        err += tmp1 * tmp1;
                    }
                    if(iter1 == 0)
                    ...
                    {
                        sprintf(fnameprof,
                                "%s/APLCapo/raw/APLCapo_%.3f.%ld.ref.prof",
                                CORONAGRAPHSDATALOCAL,
                                MASKSIZELD,
                                size);
                    }
                    ...
                    {
                        sprintf(fnameprof,
                                "%s/APLCapo/raw/APLCapo_%.3f.%.3f.%ld.ref.prof",
                                CORONAGRAPHSDATALOCAL,
                                MASKSIZELD,
                                centralObs,
                                size);
                    ...
                    profile("out",
                            ...
                            1.0,
                            size / 2);
                    EXECUTE_SYSTEM_COMMAND("mv out.prof %s", fnameprof);
                    EXECUTE_SYSTEM_COMMAND("cp APLCapo.%.3f.%.3f.info %s",
                                           MASKSIZELD,
                                           centralObs,
                                           fnameinfo);
                ...
                for(ii = 0; ii < size * size; ii++)
                {
                    data.core.image[ID0].array.F[ii] =
                        data.core.image[ID2].array.F[ii] +
                        FFcoeff * (data.core.image[ID2].array.F[ii] -
                                   data.core.image[ID1].array.F[ii]);
                }
                save_fl_fits("apo", fname1);
            }

            delete_image_ID("apo", DELETE_IMAGE_ERRMODE_WARNING);
            delete_image_ID("apo1", DELETE_IMAGE_ERRMODE_WARNING);
            delete_image_ID("apo2", DELETE_IMAGE_ERRMODE_WARNING);

            EXECUTE_SYSTEM_COMMAND("touch %s", fnamenew);
        }
    }
...
errno_t coronagraph_apofit_eval_r(double r, double *outval)
{
    DEBUG_TRACE_FSTART();
    ...
    if(isfinite(value) == 0)
    {
        ...
        for(n = 0; n < fitapoN; n++)
        {
            ...
            printf("---- %ld %f   %f %f %f -> %f %f -> %f\n",
                   ...
                   fitapo_c[n],
                   pow(rp, fitapo_c[n]),
                   exp(fitapo_b[n] * pow(rp, fitapo_c[n])),
                   value);
            fflush(stdout);
        }
    }
...
errno_t coronagraph_apofit_eval(double *outval)
{
    DEBUG_TRACE_FSTART();
    ...
    for(n = 0; n < fitapoN; n++)
    {
        ...
        value +=
            100.0 * pow(0.5 * (atan(-(fitapo_c[n] - 0.5) * 10.0 / (M_PI / 2)) /
                               (M_PI / 2) +
                               1.0),
                        8.0);
...
double coronagraph_apofit(const char *fnameout)
{
    ...
    if(fpbest != NULL)
    {
        OK = 1;
        {
            ...
            fscanfcnt = fscanf(fpbest, "%lf", &bestvalue);
            if(fscanfcnt == EOF)
            {
                if(ferror(fpbest))
                {
    ...
    for(iter1 = 0; iter1 < ITERMAX; iter1++)  // DOWNHILL SIMPLEX METHOD
    {
        ...
        if((iter1 == 0) ||
                (OK ==
                 1)) // set starting point = best point if 1st iteration or loop is still making progress
        {
            KEEPcnt = 0;
            printf("BEST  ");
            fflush(stdout);
        ...
        else
        {
            if(KEEP == 0)  // random starting point
            {
                ...
                for(n = 0; n < fitapoN; n++)
                {
                    double v1 = (1.0 - 2.0 * ran1()) * aRANGE;
                    double v2 = (1.0 - 2.0 * ran1()) * bRANGE;
                    double v3 = 0.5 + (2.0 * ran1()) * cRANGE;
            ...
            else
            {
                ...
                KEEPcnt++;
                KEEPlimit =
                    (0.94 * KEEPlimit + 0.05 * bestvalue) * 0.7 + 0.3 * s->fval;
                for(n = 0; n < fitapoN; n++)
                {
        ...
        do
        {
            ...
            if((status == GSL_SUCCESS) || (s->fval < bestvalue * eps1))
            {
                ...
                if(s->fval < bestvalue * eps1)  //if(s->f<bestvalue)
                {
                    ...
                    n = 0;
                    fprintf(fpbest,
                            "f(x) = %15.10f * exp(%15.10f * x**%15.10f)",
                            fitapo_a_best[n],
                            fitapo_b_best[n],
                            fitapo_c_best[n]);
                    for(n = 1; n < fitapoN; n++)
...
errno_t coronagraph_APLCapo_compile()
{
    ...
    for(long ii = 0; ii < iisize; ii++)
    {
        for(long jj = 0; jj < jjsize; jj++)
            if(ran1() < 1.01)
            {
                ...
                if(co < 0.0001)
                {
                    sprintf(fnameprolinfo,
                            "%s/APLCapo/APLCapo_%5.3f.4096.ref.info",
                            CORONAGRAPHSDATALOCAL,
                            fpmrad);
                    sprintf(fnameapoprof,
                            "%s/APLCapo/raw/APLCapo_%.3f.4096.ref.prof",
                            CORONAGRAPHSDATALOCAL,
                            fpmrad);
                }
                else
                {
                    sprintf(fnameprolinfo,
                            "%s/APLCapo/APLCapo_%5.3f.%5.3f.4096.ref.info",
                            CORONAGRAPHSDATALOCAL,
                            fpmrad,
                            co);
                    sprintf(fnameapoprof,
                            "%s/APLCapo/raw/APLCapo_%.3f.%.3f.4096.ref.prof",
                            CORONAGRAPHSDATALOCAL,
                            fpmrad,
                            co);
                }

                if((stat(fnameprolinfo, &buffer) == 0) &&
                        (stat(fnameapoprof, &buffer) == 0)) // if file exists
                {
                    printf("\r");
                    printf("%5ld/%5ld   %5ld/%5ld  %f %f  ",
                           ii,
                           iisize,
                           jj,
                           jjsize,
                    ...
                    fp = fopen(fnameprolinfo, "r");
                    {
                        ...
                        if(fscanfcnt == EOF)
                        {
                            ...
                            else
                            {
                                fprintf(stderr,
                                        "Error: fscanf reached end of file, no "
                                        "matching characters, no matching "
                                        "failure\n");
                            }
                            return RETURN_FAILURE;
                        }
                        else if(fscanfcnt != 4)
                        {
                            fprintf(stderr,
                                    ...
                                    "assigned %i input items, 4 expected\n",
                                    fscanfcnt);
                            return RETURN_FAILURE;
                        }
                    }
                    ...
                    //data.core.image[IDfpmasktransm].array.F[fpmaskrad_index*obs1arraysize + obs1_index] = fpmasktransm;

                    data.core.image[IDprol_init].array.F[jj * iisize + ii]   = 1.0;
                    data.core.image[IDprol_ffrac].array.F[jj * iisize + ii]  = tmpf1;
                    data.core.image[IDprol_transm].array.F[jj * iisize + ii] = tmpf2;
                    data.core.image[IDprol_peak].array.F[jj * iisize + ii]   = tmpf3;

                    sprintf(
                        fnameapofit,
                        "%s/APLCapo/APLCapo_%.3f.%.3f.4096.ref.prof.fitparams",
                        CORONAGRAPHSDATALOCAL,
                        fpmrad,
                        co);
...

                        {
                            ...
                            if(fscanfcnt == EOF)
                            {
                                ...
                                else
                                {
                                    fprintf(stderr,
                                            ...
                                            "file, no matching characters, no "
                                            "matching failure\n");
                                }
                                return RETURN_FAILURE;
                            }
                            else if(fscanfcnt != 1)
                            {
                                fprintf(
                                    stderr,
                                    "Error: fscanf successfully matched and "
                                    "assigned %i input items, 1 expected\n",
                                    fscanfcnt);
                                return RETURN_FAILURE;
                            }
                        }
                    ...
                    if(fitOK ==
                            0) // if file does not exist, or fit is not sufficiently good
                    {
                        if(co > 0.0001)
                        {
                            sprintf(fnameapoprof,
                                    "%s/APLCapo/raw/"
                                    "APLCapo_%.3f.%.3f.4096.ref.prof",
                                    CORONAGRAPHSDATALOCAL,
                                    fpmrad,
                                    co);
                        }
                        ...
                        EXECUTE_SYSTEM_COMMAND(
                            "awk '{print $1,$2}' %s > apotmp.prof",
                            fnameapoprof);

                        if((fp = fopen("apotmp.prof", "r")) == NULL)
                        {
                            printf(
                                "ERROR : cannot open file \"apotmp.prof\"\n");
                            exit(0);
                        }
                        else
                        {
                        ...
                        aporaw_r = (double *) malloc(sizeof(double) *
                                                     PIAAAPO_NBPOINTS);
                        if(aporaw_r == NULL)
                        {
                            PRINT_ERROR("malloc returns NULL pointer");
                        ...
                        {
                            {
                                ...
                                {
                                    fprintf(stderr,
                                            "Error: fscanf successfully "
                                            "matched and assigned %i input "
                                            "items, 2 expected\n",
                                            fscanfcnt);
                                    return RETURN_FAILURE;
                                }
                        ...
                        fpfit = fopen(fnameapofit, "r");
                        {
                            ...
                            fscanfcnt = fscanf(fpfit, "%lf", &fitv);
                            if(fscanfcnt == EOF)
                            {
                                if(ferror(fp))
                                {
                        ...
                        for(n = 0; n < fitapoN; n++)
                        {
                            ...
                            else if(fscanfcnt != 3)
                            {
                                fprintf(
                                    stderr,
                                    "Error: fscanf successfully matched and "
                                    "assigned %i input items, 3 expected\n",
                                    fscanfcnt);
                                return RETURN_FAILURE;
                            }
                        }
                    ...
                    for(n = 0; n < fitapoN; n++)
                    {
                        data.core.image[IDprol_fitapo_a]
                        .array.F[n * iisize * jjsize + jj * iisize + ii] =
                            fitapo_a[n];
                        data.core.image[IDprol_fitapo_b]
                        .array.F[n * iisize * jjsize + jj * iisize + ii] =
                            fitapo_b[n];
                        data.core.image[IDprol_fitapo_c]
                        .array.F[n * iisize * jjsize + jj * iisize + ii] =
                            fitapo_c[n];
                    }
                    data.core.image[IDprol_fitfit].array.F[jj * iisize + ii] = fitv;
                ...
                {
                    data.core.image[IDprol_init].array.F[jj * iisize + ii] = 0.0;
                }
            }
    }
    ...
    delete_image_ID("pffrac", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("ptransm", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("ppeak", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("fitfit", DELETE_IMAGE_ERRMODE_WARNING);

    delete_image_ID("fitapoa", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("fitapob", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("fitapoc", DELETE_IMAGE_ERRMODE_WARNING);
    printf("\n");
...
//
//
errno_t coronagraph_init_PIAA(long *outNBpoints)
{
    DEBUG_TRACE_FSTART();

    FILE    *fp;
    uint32_t size = CORONAGRAPHS_ARRAYSIZE;
    double  *r0, *r1;
    ...
    double FLUXIN =
        0.0001; // fraction of total flux to insert in central obstruction
    ...
    double dx, dz, dist, slope, y3, r0c, r1c, r0n;
    double piaaconfdist;
    ...
    /*
    EXECUTE_SYSTEM_COMMAND("awk '{print $1,$2}' %s/APLCapo/raw/%s > apotmp.prof", CORONAGRAPHSDATALOCAL, PIAAAPO_FNAME);
    ...
    */
    ...
    {
        long ii1 = (long)((1.0 * ii - 0.5) / factor + 0.5);

        //      printf("%ld/%ld %ld %ld\n", ii, size, factor, ii1);
        //fflush(stdout);

        for(long jj = 0; jj < factor * size; jj += tmpstep)
        {
            long   jj1 = (long)((1.0 * jj - 0.5) / factor + 0.5);
            double x   = 1.0 / factor * (ii - factor * size / 2);
            double y   = 1.0 / factor * (jj - factor * size / 2);
            double r   = sqrt(x * x + y * y); /* in pixel */
            ...
            if(r < PIAAextfactor1)
            {
                coronagraph_apofit_eval_r(r, &value);
                if(r > 1.0)  // OUTER RING
                {
            ...
            if((ii1 > 0) && (jj1 > 0) && (ii1 < size) && (jj1 < size))
            {
                ...
                data.core.image[IDt].array.F[jj1 * size + ii1] =
                    PIAAAPO2D[jj1 * size + ii1];

                if((r > APLC_CentOBS1) && (r < 1.0))
                {
                    data.core.image[IDtt].array.F[jj1 * size + ii1] =
                        PIAAAPO2D[jj1 * size + ii1];
                }
                else
                {
                    data.core.image[IDtt].array.F[jj1 * size + ii1] = 0.0;
                }
            }
        }
    }
    ...
    for(uint64_t ii = 0; ii < size * size; ii++)
    {
        data.core.image[IDt].array.F[ii] *= normfactor;
    }
    ...
    printf("GOAL = %g\n",
           total_ir / (APLC_CentOBS0 * APLC_CentOBS0 /
                       (1.0 - APLC_CentOBS0 * APLC_CentOBS0)));
    ...
    while(fabs(verr) > 1.0e-9)
    {
        t0    = 0.0;
        t0cnt = 0.0;
        for(long i = 0; i < NBistep; i++)
        {
            double x = 1.0 * i / NBistep;
            a        = 0.5;
            eps1     = 1e-8;
            if(x < eps1)
            {
            ...
            double value =
                b +
                (1.0 - b) *
                (0.5 + atan(-a / b / (x * x) + a / pow(x - 1.0, 2)) / M_PI);
            t0 += x * value * value;
            innerprof_cumul[i] = t0;
            t0cnt += x;
        }

        verr = t0 / t0cnt - total_ir / (APLC_CentOBS0 * APLC_CentOBS0 /
                                        (1.0 - APLC_CentOBS0 * APLC_CentOBS0));
    ...
    piaaconfpup_amp_profile =
        (double *) malloc(sizeof(double) * piaaconfNPUPFILESIZE);
    ...
    {
        double r = PIAAextfactor1 * ii / piaaconfNPUPFILESIZE;
        double v;
        coronagraph_apofit_eval_r(r, &v);
        piaaconfpup_amp_profile[ii] = normfactor * v;
    }
    ...
    total          = 0.0;
    for(long ii = 0; ii < NB_profile_pts; ii++)
    {
    ...
    for(i = 1; i < NBpoints0; i++)
    {
        double F0 = pup0sum[i];
        while(
            ((pupsum[ii] / pupsum[(long)(NB_profile_pts) - 1]) < pup0sum[i]) &&
            (ii < piaaconfNPUPFILESIZE))
        {
            ii++;
        }
        double F1 = pupsum[ii - 1] / pupsum[(long)(NB_profile_pts) - 1];
        double F2 = pupsum[ii] / pupsum[(long)(NB_profile_pts) - 1];
    ...
    for(i = 0; i < NBpoints; i++)
    {
        ...
        {
            double x = piaaconfr0[i] / APLC_CentOBS0;
            i1       = (long)(x * NBistep);
            if(i1 > NBistep - 1)
            {
                i1 = NBistep - 1;
        ...
        {
            pup0sum[i] = FLUXIN + (piaaconfr0[i] * piaaconfr0[i] -
                                   PIAACENTOBS * PIAACENTOBS) /
                         (1.0 - PIAACENTOBS * PIAACENTOBS);
        }
        //      else
    ...
    {
        total += 2.0 * PI * piaaconfpup_amp_profile[ii] *
                 piaaconfpup_amp_profile[ii] * ii; /* new */
        pupsum[ii] = total;
    }
    i  = 0;
    ii = 0;
    ...
    {
        double F0 = pup0sum
                    [i]; // fraction of total light inside input radius in input pupil
        while(((pupsum[ii] / pupsum[NB_profile_pts - 1]) < pup0sum[i]) &&
                (ii < NB_profile_pts))
        {
            ii++;
        }
        ...
        double F2       = pupsum[ii] / pupsum[NB_profile_pts - 1];
        double fluxdens = (F2 - F1) / (2.0 * PI * ii);
        /* compute pseudo radii psr0 and psr1 */
        psr0 = sqrt(1.0 * ii * ii - 1.0 * ii);
        //psr1 = sqrt(1.0 * ii * ii + 1.0 * ii);
        piaaconfr1[i] = PIAAextfactor1 *
                        sqrt((F0 - F1) / fluxdens / PI + psr0 * psr0) /
                        (NB_profile_pts - 1);
    }
    ...
    {
        long jj =
            (long)((piaaconfr0[ii] / PIAAextfactor0) * piaaconfNBr1fr0_pts +
                   0.5);
    ...
    for(ii = 0; ii < piaaconfNBr1fr0_pts; ii++)
        if(cntarray[ii] == 0)
        {
            ...
            double x = (1.0 * ii - ii0) / (ii1 - ii0);
            piaaconfr1fr0[ii] =
                (1.0 - x) * piaaconfr1fr0[ii0] + x * piaaconfr1fr0[ii1];
        }
    ...
    if(file_write == 1)
    {
        ...
        for(ii = 0; ii < piaaconfNBr1fr0_pts; ii++)
        {
            fprintf(fpout,
                    "%18.16f %18.16f\n",
                    PIAAextfactor0 * ii / piaaconfNBr1fr0_pts,
                    piaaconfr1fr0[ii]);
        }
        fclose(fpout);
    }
    ...
    for(i = 0; i < piaaconfNBr1fr0_pts - 1; i++)
    {
        ...
        piaaconfM0[i + 1] =
            piaaconfM0[i] + slope * (r0n - r0c) * piaaconfbeamradius;
        ...
        piaaconfM1[i + 1] = piaaconfM1[i] + slope *
                            (piaaconfr1fr0[i + 1] - r1c) *
                            piaaconfbeamradius;
    }
    ...
    for(ii = 0; ii < piaaconfNBr1fr0_pts; ii++)
    {
        fprintf(fp,
                ...
                PIAAextfactor0 * ii / piaaconfNBr1fr0_pts * piaaconfbeamradius,
                piaaconfM0[ii],
                piaaconfr1fr0[ii] * piaaconfbeamradius,
                piaaconfM1[ii]);
    }
    fclose(fp);
    ...
    {
        long jj =
            (long)((piaaconfr1[ii] / PIAAextfactor1) * piaaconfNBr1fr0_pts +
                   0.5);
        if((jj > -1) && (jj < piaaconfNBr1fr0_pts))
        {
    ...
    for(ii = 0; ii < piaaconfNBr1fr0_pts; ii++)
        if(cntarray[ii] == 0)
        {
            ...
            double x = (1.0 * ii - ii0) / (ii1 - ii0);
            piaaconfr0fr1[ii] =
                (1.0 - x) * piaaconfr0fr1[ii0] + x * piaaconfr0fr1[ii1];
        }
    ...
    if(file_write == 1)
    {
        ...
        for(ii = 0; ii < piaaconfNBr1fr0_pts; ii++)
        {
            fprintf(fpout,
                    "%18.16f %18.16f\n",
                    PIAAextfactor1 * ii / piaaconfNBr1fr0_pts,
                    piaaconfr0fr1[ii]);
        }
        fclose(fpout);
    }
...
int coronagraph_telescope_pupil_Subaru_inside1(double x, double y)
{
    ...
    for(arm = 0; arm < nbarms; arm++)
    {
        asq = (x1[arm] - x2[arm]) * (x1[arm] - x2[arm]) +
              (y1[arm] - y2[arm]) * (y1[arm] - y2[arm]);
        x12 = x2[arm] - x1[arm];
        y12 = y2[arm] - y1[arm];
...
int coronagraphs_make_SUBARU_pupil()
{
    ...
    for(uint32_t ii = 0; ii < Ssize; ii++)
        ...
        {
            double x = 1.0 * ii - Ssize / 2;
            double y = 1.0 * jj - Ssize / 2;
            x /= rad;
            y /= rad;
            //PA = atan2(y,x);
            //r1 = sqrt(x*x+y*y);
            ...
            */
            ...
            //x = r0*cos(PA);
            //y = r0*sin(PA);
            data.core.image[ID].array.F[jj * Ssize + ii] =
                coronagraph_telescope_pupil_Subaru_inside1(x, y);
        }
                                 ...
                                 double  ratio)
{
    ...
    imageID IDzmap; // M1 sag map
    imageID IDdx;   // x displacement
    imageID IDdy;   // y displacement
    imageID IDi;    // intensity multiplication

    uint32_t size = CORONAGRAPHS_ARRAYSIZE;

    double eps  = 1.0e-4; // step to compute derivative [pupil radius]
    ...
    for(uint32_t ii = 0; ii < size; ii++)
        for(uint32_t jj = 0; jj < size; jj++)
        {
            ...
            double x0 = 1.0 * (ii - size / 2) / trad_pix;
            double y0 = 1.0 * (jj - size / 2) / trad_pix;
            double r0 = sqrt(x0 * x0 + y0 * y0);
            //double PA0 = atan2(y0, x0);
            ...
            if(r0 < 1.1)
            {
                ...
                double p0x0  = x0 + epst * cos(0.0);
                double p0y0  = y0 + epst * sin(0.0);
                double p0r0  = sqrt(p0x0 * p0x0 + p0y0 * p0y0);
                double p0PA0 = atan2(p0y0, p0x0);

                double p0x1  = p0x0 + eps;
                double p0y1  = p0y0;
                double p0r1  = sqrt(p0x1 * p0x1 + p0y1 * p0y1);
                double p0PA1 = atan2(p0y1, p0x1);

                double p0x2  = p0x0;
                double p0y2  = p0y0 + eps;
                double p0r2  = sqrt(p0x2 * p0x2 + p0y2 * p0y2);
                double p0PA2 = atan2(p0y2, p0x2);

                double p0v0 = 0.0;
                double p0v1 = 0.0;
                double p0v2 = 0.0;
                for(int k = 0; k < NBzern; k++)
                {
                ...
                double p0dx =
                    -2.0 * ratio * (p0v1 - p0v0) /
                    (eps); // second term is unitless, result is in pupil radius
                double p0dy = -2.0 * ratio * (p0v2 - p0v0) / (eps);

                // point #1

                double p1x0  = x0 + epst * cos(2.0 * M_PI / 3.0);
                double p1y0  = y0 + epst * sin(2.0 * M_PI / 3.0);
                double p1r0  = sqrt(p1x0 * p1x0 + p1y0 * p1y0);
                double p1PA0 = atan2(p1y0, p1x0);

                double p1x1  = p1x0 + eps;
                double p1y1  = p1y0;
                double p1r1  = sqrt(p1x1 * p1x1 + p1y1 * p1y1);
                double p1PA1 = atan2(p1y1, p1x1);

                double p1x2  = p1x0;
                double p1y2  = p1y0 + eps;
                double p1r2  = sqrt(p1x2 * p1x2 + p1y2 * p1y2);
                double p1PA2 = atan2(p1y2, p1x2);

                double p1v0 = 0.0;
                double p1v1 = 0.0;
                double p1v2 = 0.0;

                for(long k = 0; k < NBzern; k++)
                {
                ...
                double p1dx =
                    -2.0 * ratio * (p1v1 - p1v0) /
                    (eps); // second term is unitless, result is in pupil radius
                double p1dy = -2.0 * ratio * (p1v2 - p1v0) / (eps);

                // point #2

                double p2x0  = x0 + epst * cos(4.0 * M_PI / 3.0);
                double p2y0  = y0 + epst * sin(4.0 * M_PI / 3.0);
                double p2r0  = sqrt(p2x0 * p2x0 + p2y0 * p2y0);
                double p2PA0 = atan2(p2y0, p2x0);

                double p2x1  = p2x0 + eps;
                double p2y1  = p2y0;
                double p2r1  = sqrt(p2x1 * p2x1 + p2y1 * p2y1);
                double p2PA1 = atan2(p2y1, p2x1);

                double p2x2  = p2x0;
                double p2y2  = p2y0 + eps;
                double p2r2  = sqrt(p2x2 * p2x2 + p2y2 * p2y2);
                double p2PA2 = atan2(p2y2, p2x2);
                ...
                double p2dx =
                    -2.0 * ratio * (p2v1 - p2v0) /
                    (eps); // second term is unitless, result is in pupil radius
                double p2dy = -2.0 * ratio * (p2v2 - p2v0) / (eps);

                data.core.image[IDzmap].array.F[jj * size + ii] =
                    (p0v0 + p1v0 + p2v0) / 3.0;

                data.core.image[IDdx].array.F[jj * size + ii] =
                    (p0dx + p1dx + p2dx) / 3.0;
                data.core.image[IDdy].array.F[jj * size + ii] =
                    (p0dy + p1dy + p2dy) / 3.0;

                double p0x0p = p0x0 + p0dx;
                double p0y0p = p0y0 + p0dy;

                double p1x0p = p1x0 + p1dx;
                double p1y0p = p1y0 + p1dy;

                double p2x0p = p2x0 + p2dx;
                double p2y0p = p2y0 + p2dy;

                double Tpre  = 0.5 * fabs((p0x0 - p2x0) * (p1y0 - p0y0) -
                                          (p0x0 - p1x0) * (p2y0 - p0y0));
                double Tpost = 0.5 * fabs((p0x0p - p2x0p) * (p1y0p - p0y0p) -
                                          (p0x0p - p1x0p) * (p2y0p - p0y0p));

                //double x1 = p1x0p - p0x0p;
                //double y1 = p1y0p - p0y0p;
                //double x2 = p2x0p - p0x0p;
                //double y2 = p2y0p - p0y0p;

                data.core.image[IDi].array.F[jj * size + ii] = Tpre / Tpost;
            }
        }
                                   ...
                                   const char *opd2)
{
    ...
    int USE_2DPROL =
        1; /* if set to 1, the 2D apodized pupil will be used to make the apodization */
    imageID ID_2DPROL;
    ...
    if(USE_2DPROL == 1)
    {
        ...
        ID_2DPROL = image_ID("prol2d", data.core.image, data.core.NB_MAX_IMAGE);
        if(ID_2DPROL == -1)
        {
            load_fits(fname_2DPROL, "prol2d", 1, &ID_2DPROL);
        }
        ...
        for(ii = 0; ii < size2; ii++)
        {
            if(data.core.image[ID_2DPROL].array.F[ii] > 0.00001)
            {
                totalProl += data.core.image[ID_2DPROL].array.F[ii] *
                             data.core.image[ID_2DPROL].array.F[ii];
                totalProlcnt++;
            }
        }
        ...
        for(ii = 0; ii < size2; ii++)
        {
            data.core.image[ID_2DPROL].array.F[ii] /= sqrt(totalProl / totalProlcnt);
        }
    }

    IDa1 = image_ID(ampl1, data.core.image, data.core.NB_MAX_IMAGE);
    IDo1 = image_ID(opd1, data.core.image, data.core.NB_MAX_IMAGE);
        ...
        for(jj2 = 0; jj2 < size; jj2++)
        {
            ...
            if(r2 < 1.0)
            {
                if(piaaconfdirection == 0)
                {
                    ...
                    {
                        /* compute r1 */
                        i = (long)(r2 * piaaconfNBr1fr0_pts / PIAAextfactor1 +
                                   0.5);
                    }
                ...
                else
                {
                    ...
                    {
                        /* compute r1 */
                        i = (long)(r2 * piaaconfNBr1fr0_pts / PIAAextfactor0 +
                                   0.5);
                    }
                ...
                //	    data.core.image[IDr].array.F[jj2*size+ii2] = r1;
                ...
                if((ii > -1) && (jj > -1) && (ii < size - 1) &&
                        (jj < size - 1))
                {
                    /*	data.core.image[IDa2].array.F[jj2*size+ii2] = data.core.image[IDa1].array.F[jj*size+ii]*factor;*/
                    ...
                    if(i < PIAAAPO_NBPOINTS)
                    {
                        if(piaaconfdirection == 0)
                        {
                            if(USE_2DPROL == 1)
                            {
                                data.core.image[IDa2].array.F[jj2 * size + ii2] =
                                    data.core.image[ID_2DPROL]
                                    .array.F[jj2 * size + ii2] *
                                    data.core.image[IDa1].array.F[jjn * size + iin];
                            }
                            else
                            {
                                data.core.image[IDa2].array.F[jj2 * size + ii2] =
                                    data.core.image[IDa1].array.F[jj * size + ii] *
                                    PIAAAPO2D[jj2 * size + ii2] *
                                    PIAA_HYBRID_CPAAPO[i];
                            }
                        }
                        else
                        {
                            data.core.image[IDa2].array.F[jj2 * size + ii2] =
                                data.core.image[IDa1].array.F[jj * size + ii] /
                                (PIAAAPO2D[jj * size + ii] + epsilon);
                        }
                        if(r > PIAACPPMASKRAD1)
                        {
                            data.core.image[IDa2].array.F[jj2 * size + ii2] = 0.0;
                        }

                        if(1)  // bilinear interpolation
                        {
                            data.core.image[IDo2].array.F[jj2 * size + ii2] =
                                (1.0 - t) * (1.0 - u) *
                                data.core.image[IDo1].array.F[jj * size + ii] +
                                t * (1.0 - u) *
                                data.core.image[IDo1]
                                .array.F[jj * size + ii + 1] +
                                t * u *
                                data.core.image[IDo1]
                                .array.F[(jj + 1) * size + ii + 1] +
                                (1.0 - t) * u *
                                data.core.image[IDo1]
                                .array.F[(jj + 1) * size + ii];
                        }
                        else
                        {
                            v00 = data.core.image[IDo1].array.F[jj * size + ii];
                            v01 = data.core.image[IDo1].array.F[jj * size + ii + 1];
                            v10 =
                                data.core.image[IDo1].array.F[(jj + 1) * size + ii];
                            v11 = data.core.image[IDo1]
                                  .array.F[(jj + 1) * size + ii + 1];
                            data.core.image[IDo2].array.F[jj2 * size + ii2] = 0.0;
                            coeff                                      = 0.0;
                            eps                                        = 1.0e-5;
                            if(v00 > eps)
                            {
                        ...
                        total += data.core.image[IDo2].array.F[jj2 * size + ii2];
                        totalcnt += 1.0;
                    }
                }
                else
    ...
    if(r2<1.0)
      data.core.image[IDo2].array.F[jj2*size+ii2] -= total;
      }*/
    ...
    delete_image_ID("rtest", DELETE_IMAGE_ERRMODE_WARNING);
    exit(0);
    */
...
int coronagraph_init_CPA()
{
    ...
    for(long i = 0; i < CPAAPO_NBPOINTS; i++)
    {
        long   tmpl;
        double tmpf;
        {
    ...
    for(long i = 1; i < CPAAPO_NBPOINTS; i++)
    {
        CPAAPO[i] /= CPAAPO[0];
    }
    CPAAPO[0] = 1.0;
    ...
    for(uint32_t ii = 0; ii < factor * size; ii++)
        for(uint32_t jj = 0; jj < factor * size; jj++)
        {
            ...
            if((ii1 > 0) && (jj1 > 0) && (ii1 < size) && (jj1 < size))
            {
                data.core.image[ID].array.F[jj1 * size + ii1] +=
                    value / factor / factor;
            }
        }
...
int coronagraph_init_ODC()
{
    ...
    for(ii = 0; ii < factor * size; ii++)
    {
        ...
        if((ii1 > 0) && (ii1 < size))
            for(jj = 0; jj < size; jj++)
            {
                data.core.image[ID].array.F[jj * size + ii1] += tmp / factor;
            }
    }
...
int coronagraph_init_RRPM()
{
    ...
    ID = make_subpixdisk("rrpmreffm",
                         size,
                         size,
                         size / 2,
                         size / 2,
                         RRPM_RADIUS / CORONAGRAPHS_PIXSCALE);
    for(ii = 0; ii < size2; ii++)
    {
        data.core.image[ID].array.F[ii] = 1.0 - 2.0 * data.core.image[ID].array.F[ii];
    }
    ...
    for(ii = 0; ii < size; ii++)
        for(jj = 0; jj < size; jj++)
        {
            ...
            data.core.image[ID].array.F[jj * size + ii] =
                1.0 + RRPM_P2 * r * r + RRPM_P3 * pow(r, 3.0) +
                RRPM_P4 * pow(r, 4.0) + RRPM_P5 * pow(r, 5.0);
            if(data.core.image[ID].array.F[jj * size + ii] > 1.0)
            {
                data.core.image[ID].array.F[jj * size + ii] = 1.0;
            }
        }
...
int coronagraph_init_OVC(long charge)
{
    ...
    for(ii = 0; ii < size; ii++)
        for(jj = 0; jj < size; jj++)
        {
            ...
            re /= cnt;
            im /= cnt;
            data.core.image[ID2].array.F[jj * size + ii] = sqrt(re * re + im * im);
            data.core.image[ID1].array.F[jj * size + ii] = atan2(im, re);
        }

    sprintf(fname1,
            "%s/ovcref_fmp%ld.ref.%ld",
            CORONAGRAPHSDATALOCAL,
            charge,
            size);
    save_fl_fits(fnamep, fname1);
    sprintf(fname1,
            "%s/ovcref_fma%ld.ref.%ld",
            CORONAGRAPHSDATALOCAL,
            charge,
            size);
...
int coronagraph_simul_SHEAR4(double xld, double yld, const char *psfname)
{
    ...
    ID = image_ID("corphase", data.core.image, data.core.NB_MAX_IMAGE);
    if(ID != -1)
    {
        for(ii = 0; ii < size2; ii++)
        {
            data.core.image[IDp1].array.F[ii] += data.core.image[ID].array.F[ii];
        }
    }
    ...
    for(jj = iistart; jj < iiend; jj++)
        for(ii = iistart; ii < iiend; ii++)
        {
            ...
            a1 = data.core.image[IDa1].array.F[index1];
            p1 = data.core.image[IDp1].array.F[index1];
            r1 = a1 * cos(p1);
            i1 = a1 * sin(p1);

            a2 = data.core.image[IDa1].array.F[index2];
            p2 = data.core.image[IDp1].array.F[index2];
            r2 = a2 * cos(p2);
            i2 = a2 * sin(p2);

            a3 = data.core.image[IDa1].array.F[index3];
            p3 = data.core.image[IDp1].array.F[index3];
            r3 = a3 * cos(p3);
            i3 = a3 * sin(p3);

            a4 = data.core.image[IDa1].array.F[index4];
            p4 = data.core.image[IDp1].array.F[index4];
            r4 = a4 * cos(p4);
            i4 = a4 * sin(p4);

            if(a1 * a2 * a3 * a4 > epsilon)
            {
                data.core.image[IDc].array.CF[jj * size + ii].re =
                    0.25 * (r1 - r2 - r3 + r4);
                data.core.image[IDc].array.CF[jj * size + ii].im =
                    0.25 * (i1 - i2 - i3 + i4);
            }
        }
    ...
    ID = image_ID(psfname, data.core.image, data.core.NB_MAX_IMAGE);
    delete_image_ID("pc2", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("fc2", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("fa2", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("fp2", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pa1", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pp1", DELETE_IMAGE_ERRMODE_WARNING);
    for(ii = 0; ii < size2; ii++)
    {
        data.core.image[ID].array.F[ii] /= total * size2;
    }
...
int coronagraph_simul_DICC(double xld, double yld, const char *psfname)
{
    ...
    ID_DICC1 = image_ID("dicc1", data.core.image, data.core.NB_MAX_IMAGE);
    if(ID_DICC1 == -1)
    {
        ...
        for(ii = 0; ii < size; ii++)
            for(jj = 0; jj < size; jj++)
            {
                if(data.core.image[IDaref].array.F[jj * size + ii] > 0.5)
                {
                    x = 1.0 * (ii - size / 2);
                    y = 1.0 * (jj - size / 2);
                    totalx2s += x * x;
        ...
        for(ii = 0; ii < size; ii++)
            for(jj = 0; jj < size; jj++)
            {
                if(data.core.image[IDaref].array.F[jj * size + ii] > 0.5)
                {
                    x = 1.0 * (ii - size / 2);
                    y = 1.0 * (jj - size / 2);
                    data.core.image[ID_DICC1].array.F[jj * size + ii] = 1.0;
                    total1 += 1.0;
                    data.core.image[ID_DICCX].array.F[jj * size + ii] = x;
                    totalx += x * x;
                    data.core.image[ID_DICCY].array.F[jj * size + ii] = y;
                    totaly += y * y;
                    data.core.image[ID_DICCX2].array.F[jj * size + ii] =
                        x * x - totalx2s;
                    totalx2 += data.core.image[ID_DICCX2].array.F[jj * size + ii] *
                               data.core.image[ID_DICCX2].array.F[jj * size + ii];
                    data.core.image[ID_DICCY2].array.F[jj * size + ii] =
                        y * y - totaly2s;
                    totaly2 += data.core.image[ID_DICCY2].array.F[jj * size + ii] *
                               data.core.image[ID_DICCY2].array.F[jj * size + ii];
                    data.core.image[ID_DICCXY].array.F[jj * size + ii] = x * y;
                    totalxy += x * y * x * y;
                }
            }
        for(ii = 0; ii < size; ii++)
            for(jj = 0; jj < size; jj++)
            {
                if(data.core.image[IDaref].array.F[jj * size + ii] > 0.5)
                {
                    data.core.image[ID_DICC1].array.F[jj * size + ii] /=
                        sqrt(total1);
                    data.core.image[ID_DICCX].array.F[jj * size + ii] /=
                        sqrt(totalx);
                    data.core.image[ID_DICCY].array.F[jj * size + ii] /=
                        sqrt(totaly);
                    data.core.image[ID_DICCX2].array.F[jj * size + ii] /=
                        sqrt(totalx2);
                    data.core.image[ID_DICCY2].array.F[jj * size + ii] /=
                        sqrt(totaly2);
                    data.core.image[ID_DICCXY].array.F[jj * size + ii] /=
                        sqrt(totalxy);
                }
            }
        ...
        for(ii = 0; ii < size; ii++)
            for(jj = 0; jj < size; jj++)
            {
                if(data.core.image[IDaref].array.F[jj * size + ii] > 0.5)
                {
                    totalx2y2 += data.core.image[ID_DICCX2].array.F[jj * size + ii] *
                                 data.core.image[ID_DICCY2].array.F[jj * size + ii];
                }
            }
        totaly2 = 0.0;
    ...
    ID_DICCX  = image_ID("diccx", data.core.image, data.core.NB_MAX_IMAGE);
    ID_DICCY  = image_ID("diccy", data.core.image, data.core.NB_MAX_IMAGE);
    ID_DICCX2 = image_ID("diccx2", data.core.image, data.core.NB_MAX_IMAGE);
    ID_DICCY2 = image_ID("diccy2", data.core.image, data.core.NB_MAX_IMAGE);
    ID_DICCXY = image_ID("diccxy", data.core.image, data.core.NB_MAX_IMAGE);
    ...
    for(iter = 0; iter < NBiter; iter++)
    {
        ...
        for(ii = 0; ii < size; ii++)
            ...
            {
                index = jj * size + ii;
                if(data.core.image[IDaref].array.F[index] > 0.5)
                {
                    amp = data.core.image[IDa1].array.F[index];
                    pha = data.core.image[IDp1].array.F[index];
                    re  = amp * cos(pha);
                    im  = amp * sin(pha);
                    v1r += re * data.core.image[ID_DICC1].array.F[index];
                    v1i += im * data.core.image[ID_DICC1].array.F[index];
                    vxr += re * data.core.image[ID_DICCX].array.F[index];
                    vxi += im * data.core.image[ID_DICCX].array.F[index];
                    vyr += re * data.core.image[ID_DICCY].array.F[index];
                    vyi += im * data.core.image[ID_DICCY].array.F[index];
                    vx2r += re * data.core.image[ID_DICCX2].array.F[index];
                    vx2i += im * data.core.image[ID_DICCX2].array.F[index];
                    vy2r += re * data.core.image[ID_DICCY2].array.F[index];
                    vy2i += im * data.core.image[ID_DICCY2].array.F[index];
                    vxyr += re * data.core.image[ID_DICCXY].array.F[index];
                    vxyi += im * data.core.image[ID_DICCXY].array.F[index];
                }
            }
        ...
        for(ii = 0; ii < size; ii++)
            for(jj = 0; jj < size; jj++)
            {
                ...
                if(data.core.image[IDaref].array.F[index] > 0.5)
                {
                    ...
                    re  = amp * cos(pha);
                    im  = amp * sin(pha);
                    re -= v1r * data.core.image[ID_DICC1].array.F[index];
                    im -= v1i * data.core.image[ID_DICC1].array.F[index];
                    re -= vxr * data.core.image[ID_DICCX].array.F[index];
                    im -= vxi * data.core.image[ID_DICCX].array.F[index];
                    re -= vyr * data.core.image[ID_DICCY].array.F[index];
                    im -= vyi * data.core.image[ID_DICCY].array.F[index];
                    re -= vx2r * data.core.image[ID_DICCX2].array.F[index];
                    im -= vx2i * data.core.image[ID_DICCX2].array.F[index];
                    re -= vy2r * data.core.image[ID_DICCY2].array.F[index];
                    im -= vy2i * data.core.image[ID_DICCY2].array.F[index];
                    re -= vxyr * data.core.image[ID_DICCXY].array.F[index];
                    im -= vxyi * data.core.image[ID_DICCXY].array.F[index];
                    data.core.image[IDa1].array.F[index] = sqrt(re * re + im * im);
                    data.core.image[IDp1].array.F[index] = atan2(im, re);
                }
            }
    }
    delete_image_ID("paref", DELETE_IMAGE_ERRMODE_WARNING);
    for(index = 0; index < size2; index++)
    {
        ...
        re                                 = amp * cos(pha);
        im                                 = amp * sin(pha);
        data.core.image[IDc].array.CF[index].re = re;
        data.core.image[IDc].array.CF[index].im = im;
    }
...
int coronagraph_simul_AIC(double xld, double yld, const char *psfname)
{
    ...
    for(jj = iistart; jj < iiend; jj++)
    {
        ...
        for(ii = iistart; ii < iiend; ii++)
        {
            index1 = index1a + ii;
            index2 = index2a - ii;
            a11    = data.core.image[IDa1].array.F[index1];
            if(a11 > epsilon)
            {
                p11 = data.core.image[IDp1].array.F[index1];
                a12 = data.core.image[IDa1].array.F[index2];
                p12 = data.core.image[IDp1].array.F[index2];
                r11 = a11 * cos(p11);
                r12 = a12 * cos(p12);
                i11 = a11 * sin(p11);
                i12 = a12 * sin(p12);
                data.core.image[IDc].array.CF[index1].re = 0.5 * (r11 - r12);
                data.core.image[IDc].array.CF[index1].im = 0.5 * (i11 - i12);
            }
        }
    }
    permut("pc2");
...
int coronagraph_simul_4QPM(double xld, double yld, const char *psfname)
{
    ...
    IDrefr = image_ID("ref4qre", data.core.image, data.core.NB_MAX_IMAGE);
    if(IDrefr == -1)
    {
        if(file_exists(fname1) == 1)
        {
            load_fits(fname1, "ref4qre", 1, &IDrefr);
        }
        else
        {
            init4q = 1;
    ...
    IDrefi = image_ID("ref4qim", data.core.image, data.core.NB_MAX_IMAGE);
    if(IDrefi == -1)
    {
        if(file_exists(fname1) == 1)
        {
            load_fits(fname1, "ref4qim", 1, &IDrefi);
        }
        else
        {
            init4q = 1;
    ...
    total = arith_image_total("pa1");
    IDp1 =
        make_slopexy("pp1", size, size, PI * x / trad_pix, PI * y / trad_pix);
    ...
    do2dfft("pc2", "fc2");
    permut("fc2");
    ID = image_ID("fc2", data.core.image, data.core.NB_MAX_IMAGE);
    for(ii = 0; ii < size; ii++)
        for(jj = 0; jj < size; jj++)
        {
            if((ii > size / 2) && (jj > size / 2))
            {
                data.core.image[ID].array.CF[jj * size + ii].re *= -1.0;
                data.core.image[ID].array.CF[jj * size + ii].im *= -1.0;
            }
            if((ii < size / 2) && (jj < size / 2))
            {
                data.core.image[ID].array.CF[jj * size + ii].re *= -1.0;
    ...
    for(ii = 0; ii < size; ii++)
    {
        data.core.image[ID].array.CF[size / 2 * size + ii].re = 0.0;
        data.core.image[ID].array.CF[size / 2 * size + ii].im = 0.0;
        data.core.image[ID].array.CF[ii * size + size / 2].re = 0.0;
        data.core.image[ID].array.CF[ii * size + size / 2].im = 0.0;
    }
    permut("fc2");
    do2dfft("fc2", "pc3");
    permut("pc3");
    ID = image_ID("pc3", data.core.image, data.core.NB_MAX_IMAGE);
    for(ii = 0; ii < size2; ii++)
        if(data.core.image[IDa1].array.F[ii] < 0.99999)
        {
            data.core.image[ID].array.CF[ii].re = 0.0;
            data.core.image[ID].array.CF[ii].im = 0.0;
        }
    ...
    if(init4q == 1)
    {
        ...
        for(ii = 0; ii < size2; ii++)
        {
            data.core.image[IDrefr].array.F[ii] = data.core.image[ID].array.CF[ii].re;
            data.core.image[IDrefi].array.F[ii] = data.core.image[ID].array.CF[ii].im;
        }
        sprintf(fname1, "%s/4qre.ref.%ld", CORONAGRAPHSDATALOCAL, size);
    ...
    else
    {
        for(ii = 0; ii < size2; ii++)
        {
            data.core.image[ID].array.CF[ii].re -= data.core.image[IDrefr].array.F[ii];
            data.core.image[ID].array.CF[ii].im -= data.core.image[IDrefi].array.F[ii];
        }
    }
    permut("pc3");
    do2dfft("pc3", "fc4");
    ...
    delete_image_ID("pc2", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("fc2", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("fc4", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("fa4", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("fp4", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pa1", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pp1", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pc3", DELETE_IMAGE_ERRMODE_WARNING);
    for(ii = 0; ii < size2; ii++)
    {
        data.core.image[ID].array.F[ii] /= total * size2 * size2 * size2;
    }
...
int coronagraph_simul_ODC(double xld, double yld, const char *psfname)
{
    ...
    char     fname1r[200];
    char     fname1i[200];
    imageID  IDrefr, IDrefi;

    //double x = xld;
    //double y = yld;

    sprintf(fname1, "%s/odcre.ref.%ul", CORONAGRAPHSDATALOCAL, size);
    sprintf(fname, "refocdre");
    IDrefr = image_ID(fname, data.core.image, data.core.NB_MAX_IMAGE);

    if(IDrefr == -1)
    {
        if(file_exists(fname1) == 1)
        {
            load_fits(fname1, fname, 1, &IDrefr);
        }
        else
        {
            initodc = 1;
    ...
    sprintf(fname1, "%s/odcim.ref.%ul", CORONAGRAPHSDATALOCAL, size);
    sprintf(fname, "refodcim");
    IDrefi = image_ID(fname, data.core.image, data.core.NB_MAX_IMAGE);

    if(IDrefi == -1)
    {
        if(file_exists(fname1) == 1)
        {
            load_fits(fname1, fname, 1, &IDrefi);
        }
        else
        {
            initodc = 1;
    ...
    sprintf(refname, "%s/odcm.ref.%ul", CORONAGRAPHSDATALOCAL, size);
    sprintf(refname1, "odcmref");
    IDrefm = image_ID(refname1, data.core.image, data.core.NB_MAX_IMAGE);

    if(IDrefm == -1)
    {
        if(file_exists(refname) == 1)
        {
            load_fits(refname, refname1, 1, &IDrefm);
        }
        else
        {
    ...
    trad_pix = CORONAGRAPHS_TDIAM / CORONAGRAPHS_PSCALE / 2.0;
    //IDa1 =
    make_subpixdisk("pa1", size, size, 0.5 * size, 0.5 * size, trad_pix);
    total        = arith_image_total("pa1");
    imageID IDp1 = make_slopexy("pp1",
                                size,
                                size,
                                PI * xld / trad_pix,
                                PI * yld / trad_pix);
    ...
    for(uint64_t ii = 0; ii < size2; ii++)
    {
        data.core.image[ID].array.CF[ii].re *= data.core.image[IDrefm].array.F[ii];
        data.core.image[ID].array.CF[ii].im *= data.core.image[IDrefm].array.F[ii];
    }
    ...
    for(uint32_t ii = 0; ii < size; ii++)
        for(uint32_t jj = 0; jj < size; jj++)
        {
            double tmp1 = 1.0 * ii - size / 2;
            double tmp2 = 1.0 * jj - size / 2;
            double r    = sqrt(tmp1 * tmp1 + tmp2 * tmp2) / trad_pix;
            if(r > ODCMASK_eps)
            {
                data.core.image[ID].array.CF[jj * size + ii].re = 0.0;
                data.core.image[ID].array.CF[jj * size + ii].im = 0.0;
            }
        }
...
int coronagraph_simul_BL8(double xld, double yld, const char *psfname)
{
    ...
    if(BL8MODE == 0)
    {
        sprintf(fname1, "%s/bl8re.ref.%ul", CORONAGRAPHSDATALOCAL, size);
        sprintf(fname, "refbl8re");
    }
    else
    {
        sprintf(fname1, "%s/bl8lre.ref.%ul", CORONAGRAPHSDATALOCAL, size);
    ...
    if(BL8MODE == 0)
    {
        sprintf(fname1, "%s/bl8im.ref.%ul", CORONAGRAPHSDATALOCAL, size);
        sprintf(fname, "refbl8im");
    }
    else
    {
        sprintf(fname1, "%s/bl8lim.ref.%ul", CORONAGRAPHSDATALOCAL, size);
    ...
    if(BL8MODE == 0)
    {
        sprintf(refname, "%s/bl8m.ref.%ul", CORONAGRAPHSDATALOCAL, size);
        sprintf(refname1, "bl8mref");
    }
    else
    {
        sprintf(refname, "%s/bl8ml.ref.%ul", CORONAGRAPHSDATALOCAL, size);
    ...
    permut("pc3");
    ID            = image_ID("pc3", data.core.image, data.core.NB_MAX_IMAGE);
    long iioffset = (long)(BL8MASK_eps * trad_pix);
    if(BL8MODE == 0)
    {
        for(uint32_t ii = 0; ii < size; ii++)
            for(uint32_t jj = 0; jj < size; jj++)
    ...
    else
    {
        for(uint32_t ii = 0; ii < size; ii++)
            for(uint32_t jj = 0; jj < size; jj++)
            {
                double tmp1  = (1.0 * ii - size / 2) - iioffset;
                double tmp1b = (1.0 * ii - size / 2) + iioffset;
                double tmp2  = 1.0 * jj - size / 2;
                double r1    = sqrt(tmp1 * tmp1 + tmp2 * tmp2) / trad_pix;
                double r2    = sqrt(tmp1b * tmp1b + tmp2 * tmp2) / trad_pix;
                if((r1 > 1.0) || (r2 > 1.0))
                {
                    data.core.image[ID].array.CF[jj * size + ii].re = 0.0;
                    data.core.image[ID].array.CF[jj * size + ii].im = 0.0;
...
int coronagraph_simul_BL4(double xld, double yld, const char *psfname)
{
    ...
    sprintf(fname1, "%s/bl4re.ref.%ul", CORONAGRAPHSDATALOCAL, size);
    ...
    sprintf(fname1, "%s/bl4im.ref.%ul", CORONAGRAPHSDATALOCAL, size);
    ...
    sprintf(refname, "%s/bl4m.ref.%ul", CORONAGRAPHSDATALOCAL, size);
    ...
    permut("pc3");
    ID            = image_ID("pc3", data.core.image, data.core.NB_MAX_IMAGE);
    long iioffset = (long)(BL4MASK_eps * trad_pix + 0.2);
    iioffset += 1;
    ...
    for(uint32_t ii = 0; ii < size; ii++)
        for(uint32_t jj = 0; jj < size; jj++)
        {
            ...
            double tmp1b = (1.0 * ii - size / 2) + iioffset;
            double tmp2  = 1.0 * jj - size / 2;
            double r1    = (sqrt(tmp1 * tmp1 + tmp2 * tmp2) + 1.0) / trad_pix;
            double r2    = (sqrt(tmp1b * tmp1b + tmp2 * tmp2) + 1.0) / trad_pix;
            if((r1 > 1.0) || (r2 > 1.0))
            {
                data.core.image[ID].array.CF[jj * size + ii].re = 0.0;
                data.core.image[ID].array.CF[jj * size + ii].im = 0.0;
...
int coronagraph_simul_RRPM(double xld, double yld, const char *psfname)
{
    ...
    sprintf(PIAAAPO_FNAME, "rrpmref_pm.ref.%ul.prof", size);
    sprintf(PIAAAPODIZE_2DAPOFNAME, "rrpmref_pm.ref.%ul", size);

    sprintf(refname, "%s/rrpmref_fm.ref.%ul", CORONAGRAPHSDATALOCAL, size);
    sprintf(refname1, "rrpmreffm");
    IDref_fm = image_ID(refname1, data.core.image, data.core.NB_MAX_IMAGE);
    if(IDref_fm == -1)
    {
        if(file_exists(refname) == 1)
        {
            load_fits(refname, refname1, 1, &IDref_fm);
        }
        else
        {
    ...
    sprintf(refname, "%s/rrpmref_pm.ref.%ul", CORONAGRAPHSDATALOCAL, size);
    sprintf(refname1, "rrpmrefpm");
    IDref_pm = image_ID(refname1, data.core.image, data.core.NB_MAX_IMAGE);
    if(IDref_pm == -1)
    {
        if(file_exists(refname) == 1)
        {
            load_fits(refname, refname1, 1, &IDref_pm);
        }
        else
        {
            coronagraph_init_RRPM();
    ...
    IDrefr = image_ID("refrrpmre", data.core.image, data.core.NB_MAX_IMAGE);
    if(IDrefr == -1)
    {
        if(file_exists(fname1) == 1)
        {
            load_fits(fname1, "refrrpmre", 1, &IDrefr);
        }
        else
        {
            initrrpm = 1;
    ...
    IDrefi = image_ID("refrrpmim", data.core.image, data.core.NB_MAX_IMAGE);
    if(IDrefi == -1)
    {
        if(file_exists(fname1) == 1)
        {
            load_fits(fname1, "refrrpmim", 1, &IDrefi);
        }
        else
        {
            initrrpm = 1;
    ...
    for(uint64_t ii = 0; ii < size2; ii++)
    {
        data.core.image[IDa2].array.F[ii] =
            data.core.image[IDa1].array.F[ii] * data.core.image[IDref_pm].array.F[ii];
    }
    ...
    else
    {
        ...
        if(initPIAA == 0)
        {
            coronagraph_init_PIAA(NULL);
            initPIAA = 1;
        }
        coronagraphs_PIAA_apodize_beam("pa1", "pp1", "pa2", "pp2");
        IDa2 = image_ID("pa2", data.core.image, data.core.NB_MAX_IMAGE);
        IDp2 = image_ID("pp2", data.core.image, data.core.NB_MAX_IMAGE);
        //      save_fl_fits("pa2","pa2");
        //      save_fl_fits("pp2","pp2");
        for(uint64_t ii = 0; ii < size2; ii++)
        {
            data.core.image[IDp1].array.F[ii] = data.core.image[IDp2].array.F[ii];
        }
        double tmp1 = 0.0;
        double tmp2 = 0.0;
        for(uint64_t ii = 0; ii < size2; ii++)
        {
            v1 = data.core.image[IDa1].array.F[ii];
            v2 = data.core.image[IDref_pm].array.F[ii];
            tmp1 += v1 * v1;
            if(v1 > 0.5)
            {
                tmp2 += v2 * v2;
        ...
        delete_image_ID("pp2", DELETE_IMAGE_ERRMODE_WARNING);
    }
    ...
    for(uint64_t ii = 0; ii < size2; ii++)
    {
        data.core.image[ID].array.CF[ii].re *= data.core.image[IDref_fm].array.F[ii];
        data.core.image[ID].array.CF[ii].im *= data.core.image[IDref_fm].array.F[ii];
    }
    ...
    for(uint32_t ii = 0; ii < size; ii++)
        for(uint32_t jj = 0; jj < size; jj++)
        {
            ...
            //double tmp2 = 1.0 * jj - size / 2;
            //double r = sqrt(tmp1 * tmp1 + tmp2 * tmp2) / trad_pix;
            if(data.core.image[IDa1].array.F[jj * size + ii] < 0.999)
            {
                data.core.image[ID].array.CF[jj * size + ii].re = 0.0;
                data.core.image[ID].array.CF[jj * size + ii].im = 0.0;
            }
    ...
    delete_image_ID("fp4", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pa1", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pa2", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pp1", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pc3", DELETE_IMAGE_ERRMODE_WARNING);
    for(uint64_t ii = 0; ii < size2; ii++)
    {
        data.core.image[ID].array.F[ii] /= total * size2 * size2 * size2 / factor;
    }
...
int coronagraph_simul_OVC(double xld, double yld, const char *psfname)
{
    ...
    IDref_fmp = image_ID(refname1, data.core.image, data.core.NB_MAX_IMAGE);
    if(IDref_fmp == -1)
    {
        if(file_exists(refname) == 1)
        {
            load_fits(refname, refname1, 1, &IDref_fmp);
        }
        else
        {
    ...
    IDref_fma = image_ID(refname1, data.core.image, data.core.NB_MAX_IMAGE);
    if(IDref_fma == -1)
    {
        if(file_exists(refname) == 1)
        {
            load_fits(refname, refname1, 1, &IDref_fma);
        }
        else
        {
            coronagraph_init_OVC(OVC_CHARGE);
    ...
    sprintf(fname1,
            ...
            OVC_CHARGE,
            size);
    IDrefr = image_ID("refovcre", data.core.image, data.core.NB_MAX_IMAGE);
    if(IDrefr == -1)
    {
        if(file_exists(fname1) == 1)
        {
            load_fits(fname1, "refovcre", 1, &IDrefr);
        }
        else
        {
            initovc = 1;
    ...
    sprintf(fname1,
            ...
            OVC_CHARGE,
            size);
    IDrefi = image_ID("refovcim", data.core.image, data.core.NB_MAX_IMAGE);
    if(IDrefi == -1)
    {
        if(file_exists(fname1) == 1)
        {
            load_fits(fname1, "refovcim", 1, &IDrefi);
        }
        else
        {
            initovc = 1;
    ...
    if(sqrt(x1*x1+y1*y1)<trad_pix*0.2)
        data.core.image[IDa1].array.F[jj*size+ii] = 0.0;
        }
    */

    total = arith_image_total("pa1");
    ID    = image_ID("coramp", data.core.image, data.core.NB_MAX_IMAGE);
    if(ID != -1)
    {
        printf("---------- USING AMPLITUDE FILE ----------\n");
        for(ii = 0; ii < size2; ii++)
        {
            data.core.image[IDa1].array.F[ii] = data.core.image[ID].array.F[ii];
        }
    }

    IDp1 =
        make_slopexy("pp1", size, size, PI * x / trad_pix, PI * y / trad_pix);
    ID = image_ID("corpha", data.core.image, data.core.NB_MAX_IMAGE);
    if(ID != -1)
    {
    ...
    for(ii = 0; ii < size2; ii++)
    {
        ...
        pha = atan2(im, re);
        amp *= data.core.image[IDref_fma].array.F[ii];
        pha += data.core.image[IDref_fmp].array.F[ii];
        data.core.image[ID].array.CF[ii].re = amp * cos(pha);
        data.core.image[ID].array.CF[ii].im = amp * sin(pha);
    }
    ...
    if(0 == 0)
    {
        ...
        delete_image_ID("pc3a", DELETE_IMAGE_ERRMODE_WARNING);
        delete_image_ID("pc3p", DELETE_IMAGE_ERRMODE_WARNING);
    }
    ...
    */
    ...
    for(ii = 0; ii < size; ii++)
        for(jj = 0; jj < size; jj++)
        {
            ...
            tmp2 = 1.0 * jj - size / 2;
            r    = sqrt(tmp1 * tmp1 + tmp2 * tmp2) / trad_pix;
            //	if(data.core.image[IDa1].array.F[jj*size+ii]<0.99999)
            if(r > 0.96)
            {
                data.core.image[ID].array.CF[jj * size + ii].re = 0.0;
                data.core.image[ID].array.CF[jj * size + ii].im = 0.0;
...
int coronagraph_simul_CPA(double xld, double yld, const char *psfname)
{
    ...
    IDref = image_ID(refname1, data.core.image, data.core.NB_MAX_IMAGE);
    if(IDref == -1)
    {
        if(file_exists(refname) == 1)
        {
            load_fits(refname, refname1, 1, &IDref);
        }
        else
        {
    ...
    {
        data.core.image[IDa2].array.F[ii] =
            data.core.image[IDa1].array.F[ii] * data.core.image[IDref].array.F[ii];
    }
    ...
    for(ii = 0; ii < size; ii++)
        for(jj = 0; jj < size; jj++)
        {
            ...
            tmp2 = 1.0 * jj - size / 2;
            r    = sqrt(tmp1 * tmp1 + tmp2 * tmp2) / trad_pix;
            if(data.core.image[IDa1].array.F[jj * size + ii] > CPAPPMASKRAD)
            {
                data.core.image[ID].array.CF[jj * size + ii].re = 0.0;
                data.core.image[ID].array.CF[jj * size + ii].im = 0.0;
            }
...
int coronagraph_simul_PPA(double xld, double yld, const char *psfname)
{
    ...
    IDrefa = image_ID("pparefa", data.core.image, data.core.NB_MAX_IMAGE);
    IDrefp = image_ID("pparefp", data.core.image, data.core.NB_MAX_IMAGE);
    ...
    if((IDrefa == -1) || (IDrefp == -1))
    {
        ...
        for(k = 0; k < NBsample; k++)
        {
            ...
            pha_array[k] =
                a * log(((1.0 + epsilon) + x) / ((1.0 + epsilon) - x));
            index_array[k] =
                (long)(x * (trad_pix - 1.0) / sqrt(2.0) + size / 2);
        }
        ...
        for(kx = 0; kx < NBsample; kx++)
        {
            ...
            for(ky = 0; ky < NBsample; ky++)
            {
                ...
                pha   = phax + phay;
                index = jj * size + ii;
                data.core.image[IDrefr].array.F[index] += cos(pha);
                data.core.image[IDrefi].array.F[index] += sin(pha);
                data.core.image[IDcnt].array.F[index] += 1.0;
            }
        }

        for(ii = 0; ii < size2; ii++)
            if(data.core.image[IDcnt].array.F[ii] > 0.5)
            {
                re = data.core.image[IDrefr].array.F[ii] /
                     data.core.image[IDcnt].array.F[ii];
                im = data.core.image[IDrefi].array.F[ii] /
                     data.core.image[IDcnt].array.F[ii];
                data.core.image[IDrefa].array.F[ii] = sqrt(re * re + im * im);
                data.core.image[IDrefp].array.F[ii] = atan2(im, re);
            }
        delete_image_ID("pparefr", DELETE_IMAGE_ERRMODE_WARNING);
        delete_image_ID("pparefi", DELETE_IMAGE_ERRMODE_WARNING);
        delete_image_ID("cntref", DELETE_IMAGE_ERRMODE_WARNING);
        free(pha_array);
        free(index_array);
    }
    ...
    for(ii = 0; ii < size; ii++)
        for(jj = 0; jj < size; jj++)
        {
            data.core.image[IDp1].array.F[jj * size + ii] +=
                data.core.image[IDrefp].array.F[jj * size + ii];
            data.core.image[IDa2].array.F[jj * size + ii] =
                data.core.image[IDa1].array.F[jj * size + ii] *
                data.core.image[IDrefa].array.F[jj * size + ii];
        }
...
int coronagraph_simul_PIAA(double xld, double yld, const char *psfname)
{
    ...
    make_subpixdisk("pa1",
                    ...
                    0.5 * size,
                    0.5 * size,
                    trad_pix * PIAAOVERSIZE);
    double  total = arith_image_total("pa1") / PIAAOVERSIZE / PIAAOVERSIZE;
    imageID IDp1  = make_slopexy("pp1",
                                 size,
                                 size,
                                 PI * xld / trad_pix,
                                 PI * yld / trad_pix);
    imageID ID    = image_ID("corphase", data.core.image, data.core.NB_MAX_IMAGE);
    if(ID != -1)
    {
        for(uint64_t ii = 0; ii < size2; ii++)
        {
    ...
    if(PIAAFPMASK == 1)
    {
        for(uint32_t ii = 0; ii < size; ii++)
            for(uint32_t jj = 0; jj < size; jj++)
            {
                ...
                double tmp2 = 1.0 * jj - size / 2;
                double r =
                    sqrt(tmp1 * tmp1 + tmp2 * tmp2) * CORONAGRAPHS_PIXSCALE;
                if(r < PIAAFPMASKRAD)
                {
                    data.core.image[ID].array.CF[jj * size + ii].re = 0.0;
                    data.core.image[ID].array.CF[jj * size + ii].im = 0.0;
    ...
    if(PIAALOWFS == 1)
    {
        ...
        imageID IDp = image_ID("pc3p", data.core.image, data.core.NB_MAX_IMAGE);
        imageID IDm = image_ID("pc3m", data.core.image, data.core.NB_MAX_IMAGE);
        ID          = image_ID("pc3", data.core.image, data.core.NB_MAX_IMAGE);
        for(uint32_t ii = 0; ii < size; ii++)
            for(uint32_t jj = 0; jj < size; jj++)
            {
                ...
                if(r < PIAAPPMASKRAD)
                {
                    double re   = data.core.image[ID].array.CF[jj * size + ii].re;
                    double im   = data.core.image[ID].array.CF[jj * size + ii].im;
                    double amp  = sqrt(re * re + im * im);
                    double pha  = atan2(im, re);
                    double phap = pha + r * r * 6.0;
                    double pham = pha - r * r * 6.0;
                    data.core.image[IDp].array.CF[jj * size + ii].re =
                        amp * cos(phap);
                    data.core.image[IDp].array.CF[jj * size + ii].im =
                        amp * sin(phap);
                    data.core.image[IDm].array.CF[jj * size + ii].re =
                        amp * cos(pham);
                    data.core.image[IDm].array.CF[jj * size + ii].im =
                        amp * sin(pham);
                }
            }
        ...
        ID = image_ID("psfWFSp", data.core.image, data.core.NB_MAX_IMAGE);
        for(uint64_t ii = 0; ii < size2; ii++)
        {
            data.core.image[ID].array.F[ii] /= total * size2 * size2 * size2;
        }
        delete_image_ID("pc3p", DELETE_IMAGE_ERRMODE_WARNING);
        delete_image_ID("fc4p", DELETE_IMAGE_ERRMODE_WARNING);
        delete_image_ID("fa4p", DELETE_IMAGE_ERRMODE_WARNING);
        delete_image_ID("fp4p", DELETE_IMAGE_ERRMODE_WARNING);
        ...
        ID = image_ID("psfWFSm", data.core.image, data.core.NB_MAX_IMAGE);
        for(uint64_t ii = 0; ii < size2; ii++)
        {
            data.core.image[ID].array.F[ii] /= total * size2 * size2 * size2;
        }
        delete_image_ID("pc3m", DELETE_IMAGE_ERRMODE_WARNING);
        delete_image_ID("fc4m", DELETE_IMAGE_ERRMODE_WARNING);
        delete_image_ID("fa4m", DELETE_IMAGE_ERRMODE_WARNING);
        delete_image_ID("fp4m", DELETE_IMAGE_ERRMODE_WARNING);
    }
...
int coronagraph_simul_PIAAC(double xld, double yld, const char *psfname)
{
    ...
    IDp1 = image_ID("pp1", data.core.image, data.core.NB_MAX_IMAGE);*/
    ID = image_ID("corphase", data.core.image, data.core.NB_MAX_IMAGE);
    if(ID != -1)
    {
        for(uint64_t ii = 0; ii < size2; ii++)
    ...
    delete_image_ID("tfa2", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("tfp2", DELETE_IMAGE_ERRMODE_WARNING);
    */
    ...
    delete_image_ID("pa3t", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pp3t", DELETE_IMAGE_ERRMODE_WARNING);
    ...
    IDb = image_ID("pp3b", data.core.image, data.core.NB_MAX_IMAGE);
    for(uint64_t ii = 0; ii < size * size; ii++)
    {
        double tmp1 = data.core.image[IDb].array.F[ii];
        tmp1 += 2.0 * PI / 3.0;
        if(tmp1 > PI)
        {
            tmp1 -= 2.0 * PI;
        }
        data.core.image[IDb].array.F[ii] = tmp1;
    }
    copy_image_ID("pp3", "pp3c", 0);
    IDc = image_ID("pp3c", data.core.image, data.core.NB_MAX_IMAGE);
    for(uint64_t ii = 0; ii < size * size; ii++)
    {
        double tmp1 = data.core.image[IDc].array.F[ii];
        tmp1 -= 2.0 * PI / 3.0;
        if(tmp1 < -PI)
        {
            tmp1 += 2.0 * PI;
        }
        data.core.image[IDc].array.F[ii] = tmp1;
    }
    ...
    ID  = image_ID("pp4", data.core.image, data.core.NB_MAX_IMAGE);
    IDb = image_ID("pp4b", data.core.image, data.core.NB_MAX_IMAGE);
    IDc = image_ID("pp4c", data.core.image, data.core.NB_MAX_IMAGE);
    for(uint64_t ii = 0; ii < size * size; ii++)
    {
        double tmp1 = data.core.image[ID].array.F[ii];
        double tmp2 = data.core.image[IDb].array.F[ii];
        double tmp3 = data.core.image[IDc].array.F[ii];

        if((fabs(tmp2 - (2.0 * PI / 3.0 - PI)) < 0.3 * PI) &&
                (fabs(tmp3 + (2.0 * PI / 3.0 - PI)) < 0.3 * PI))
        {
            ...
            data.core.image[ID].array.F[ii] = tmp1;
        }
    }

    delete_image_ID("pp3b", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pp3c", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pa4b", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pp4b", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pa4c", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pp4c", DELETE_IMAGE_ERRMODE_WARNING);

    if(CHEAT == 1)
    {
        printf("------------ MASK SUBTRACTION ----------\n");
        ID1 = image_ID("pa1", data.core.image, data.core.NB_MAX_IMAGE);
        ID2 = image_ID("pp1", data.core.image, data.core.NB_MAX_IMAGE);
        ID3 = image_ID("pa4", data.core.image, data.core.NB_MAX_IMAGE);
        ID4 = image_ID("pp4", data.core.image, data.core.NB_MAX_IMAGE);

        for(uint32_t ii = 10; ii < size - 10; ii++)
            for(uint32_t jj = 10; jj < size - 10; jj++)
            {
                long   index1 = jj * size + ii;
                long   index2 = (size - jj) * size + (size - ii);
                double re1    = data.core.image[ID1].array.F[index2] *
                                cos(data.core.image[ID2].array.F[index2]);
                double im1 = data.core.image[ID1].array.F[index2] *
                             sin(data.core.image[ID2].array.F[index2]);
                double re2 = data.core.image[ID3].array.F[index1] *
                             cos(data.core.image[ID4].array.F[index1]);
                double im2 = data.core.image[ID3].array.F[index1] *
                             sin(data.core.image[ID4].array.F[index1]);

                double re = re1 * size * size - re2;
                double im = im1 * size * size - im2;
                ...
                data.core.image[ID3].array.F[index1] = sqrt(re * re + im * im);
                data.core.image[ID4].array.F[index1] = atan2(im, re);
            }
    }
    ...
    delete_image_ID("pa1", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pa2", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pa3", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pa4", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pp1", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pp2", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pp3", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pp4", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pc3", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pc4", DELETE_IMAGE_ERRMODE_WARNING);
    for(uint64_t ii = 0; ii < size2; ii++)
    {
        data.core.image[ID].array.F[ii] /= total * size2 * size2 * size2;
    }
...
int coronagraph_simul_STRIPC(double xld, double yld, const char *psfname)
{
    ...
    double  trad_pix = CORONAGRAPHS_TDIAM / CORONAGRAPHS_PSCALE / 2.0;
    imageID IDa1 =
        make_disk("pa1", size, size, 0.5 * size, 0.5 * size, trad_pix);
    double total = arith_image_total("pa1");
    ...
    totcnt = 0;
    for(long ii = 0; ii < (long) trad_pix; ii++)
    {
        for(long jj = 0; jj < (long) trad_pix; jj++)
        {
            double rad1 = sqrt(ii * ii + jj * jj);
            double rad2 = sqrt((2.0 * OFFSET - ii) * (2.0 * OFFSET - ii) +
                               (2.0 * OFFSET - jj) * (2.0 * OFFSET - jj));
            double rad3 =
                sqrt((2.0 * OFFSET - ii) * (2.0 * OFFSET - ii) + jj * jj);
            double rad4 =
                sqrt(ii * ii + (2.0 * OFFSET - jj) * (2.0 * OFFSET - jj));
            if((rad1 < (trad_pix)) && (rad2 < (trad_pix)) &&
                    (rad3 < (trad_pix)) && (rad4 < (trad_pix)) &&
                    (ii < 2.0 * OFFSET) && (jj < 2.0 * OFFSET))
            {
                totcnt++;
                long index1 = (size / 2 + jj) * size + (size / 2 + ii);
                long index2 =
                    (size / 2 - 2 * OFFSET + jj) * size + (size / 2 + ii);
                long index3 = (size / 2 - 2 * OFFSET + jj) * size +
                              (size / 2 - 2 * OFFSET + ii);
                long index4 =
                    (size / 2 + jj) * size + (size / 2 - 2 * OFFSET + ii);

                /*		data.core.image[IDpi].array.F[index1] += 1.0;
                data.core.image[IDpi].array.F[index2] += 1.0;
                data.core.image[IDpi].array.F[index3] += 1.0;
                data.core.image[IDpi].array.F[index4] += 1.0;*/

                double a1 = data.core.image[IDa1].array.F[index1];
                double p1 = data.core.image[IDp1].array.F[index1];
                double a2 = data.core.image[IDa1].array.F[index2];
                double p2 = data.core.image[IDp1].array.F[index2];
                //double a3 = data.core.image[IDa1].array.F[index3];
                double p3 = data.core.image[IDp1].array.F[index3];
                //double a4 = data.core.image[IDa1].array.F[index4];
                double p4 = data.core.image[IDp1].array.F[index4];
                double r1 = a1 * cos(p1);
                double i1 = a1 * sin(p1);
                double r2 = a2 * cos(p2);
                double i2 = a2 * sin(p2);
                ...
                double r4 = a2 * cos(p4);
                double i4 = a2 * sin(p4);
                data.core.image[IDc].array.CF[index1].re =
                    0.25 * ((r1 - r4) - (r2 - r3));
                data.core.image[IDc].array.CF[index1].im =
                    0.25 * ((i1 - i4) - (i2 - i3));

                data.core.image[IDc].array.CF[index2].re =
                    0.25 * ((r2 - r3) - (r1 - r4));
                data.core.image[IDc].array.CF[index2].im =
                    0.25 * ((i2 - i3) - (i1 - i4));

                data.core.image[IDc].array.CF[index3].re =
                    0.25 * ((r3 - r2) - (r4 - r1));
                data.core.image[IDc].array.CF[index3].im =
                    0.25 * ((i3 - i2) - (i4 - i1));

                data.core.image[IDc].array.CF[index4].re =
                    0.25 * ((r4 - r1) - (r3 - r2));
                data.core.image[IDc].array.CF[index4].im =
                    0.25 * ((i4 - i1) - (i3 - i2));
            }
        }
    }
...
int coronagraph_simul_SIMXY(double xld, double yld, const char *psfname)
{
    ...
    {
        for(long jj = 0; jj < trad_pix; jj++)
        {
            rad1 = sqrt(ii * ii + jj * jj);
            if(rad1 < trad_pix)
            {
                long index1 = (size / 2 + jj) * size + (size / 2 + ii);
                long index2 = (size / 2 - jj) * size + (size / 2 + ii);
                long index3 = (size / 2 - jj) * size + (size / 2 - ii);
                long index4 = (size / 2 + jj) * size + (size / 2 - ii);
...
int coronagraph_simul_AIC_PIAAC(double xld, double yld, const char *psfname)
{
    ...
    imageID  ID;
    uint64_t size2 = size * size;
    imageID  IDaicfield, IDaicc, IDaicd;
    imageID  IDpiaac;
    ...
    for(uint32_t ii = 0; ii < size; ii++)
        for(uint32_t jj = 0; jj < size; jj++)
        {
            double tmp1 = 1.0 * ii - size / 2;
            double tmp2 = 1.0 * jj - size / 2;
            double r = sqrt(tmp1 * tmp1 + tmp2 * tmp2) * CORONAGRAPHS_PIXSCALE;

            if(r < PIAAAIC_FIELDLIMIT)
            {
                data.core.image[IDaicfield].array.CF[jj * size + ii].re =
                    data.core.image[ID].array.CF[jj * size + ii].re;
                data.core.image[IDaicfield].array.CF[jj * size + ii].im =
                    data.core.image[ID].array.CF[jj * size + ii].im;
    ...
    ID = image_ID("psfPIAAC", data.core.image, data.core.NB_MAX_IMAGE);
    delete_image_ID("pc2", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("fc2", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("fc4", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("fa4", DELETE_IMAGE_ERRMODE_WARNING);
    ...
    for(uint32_t ii = 1; ii < size; ii++)
        ...
        {
            double r11 = data.core.image[IDaicfield].array.CF[jj * size + ii].re;
            double r12 = data.core.image[IDaicfield]
                         .array.CF[(size - jj) * size + (size - ii)]
                         .re;
            double i11 = data.core.image[IDaicfield].array.CF[jj * size + ii].im;
            double i12 = data.core.image[IDaicfield]
                         .array.CF[(size - jj) * size + (size - ii)]
                         .im;
            data.core.image[IDaicd].array.F[jj * size + ii] =
                0.25 * ((r11 - r12) * (r11 - r12) + (i11 - i12) * (i11 - i12));
            data.core.image[IDaicc].array.F[jj * size + ii] =
                0.25 * ((r11 + r12) * (r11 + r12) + (i11 + i12) * (i11 + i12));
            data.core.image[IDaicd].array.F[jj * size + ii] /= total * size2;
            data.core.image[IDaicc].array.F[jj * size + ii] /= total * size2;
        }

    delete_image_ID("fc2_aic", DELETE_IMAGE_ERRMODE_WARNING);

    /* combine the 3 images */
    IDpiaac = image_ID("psfPIAAC", data.core.image, data.core.NB_MAX_IMAGE);
    ...
    for(uint32_t ii = 0; ii < size; ii++)
        ...
        {
            data.core.image[ID].array.F[jj * size + ii] =
                data.core.image[IDpiaac].array.F[(jj + size / 4) * size + ii];
        }

    for(uint32_t ii = 0; ii < size / 2; ii++)
        for(uint32_t jj = size / 2; jj < size; jj++)
        {
            data.core.image[ID].array.F[jj * size + ii] =
                data.core.image[IDaicd]
                .array.F[(jj - size / 4) * size + (ii + size / 4)];
        }

    for(uint32_t ii = size / 2; ii < size; ii++)
        for(uint32_t jj = size / 2; jj < size; jj++)
        {
            data.core.image[ID].array.F[jj * size + ii] =
                data.core.image[IDaicc]
                .array.F[(jj - size / 4) * size + (ii - size / 4)];
        }

    delete_image_ID("psfPIAAC", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("psfAICc", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("psfAICd", DELETE_IMAGE_ERRMODE_WARNING);
...
int coronagraph_RRPM_optimize()
{
    ...
    EXECUTE_SYSTEM_COMMAND("rm %s/rrpmref_fm.ref %s/rrpmref_pm.ref",
                           CORONAGRAPHSDATALOCAL,
                           CORONAGRAPHSDATALOCAL);
    delete_image_ID("rrpmref_fm.ref", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("rrpmref_pm.ref", DELETE_IMAGE_ERRMODE_WARNING);
    coronagraph_simul_RRPM(0.0, 0.0, "psf");
    best_value = arith_image_total("psf");
    delete_image_ID("psf", DELETE_IMAGE_ERRMODE_WARNING);
                                     ...
                                     const char *psfname)
{
    ...
    IDprol_init = image_ID("pinit", data.core.image, data.core.NB_MAX_IMAGE);
    if(IDprol_init == -1)
    {
        sprintf(fname, "%s/APLCapo_init.fits.gz", CORONAGRAPHSDATADIR);
        load_fits(fname, "pinit", 1, &IDprol_init);
    }

    IDprol_ffrac = image_ID("pffrac", data.core.image, data.core.NB_MAX_IMAGE);
    if(IDprol_ffrac == -1)
    {
        sprintf(fname, "%s/APLCapo_ffrac.fits.gz", CORONAGRAPHSDATADIR);
        load_fits(fname, "pffrac", 1, &IDprol_ffrac);
    }

    IDprol_transm = image_ID("ptransm", data.core.image, data.core.NB_MAX_IMAGE);
    if(IDprol_transm == -1)
    {
        sprintf(fname, "%s/APLCapo_transm.fits.gz", CORONAGRAPHSDATADIR);
        load_fits(fname, "ptransm", 1, &IDprol_transm);
    }

    IDprol_peak = image_ID("ppeak", data.core.image, data.core.NB_MAX_IMAGE);
    if(IDprol_peak == -1)
    {
        sprintf(fname, "%s/APLCapo_peak.fits.gz", CORONAGRAPHSDATADIR);
        load_fits(fname, "ppeak", 1, &IDprol_peak);
    }

    IDprol_fitapo_a = image_ID("fitapoa", data.core.image, data.core.NB_MAX_IMAGE);
    if(IDprol_fitapo_a == -1)
    {
        sprintf(fname, "%s/APLCapo_fitapo_a.fits.gz", CORONAGRAPHSDATADIR);
        load_fits(fname, "fitapoa", 1, &IDprol_fitapo_a);
    }

    IDprol_fitapo_b = image_ID("fitapob", data.core.image, data.core.NB_MAX_IMAGE);
    if(IDprol_fitapo_b == -1)
    {
        sprintf(fname, "%s/APLCapo_fitapo_b.fits.gz", CORONAGRAPHSDATADIR);
        load_fits(fname, "fitapob", 1, &IDprol_fitapo_b);
    }

    IDprol_fitapo_c = image_ID("fitapoc", data.core.image, data.core.NB_MAX_IMAGE);
    if(IDprol_fitapo_c == -1)
    {
        sprintf(fname, "%s/APLCapo_fitapo_c.fits.gz", CORONAGRAPHSDATADIR);
        load_fits(fname, "fitapoc", 1, &IDprol_fitapo_c);
    }

    IDprol_fitfit = image_ID("fitfit", data.core.image, data.core.NB_MAX_IMAGE);
    if(IDprol_fitfit == -1)
    {
        sprintf(fname, "%s/APLCapo_fitfit.fits.gz", CORONAGRAPHSDATADIR);
        load_fits(fname, "fitfit", 1, &IDprol_fitfit);
    }

    iisize = data.core.image[IDprol_init].md[0].size[0];
    jjsize = data.core.image[IDprol_init].md[0].size[1];

    iico = (long)((APLC_CentOBS1 - APLCapo_CO_START) / APLCapo_CO_STEP + 0.1);
    jjfpmrad =
        (long)((APLC_FPMASKsize - APLCapo_FPMRAD_START) / APLCapo_FPMRAD_STEP +
               0.1);
    ...
    if(data.core.image[IDprol_init].array.F[jjfpmrad * iisize + iico] > 0.1)
    {
        double tmp4 = data.core.image[IDprol_peak].array.F[jjfpmrad * iisize + iico];
        MASKCAMULT  = -(1.0 - tmp4) / tmp4;
        printf("Focal plane mask CA = %f (peak = %f)\n", MASKCAMULT, tmp4);

        if(fitapoINIT == 0)
        {
            fitapoN    = data.core.image[IDprol_fitapo_a].md[0].size[2];
            fitapo_a   = (double *) malloc(sizeof(double) * fitapoN);
            fitapo_b   = (double *) malloc(sizeof(double) * fitapoN);
            fitapo_c   = (double *) malloc(sizeof(double) * fitapoN);
            fitapo_c1  = (double *) malloc(sizeof(double) * fitapoN);
        ...
        for(n = 0; n < fitapoN; n++)
        {
            fitapo_a[n] =
                data.core.image[IDprol_fitapo_a]
                .array.F[n * iisize * jjsize + jjfpmrad * iisize + iico];
            fitapo_b[n] =
                data.core.image[IDprol_fitapo_b]
                .array.F[n * iisize * jjsize + jjfpmrad * iisize + iico];
            fitapo_c[n] =
        ...
        n = 0;
        printf("f(x) = %15.10f * exp(%15.10f * x**%15.10f)",
               fitapo_a[n],
               fitapo_b[n],
               fitapo_c1[n]);
        for(n = 1; n < fitapoN; n++)
        {
            printf("+ %15.10f * exp(%15.10f * x**%15.10f)",
                   fitapo_a[n],
                   fitapo_b[n],
                   fitapo_c1[n]);
        }
    ...
    /*
        ...
        IDapo = image_ID(fname1, data.core.image, data.core.NB_MAX_IMAGE);
        if(IDapo==-1)
          printf("not found\n");
        else
          printf("found\n");
        ...
        ID = image_ID("apomask", data.core.image, data.core.NB_MAX_IMAGE);
    ...
    if(FPMASKSIZE_ERROR == 0)
    {
        ...
        IDfpm = image_ID(fname1, data.core.image, data.core.NB_MAX_IMAGE);
        ...
        if(IDfpm == -1)  // if focal plane mask not already in memory, create it
        {
            ...
            IDfpm = make_subpixdisk(fname1,
                                    size,
                                    size,
                                    size / 2,
                                    size / 2,
                                    DFTZFACTOR * APLC_FPMASKsize /
                                    CORONAGRAPHS_PIXSCALE);

            for(uint64_t ii = 0; ii < size2; ii++)
            {
                data.core.image[IDfpm].array.F[ii] =
                    1.0 - (1.0 - MASKCAMULT * (1.0 + FPMASK_transm_error)) *
                    data.core.image[IDfpm].array.F[ii];
            }
    ...
    else
    {
        ...
        if(IDfpm == -1)
        {
            FPMZONES = 1;
            printf(
                "-------------- FPMASKSIZE_ERROR = 1, FPMASK_FACTOR = %f, "
                "FPMZONES = %d ----------- (%f %f)\n",
                FPMASK_FACTOR,
                FPMZONES,
                FPMASK_transm_error,
            ...
            {
                IDfpm = make_subpixdisk(
                            "aplcfpm",
                            size,
                            size,
                            size / 2,
                            size / 2,
                            DFTZFACTOR * APLC_FPMASKsize * FPMASK_FACTOR /
                            CORONAGRAPHS_PIXSCALE * (1.0 + FPMASK_size_error));
                //    for(ii=0;ii<size2;ii++)
                //data.core.image[IDfpm].array.F[ii] = 1.0-(1.0-MASKCAMULT*(1.0+FPMASK_transm_error))*data.core.image[IDfpm].array.F[ii];

                save_fl_fits("aplcfpm", "fpm.test.fits");
                //	      sprintf(fname, "%s/APLCfpmask_%f.%ld.ref", CORONAGRAPHSDATALOCAL, APLC_FPMASKsize, size);
                //	      save_fl_fits("aplcfpm", fname);
                IDfpm = image_ID("aplcfpm", data.core.image, data.core.NB_MAX_IMAGE);
            }
            else // 2 zones mask
            {
                ...
                printf("--------------- 2 zones mask  -------------\n");
                IDfpm =
                    make_subpixdisk("aplcfpm",
                                    size,
                                    size,
                                    size / 2,
                                    size / 2,
                                    DFTZFACTOR * APLC_FPMASKsize *
                                    FPMASK_FACTOR1 / CORONAGRAPHS_PIXSCALE);
                ID2 =
                    make_subpixdisk("aplcfpm2",
                                    size,
                                    size,
                                    size / 2,
                                    size / 2,
                                    DFTZFACTOR * APLC_FPMASKsize *
                                    FPMASK_FACTOR2 / CORONAGRAPHS_PIXSCALE);
                for(uint64_t ii = 0; ii < size2; ii++)
                {
                    v1 = data.core.image[IDfpm].array.F[ii];
                    v2 = data.core.image[ID2].array.F[ii];
                    data.core.image[IDfpm].array.F[ii] =
                        1.0 - v1 * (1.0 - FPM_TRANSM1) -
                        (v2 - v1) * (1.0 - FPM_TRANSM2);
                }
                delete_image_ID("aplcfpm2", DELETE_IMAGE_ERRMODE_WARNING);
    ...
    for(uint64_t ii = 0; ii < size2; ii++)
    {
        total0 += data.core.image[IDa1].array.F[ii] * data.core.image[IDa1].array.F[ii];
    }
    ...
    if(APLC_PIAA == 0)
    {
        ...
        else // default apodization
        {
            if(IDapo != -1)
                for(uint64_t ii = 0; ii < size2; ii++)
                {
                    data.core.image[IDa1].array.F[ii] *=
                        data.core.image[IDapo].array.F[ii];
                }
        }
        ...
        if(ID != -1)
        {
            for(long ii = 3; ii < size - 3; ii++)
                for(long jj = 3; jj < size - 3; jj++)
                {
                    int ok1 = 1;
                    kx      = 0;
                    ky      = 0;
                    if(data.core.image[ID].array.F[(jj + ky) * size + ii + kx] <
                            0.001)
                    {
                        ok1 = 0;
                    }
                    if(ok1 == 0)
                    {
                        data.core.image[IDpupa0].array.F[jj * size + ii] = 0.0;
                    }
                }
        }
    }
    ...
    {
        PIAAAPO_NBPOINTS =
            (long)(CORONAGRAPHS_TDIAM / CORONAGRAPHS_PSCALE / 2.0);
        if(initPIAA == 0)
        {
        ...
        // AUTO DEFINE LYOT MASK
        ID1 = image_ID("pa1a", data.core.image, data.core.NB_MAX_IMAGE);
        if(WriteFiles == 1)
        {
        ...
        if(ID!=-1)
        {
          for(ii=3;ii<size-3;ii++)
        	...
        	kx = 0;
        	ky = 0;
        	if(data.core.image[ID].array.F[(jj+ky)*size+ii+kx]<0.001)
        	  ok1 = 0;
        	if(ok1==0)
        	  data.core.image[IDpupa0].array.F[jj*size+ii] = 0.0;
              }
        ...
        ID1 = image_ID("pa1a", data.core.image, data.core.NB_MAX_IMAGE);
        ID  = image_ID("postPIAAmask", data.core.image, data.core.NB_MAX_IMAGE);
        if(ID != -1)
        {
            printf("Applying postPIAA amplitude mask\n");
            for(uint64_t ii = 0; ii < size2; ii++)
            {
                data.core.image[ID1].array.F[ii] *= data.core.image[ID].array.F[ii];
            }
            if(WriteFiles == 1)
            {
        ...
        delete_image_ID("pa1a", DELETE_IMAGE_ERRMODE_WARNING);
        delete_image_ID("pp1a", DELETE_IMAGE_ERRMODE_WARNING);
    }
    ...
    for(iter = 0; iter < NB_APLC_STEP; iter++)
    {
        ...
        if(useDFT == 0)  // regular FFT propagation
        {
            ...
            delete_image_ID("pc1", DELETE_IMAGE_ERRMODE_WARNING);
            permut("fc1");
            ID   = image_ID("fc1", data.core.image, data.core.NB_MAX_IMAGE);
            tot1 = 0.0;
            tot2 = 0.0;
            for(uint64_t ii = 0; ii < size2; ii++)
            {
                double re = data.core.image[ID].array.CF[ii].re;
                double im = data.core.image[ID].array.CF[ii].im;
                tot1 += re * re + im * im;
                data.core.image[ID].array.CF[ii].re *= data.core.image[IDfpm].array.F[ii];
                data.core.image[ID].array.CF[ii].im *= data.core.image[IDfpm].array.F[ii];
                re = data.core.image[ID].array.CF[ii].re;
                im = data.core.image[ID].array.CF[ii].im;
                tot2 += re * re + im * im;
            }
            ...
            delete_image_ID("fc1_a", DELETE_IMAGE_ERRMODE_WARNING);
            delete_image_ID("fc1_p", DELETE_IMAGE_ERRMODE_WARNING);
            ...
            if(WriteFiles == 1)
            {
                ...
                delete_image_ID("pc1a", DELETE_IMAGE_ERRMODE_WARNING);
                delete_image_ID("pc1p", DELETE_IMAGE_ERRMODE_WARNING);
            }

            ID = image_ID("pc1", data.core.image, data.core.NB_MAX_IMAGE);

            // APPLY LYOT MASK
            IDlm = image_ID("LyotMask", data.core.image, data.core.NB_MAX_IMAGE);
            for(uint32_t ii = 0; ii < size; ii++)
                for(uint32_t jj = 0; jj < size; jj++)
                {
                    if((ii < 5) || (jj < 5) || (ii > size - 5) ||
                            (jj > size - 5))
                    {
                        data.core.image[ID].array.CF[jj * size + ii].re = 0.0;
                        data.core.image[ID].array.CF[jj * size + ii].im = 0.0;
                    ...
                    else
                    {
                        ...
                        data.core.image[ID].array.CF[jj * size + ii].im *=
                            ...
                            .array.F[(size - jj) * size + (size - ii)] /
                            size2;
                        //			data.core.image[ID].array.CF[jj*size+ii].re /= size2;
                        //			data.core.image[ID].array.CF[jj*size+ii].im /= size2;
                    }
                }
            if(WriteFiles == 1)
            {
                ...
                delete_image_ID("pc1am", DELETE_IMAGE_ERRMODE_WARNING);
                delete_image_ID("pc1pm", DELETE_IMAGE_ERRMODE_WARNING);
            }
        }
        else // DFT propagation
        {
            ...
            ID = image_ID("aplcfpm", data.core.image, data.core.NB_MAX_IMAGE);
            create_2Dimage_ID("aplcfpm1", size, size, &ID1);
            for(uint64_t ii = 0; ii < size2; ii++)
            {
                data.core.image[ID1].array.F[ii] = 1.0 - data.core.image[ID].array.F[ii];
            }
            if(WriteFiles == 1)
            {
            ...
            IDfpm_a = image_ID("aplcfpm_a", data.core.image, data.core.NB_MAX_IMAGE);
            IDfpm_p = image_ID("aplcfpm_p", data.core.image, data.core.NB_MAX_IMAGE);
            if((IDfpm_a != -1) && (IDfpm_p != -1))
            {
                printf(
                    "============= USING CUSTOM COMPLEX FOCAL PLANE MASK "
                    "============\n");
                mk_complex_from_amph("aplcfpm_a", "aplcfpm_p", "aplcfpm_c", 0);
                ID = image_ID("aplcfpm_c", data.core.image, data.core.NB_MAX_IMAGE);
                for(uint64_t ii = 0; ii < size2; ii++)
                {
                    data.core.image[ID].array.CF[ii].re =
                        1.0 - data.core.image[ID].array.CF[ii].re;
            ...
            if(0)  // testing
            {
                ...
                delete_image_ID("dftia", DELETE_IMAGE_ERRMODE_WARNING);
                delete_image_ID("dftip", DELETE_IMAGE_ERRMODE_WARNING);
                mk_amph_from_complex("pc2", "dftoa", "dftop", 0);
                save_fl_fits("dftoa", "dftoa.fits");
                save_fl_fits("dftop", "dftop.fits");
                delete_image_ID("dftoa", DELETE_IMAGE_ERRMODE_WARNING);
                delete_image_ID("dftop", DELETE_IMAGE_ERRMODE_WARNING);
                //mk_amph_from_complex("aplcfpm1", "dftfpma", "dftfpmp", 0);
                save_fl_fits("aplcfpm1", "dftfpma.fits");
                //	      save_fl_fits("dftfpmp", "dftfpmp.fits");
                // delete_image_ID("dftfpma", DELETE_IMAGE_ERRMODE_WARNING);
                //delete_image_ID("dftfpmp", DELETE_IMAGE_ERRMODE_WARNING);
            }
            delete_image_ID("aplcfpm1", DELETE_IMAGE_ERRMODE_WARNING);
            //	  exit(0);

            ID1 = image_ID("pc1", data.core.image, data.core.NB_MAX_IMAGE);
            ID2 = image_ID("pc2", data.core.image, data.core.NB_MAX_IMAGE);
            for(uint64_t ii = 0; ii < size2; ii++)
            {
                double re1 = data.core.image[ID1].array.CF[ii].re;
                double im1 = data.core.image[ID1].array.CF[ii].im;

                double re2 = data.core.image[ID2].array.CF[ii].re;
                double im2 = data.core.image[ID2].array.CF[ii].im;

                re1 = re1 - (1.0 - 0.0 * MASKCAMULT) * re2;
                im1 = im1 - (1.0 - 0.0 * MASKCAMULT) * im2;

                data.core.image[ID1].array.CF[ii].re = re1;
                data.core.image[ID1].array.CF[ii].im = im1;
            }
            delete_image_ID("pc2", DELETE_IMAGE_ERRMODE_WARNING);
            if(WriteFiles == 1)
            {
    ...
    if(APLC_PIAA == 2)  // INVERSE PIAA
    {
        ...
        if(0)  // testing
        {
            ...
            coronagraphs_PIAA_apodize_beam("postPIAAmask",
                                           "postPIAAmask",
                                           "prePIAAmask1",
                                           "prePIAAmask");
        ...
        if(CHEAT == 1)
        {
            ...
            for(long ii = 10; ii < size - 10; ii++)
                for(long jj = 10; jj < size - 10; jj++)
                {
                    long   index1 = jj * size + ii;
                    long   index2 = (size - jj) * size + (size - ii);
                    double re1    = data.core.image[ID1].array.F[index2] *
        ...
        else
        {
            ...
            for(uint32_t ii = 0; ii < size; ii++)
                ...
                {
                    long   index1 = jj * size + ii;
                    double re     = data.core.image[ID3].array.F[index1] *
                                    cos(data.core.image[ID4].array.F[index1]);
                    double im = data.core.image[ID3].array.F[index1] *
                                sin(data.core.image[ID4].array.F[index1]);
                    double tmp1 = 1.0 * ii - size / 2;
                    double tmp2 = 1.0 * jj - size / 2;
                    double r    = sqrt(tmp1 * tmp1 + tmp2 * tmp2);
                    if((r > PIAAPPMASKRAD * trad_pix) ||
                            (r < APLC_CentOBS0 * trad_pix +
                             3.0)) // 3 pixel buffer to avoid edge effects
                    {
                        re = 0.0;
                        im = 0.0;
                    }
        ...
        {
            printf("-------- FLIP --------\n");
            ID = image_ID("fc4", data.core.image, data.core.NB_MAX_IMAGE);
            for(uint32_t ii = 1; ii < size; ii++)
                for(uint32_t jj = 1; jj < size / 2 + 1; jj++)
                {
                    uint32_t ii1 = ii;
                    uint32_t jj1 = jj;
                    long     ii2 = size - ii;
                    long     jj2 = size - jj;
                    double   re1 = data.core.image[ID].array.CF[jj1 * size + ii1].re;
                    double   im1 = data.core.image[ID].array.CF[jj1 * size + ii1].im;
                    double   re2 = data.core.image[ID].array.CF[jj2 * size + ii2].re;
                    double   im2 = data.core.image[ID].array.CF[jj2 * size + ii2].im;
                    double   re  = 0.5 * (re1 + re2);
                    double   im  = 0.5 * (im1 + im2);
                    data.core.image[ID].array.CF[jj1 * size + ii1].re = re;
                    data.core.image[ID].array.CF[jj1 * size + ii1].im = im;
                    data.core.image[ID].array.CF[jj2 * size + ii2].re = re;
                    data.core.image[ID].array.CF[jj2 * size + ii2].im = im;
                }
        }
        mk_amph_from_complex("fc4", "fa4", "fp4", 0);
        ...
        for(uint64_t ii = 0; ii < size2; ii++)
        {
            data.core.image[ID].array.F[ii] /= size2 * total0;
        }
    ...
    else
    {
        ...
        ID = image_ID("fa2", data.core.image, data.core.NB_MAX_IMAGE);
        delete_image_ID("fp2", DELETE_IMAGE_ERRMODE_WARNING);
        create_2Dimage_ID(psfname, size, size, &IDout);
        for(uint64_t ii = 0; ii < size2; ii++)
        {
            data.core.image[IDout].array.F[ii] = data.core.image[ID].array.F[ii] / size2 /
                                            total0 * data.core.image[ID].array.F[ii];
        }
                                       ...
                                       long   FACTOR)
{
    ...
    make_offsetHyperGaussian(size * FACTOR,
    ...
    ID = image_ID("extoccp", data.core.image, data.core.NB_MAX_IMAGE);

    delete_image_ID("zero", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("extocc", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("extocc_c", DELETE_IMAGE_ERRMODE_WARNING);
                                        ...
                                        const char *psfname)
{
    ...
    IDocc = image_ID("extoccp", data.core.image, data.core.NB_MAX_IMAGE);
    if(IDocc == -1)
    {
    ...
    IDocca = image_ID("exta", data.core.image, data.core.NB_MAX_IMAGE);
    IDoccp = image_ID("extp", data.core.image, data.core.NB_MAX_IMAGE);
    ...
    for(ii = 0; ii < size; ii++)
        for(jj = 0; jj < size; jj++)
        {
            ...
            if((ii1 > 0) && (ii1 < size * FACTOR) && (jj1 > 0) &&
                    (jj1 < size * FACTOR))
            {
                data.core.image[IDa1].array.F[jj * size + ii] =
                    data.core.image[IDocca].array.F[jj1 * size * FACTOR + ii1];
                data.core.image[IDp1].array.F[jj * size + ii] +=
                    data.core.image[IDoccp].array.F[jj1 * size * FACTOR + ii1];
            }
            else
            {
                data.core.image[IDa1].array.F[jj * size + ii] = 1.0;
            }
    ...
    delete_image_ID("exta", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("extp", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pa1", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pp1", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("pc1", DELETE_IMAGE_ERRMODE_WARNING);
    delete_image_ID("fc2", DELETE_IMAGE_ERRMODE_WARNING);
    ...
    */
    ...
    switch(coronagraph_type)
    {
        ...
        case 50: // generalized PIAA APLC, read parameters from file
            ...
            {
                printf(
                    "No focal plane mask error file - using perfect focal plane "
                    "mask\n");
                       ...
                       const char *options)
{
    ...
    if((AUTOPIAACMASK == 1) && (coronagraph_type == 5))
    {
        ...
        if((FOCerror_included == 1) || (TTerror_included == 1))
        {
                ...
                for(i = -nbTTpts; i < nbTTpts; i++)
                    for(j = -nbTTpts; j < nbTTpts; j++)
                    {
                        ...
                        if(r < TTerror)
                        {
                            ...
                            ID1 = image_ID("psf1", data.core.image, data.core.NB_MAX_IMAGE);
                            for(ii = 0; ii < size2; ii++)
                            {
                                data.core.image[ID0].array.F[ii] +=
                                    data.core.image[ID1].array.F[ii];
            ...
            else
            {
                ...
                for(i = -nbFOCpts; i < nbFOCpts; i++)
                {
                    ...
                    delete_image_ID("psf1", DELETE_IMAGE_ERRMODE_WARNING);
                    printf(".");
                    fflush(stdout);
                    cnt++;
                }
                delete_image_ID("corphase", DELETE_IMAGE_ERRMODE_WARNING);
                printf("\n");
            }
            for(ii = 0; ii < size2; ii++)
            {
                data.core.image[ID0].array.F[ii] /= cnt;
            }
        }
        else
        {
            coronagraph_simulPSF(0.0, 0.0, "psf0", 4, "");
            ID0 = image_ID("psf0", data.core.image, data.core.NB_MAX_IMAGE);
        }
        peak = data.core.image[ID0].array.F[(size / 2) * size + size / 2];
        ii   = 0;
        printf("%g %g\n",
               data.core.image[ID0].array.F[(size / 2) * size + ii],
               peak);
        while(data.core.image[ID0].array.F[(size / 2) * size + ii] <
                2.0 * peak / pow(10.0, logcontrast))
        {
            ii++;
        }
        ...
        printf("PIAAC:  MASK RADIUS = %f lambda/d  (ii = %ld)\n",
               PIAAFPMASKRAD,
               ii);
        delete_image_ID("psf0", DELETE_IMAGE_ERRMODE_WARNING);
    }
    ...
    if((FOCerror_included == 1) || (TTerror_included == 1))
    {
        ...
        else
        {
            ...
            for(i = -nbFOCpts; i < nbFOCpts; i++)
            {
                ...
                for(ii = 0; ii < size2; ii++)
                {
                    data.core.image[ID0].array.F[ii] += data.core.image[ID1].array.F[ii];
                }
                delete_image_ID("psf1", DELETE_IMAGE_ERRMODE_WARNING);
                printf(".");
                fflush(stdout);
    ...
    else
    {
        if(Zerror_included == 1)
        {
            ...
            for(ii = 0; ii < size * size; ii++)
            {
                data.core.image[ID].array.F[ii] *= Zerror;
            }
    ...
    if(RADAVER == 0)
    {
        ...
        while(sep < sepmax)
        {
            if((coronagraph_type == 3) || (coronagraph_type == 17) ||
                    (coronagraph_type == 19) || (coronagraph_type == 20) ||
                    (coronagraph_type == 22) || (coronagraph_type == 23) ||
                    (coronagraph_type == 30))
            {
                if(coronagraph_type == 30)
                {
                    coronagraph_simulPSF(-sep / sqrt(2.0),
                                         -sep / sqrt(2.0),
                                         "psf",
                                         coronagraph_type,
                                         "");
            ...
            ID = image_ID("psf", data.core.image, data.core.NB_MAX_IMAGE);
            ...
            for(ii = 0; ii < size; ii++)
                for(jj = 0; jj < size; jj++)
                {
                    flux0 = data.core.image[ID0].array.F[jj * size + ii] *
                            pow(10.0, logcontrast);
                    flux = data.core.image[ID].array.F[jj * size + ii];
                    if(flux > peak)
                    {
                        peak = flux;
                    }
                    if(flux > flux0)
                    {
                        total += data.core.image[ID].array.F[jj * size + ii];
                    }
                }
            kk = 0;
            for(ii = 0; ii < size; ii++)
                for(jj = 0; jj < size; jj++)
                {
                    ...
                    {
                        arrayS1[kk] = data.core.image[ID0].array.F[jj * size + ii] *
                                      pow(10.0, logcontrast);
                        arrayS2[kk] = data.core.image[ID].array.F[jj * size + ii];
            ...
            {
                iwa = sep -
                      (sep - sepold) * (total2 - 0.5) / (total2 - total2old);
                iwaOK = 1;
                fp    = fopen(fname, "a");
                fprintf(fp,
                        "#IWA50 %f %f %f %f\n",
    ...
    else
    {
        while(sep < sepmax)
        {
            ...
            for(anglestep = 0; anglestep < NBanglestep; anglestep++)
            {
                ...
                coronagraph_simulPSF(sep * cos(angle),
                                     sep * sin(angle),
                                     "psf",
                                     coronagraph_type,
                                     "");
                ...
                for(ii = 0; ii < size; ii++)
                    for(jj = 0; jj < size; jj++)
                    {
                        ...
                        {
                            arrayS1[kk] =
                                data.core.image[ID0].array.F[jj * size + ii] *
                                pow(10.0, logcontrast);
                            arrayS2[kk] =
                                data.core.image[ID].array.F[jj * size + ii];
                            arrayS[kk] = arrayS1[kk] / arrayS2[kk];
...
int coronagraph_userfunc()
{
    ...
    trad_pix = CORONAGRAPHS_TDIAM / CORONAGRAPHS_PSCALE / 2.0;
    ID =
        make_subpixdisk("coramp", size, size, 0.5 * size, 0.5 * size, trad_pix);
    for(ii = 0; ii < size; ii++)
        for(jj = 0; jj < size; jj++)
        {
            x = 1.0 * (ii - size / 2);
            //y = 1.0 * (jj - size / 2);
            //r = sqrt(x * x + y * y);
            data.core.image[ID].array.F[jj * size + ii] *= fabs(x) / trad_pix;
        }
    save_fl_fits("coramp", "coramp");

    ID =
        make_subpixdisk("corpha", size, size, 0.5 * size, 0.5 * size, trad_pix);
    for(ii = 0; ii < size; ii++)
        for(jj = 0; jj < size; jj++)
        {
            x = 1.0 * (ii - size / 2);
            ...
            {
                data.core.image[ID].array.F[jj * size + ii] = PI;
            }
        }
    ...
    while(0)
    {
        ...
        delete_image_ID("aplcfpm", DELETE_IMAGE_ERRMODE_WARNING);
        v1 = COROTMP1;
...
int CORONAGRAPHS_scanPIAACMC_centObs_perf(double obs0input)
{
    ...
    for(fpmaskrad_index = 0; fpmaskrad_index < fpmaskradarraysize;
            fpmaskrad_index++)
    {
        fpmaskradarray[fpmaskrad_index] =
            fpmaskradmin + fpmaskradstep * fpmaskrad_index;
    }
    ...
    if((fp = fopen("obs1indexstart.txt", "r")) != NULL)
    {
        //ret = fscanf(fp, "%ld", &OBS1INDEXSTART);
        {
            ...
            fscanfcnt = fscanf(fp, "%ld", &OBS1INDEXSTART);
            if(fscanfcnt == EOF)
            {
                if(ferror(fp))
                {
    ...
    if(0)  // rename
    {
        for(fpmaskrad_index = 0; fpmaskrad_index < fpmaskradarraysize;
                fpmaskrad_index++)
            for(obs1_index = OBS1INDEXSTART; obs1_index < obs1arraysize;
                    obs1_index++)
            {
                ...
                for(k = 0; k < kmax; k++)
                {
                    printf(
                        ...
                        "=======================\n");
                    EXECUTE_SYSTEM_COMMAND(
                        "mv ./PSF/PSF_%4.2f_%5.3f_%5.3f_%5.3f.fits  "
                        "./PSF/PSF_%5.3f_%5.3f_%5.3f_%5.3f.fits\n",
                        fpmaskrad,
                        obs0,
                        obs1,
                        separray[k],
                    ...
                    EXECUTE_SYSTEM_COMMAND(
                        "mv ./PSF/PSFc_%4.2f_%5.3f_%5.3f_%5.3f.fits  "
                        "./PSF/PSFc_%5.3f_%5.3f_%5.3f_%5.3f.fits\n",
                        fpmaskrad,
                        obs0,
                        obs1,
                        separray[k],
                ...
                EXECUTE_SYSTEM_COMMAND(
                    "mv ./PROF/star_%4.2f_%5.3f_%5.3f.prof "
                    "./PROF/star_%5.3f_%5.3f_%5.3f.prof\n",
                    fpmaskrad,
                    obs0,
                    obs1,
                    fpmaskrad,
                ...
                //	    EXECUTE_SYSTEM_COMMAND("mv ./TRANSM/transm_%4.2f_%5.3f_%5.3f.prof ./TRANSM/transm_%5.3f_%5.3f_%5.3f.prof\n",  fpmaskrad, obs0, obs1, fpmaskrad, obs0, obs1);
            }
    ...
    if(COMPUTEDATA == 1)
    {
                ...
                fpmaskrad_index++)
        {
                    ...
                    obs1_index++)
            {
                ...
                if((fpmaskrad > 0.8) && (fpmaskrad < 2.0) && (obs1 > 0.0) &&
                        (obs1 < 0.4)) // put limits here if required
                {
                    // write configuration file
                    fpconf = fopen("APLCPIAA.conf", "w");
                    fprintf(fpconf,
                            "1 %5.3f-%5.3f-%5.3f 2 1\n",
                            fpmaskrad,
                            obs0,
                            obs1);
                    ...
                    {
                        sprintf(fnameprol,
                                "%s/APLCapo/raw/APLCapo_%5.3f.4096.ref.gz",
                                CORONAGRAPHSDATALOCAL,
                                fpmaskrad);
                    }
                    else
                    {
                        sprintf(
                            fnameprol,
                            "%s/APLCapo/raw/APLCapo_%5.3f.%5.3f.4096.ref.gz",
                            CORONAGRAPHSDATALOCAL,
                            fpmaskrad,
                            obs1);
                    }
                    ...
                    sprintf(fnamenew,
                            "%s/APLCapo/APLCapo_%5.3f.%5.3f.4096.ref.new1",
                            CORONAGRAPHSDATALOCAL,
                            fpmaskrad,
                            obs1);

                    if(stat(fnameprol, &buffer) ==
                            0) // if the prolate file exists
                    {
                        ...
                        for(k = 0; k < kmax; k++)
                        {
                            ...
                            {
                                EXECUTE_SYSTEM_COMMAND("rm STOP");
                                exit(0);
                            }
                            sprintf(fname,
                                    "./PSF/PSF_%5.3f_%5.3f_%5.3f_%5.3f.fits",
                            ...
                            if(stat(fname, &buffer) !=
                                    0) // if PSF file does not exist, create it
                            {
                                if(skipPSFcomp == 1)
                                {
                                    psfsok = 0;
                                ...
                                else
                                {
                                    ...
                                    EXECUTE_SYSTEM_COMMAND("./mkpsf %f %f",
                                                           separray[k],
                                                           0.0);
                                    EXECUTE_SYSTEM_COMMAND("mv psf.fits %s",
                                                           fname);
                                }
                            }
                        ...
                        if(psfsok == 1)
                        {
                            ...
                            k = 2; // 0.01 l/D
                            sprintf(fname1, "psf%02ld", k);
                            ID = image_ID(fname1, data.core.image, data.core.NB_MAX_IMAGE);
                            for(ii = 0; ii < size; ii++)
                                for(jj = 0; jj < size; jj++)
                                {
                                    x = 1.0 * ii - size / 2;
                            ...
                            {
                                starprofarray[i] /=
                                    (starprofarraycnt[i] + 0.000001);
                                starprofarray[i] *=
                                    0.5 *
                                    pow((star_radius / separray[k]), 2.0) /
                                    peak;
                                fprintf(fp,
                                        "%5.3f %g\n",
                                        starprof_step * i,
                            ...
                            sprintf(transmname,
                                    "./TRANSM/transm_%5.3f_%5.3f_%5.3f.prof",
                                    fpmaskrad,
                                    obs0,
                                    obs1);
                            if(stat(transmname, &buffer) !=
                                    0) // if does not exist
                            {
                                tmax = 0.0;
                                for(k = 0; k < kmax; k++)
                                {
                                    sprintf(fname1, "psf%02ld", k);
                                    transmarray[k] = arith_image_total(fname1) /
                                                     (1.0 - obs0 * obs0);
    ...
    create_3Dimage_ID("PIAACMCtransm",
                      obs1arraysize,
                      fpmaskradarraysize,
                      kmax,
                      &IDtransm);
    ...
    create_2Dimage_ID("PIAACMCcontrastmax",
                      obs1arraysize,
                      fpmaskradarraysize,
                      &IDcontrastmax);

    create_2Dimage_ID("PIAACMCfpmasktransm",
                      obs1arraysize,
                      fpmaskradarraysize,
                      &IDfpmasktransm);
    ...
    for(obs1_index = 0; obs1_index < obs1arraysize; obs1_index++)
    {
                ...
                fpmaskrad_index++)
        {
            ...
            if(stat(fnameprolinfo, &buffer) == 0)  // if file exists
            {
                ...
                fclose(fp);
                fpmasktransm = -(1.0 - tmpf3) / tmpf3;
                data.core.image[IDfpmasktransm]
                .array.F[fpmaskrad_index * obs1arraysize + obs1_index] =
                    fpmasktransm;
            }
            ...
            if(stat(profname, &buffer) == 0)  // if file exists
            {
                ...
                for(i = 0; i < starprofNBstep; i++)
                {
                    ...
                    //		  printf(" -- %5.3f %g\n", tmpf0, tmpf1 );
                    if((tmpf0 > eval_contrast_start) &&
                            (tmpf0 < eval_contrast_end))
                        if(tmpf1 > maxval)
                        {
                            maxval = tmpf1;
                        }
            ...
            data.core.image[IDcontrastmax]
            .array.F[fpmaskrad_index * obs1arraysize + obs1_index] = maxval;
            ...
            if(stat(transmname, &buffer) == 0)  // if file exists
            {
                ...
                for(k = 0; k < kmax; k++)
                {
                    ...
                    data.core.image[IDtransm]
                    .array.F[k * fpmaskradarraysize * obs1arraysize +
                               fpmaskrad_index * obs1arraysize + obs1_index] =
                                 tmpf1;
                    transmarray[k] = tmpf1;
                }
                fclose(fp);
                ...
                k--;
                alpha = (0.5 - transmarray[k - 1]) /
                        (transmarray[k] - transmarray[k - 1]);
                iwa = separray[k - 1] + alpha * (separray[k] - separray[k - 1]);
                data.core.image[IDiwa]
                .array.F[fpmaskrad_index * obs1arraysize + obs1_index] =
                    iwa;
