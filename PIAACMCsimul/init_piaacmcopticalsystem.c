...
errno_t init_piaacmcopticalsystem(double TTxld, double TTyld)
{
    ...
    piaacmcopticalsystem.DFTgridpad =
        0; // 0 for full DFT sampling, >0 for faster execution
    ...
    */
    ...
    if(piaacmcopticaldesign.PIAAmode == 1)
    {
        ...
        piaacmcopticalsystem.elemarrayindex[elem] =
    ...
    if(piaacmcopticaldesign.PIAAmode == 1)
    {
        ...
        snprintf(piaacmcopticalsystem.name[elem],
                 STRINGMAXLEN_OPTSYST_ELEMNAME,
                 "opaque mask at PIAA elem 1");
        piaacmcopticalsystem.elemtype[elem] = 1; // opaque mask
        {
            imageID ID;
            load_fits("piaa1mask.fits",
                      "piaa1mask",
                      LOADFITS_ERRMODE_IGNORE,
                      &ID);
            if(ID == -1)
        ...
        piaacmcopticalsystem.elemZpos[elem] =
            piaacmcopticalsystem.elemZpos[elem - 1];
    ...
    if((variable_ID("PIAACMC_NOFPM")) == -1)
    {
        ...
        piaacmcopticalsystem.FOCMASKarray[0].zfactor =
