 ...
 *
 */
errno_t init_piaacmcopticalsystem(double TTxld, double TTyld)
{
    DEBUG_TRACE_FSTART();
    ...
    for (int k = 0; k < piaacmcopticalsystem.nblambda; k++)
    {
        piaacmcopticalsystem.lambdaarray[k] =
            piaacmcparams.LAMBDASTART +
            (0.5 + k) * (piaacmcparams.LAMBDAEND - piaacmcparams.LAMBDASTART) / piaacmcopticalsystem.nblambda;
    }
    ...
    { // write wavelength list
        ...
        WRITE_FULLFILENAME(fname, "%s/lambdalist.txt", piaacmcparams.piaacmcconfdir);
    ...
    */
    ...
    WRITE_FULLFILENAME(fname_pupa0, "%s/pupa0_%ld.fits", piaacmcparams.piaacmcconfdir, size);
    ...
    if (IDa == -1) // if pupil does not exist, use circular one (this occurs in initial design steps)
    {
        ...
        FUNC_CHECK_RETURN(save_fl_fits("pupa0", fname_pupa0));
    }
    ...
    // typically not present for PIAACMC
    {
        ...
        if (ID == -1)
        {
            FUNC_CHECK_RETURN(load_fits("prePIAA0mask.fits", "prePIAA0mask", LOADFITS_ERRMODE_WARNING, &ID));
        }
        if (ID != -1)
        {
    ...
    if (piaacmcopticaldesign.PIAAmode == 1)
    {
        ...
        else // lens
        {
            ...
            piaacmcopticalsystem.ASPHSURFRarray[piaacmcopticalsystem.elemarrayindex[elem]].surfID =
                image_ID("piaar0zsag");
            ...
            piaacmcopticalsystem.ASPHSURFRarray[piaacmcopticalsystem.elemarrayindex[elem]].mat1 =
    ...
    // we don't have one in the nominal design
    {
        ...
        if (ID != -1)
        {
            ...
            snprintf(piaacmcopticalsystem.name[elem], STRINGMAXLEN_OPTSYST_ELEMNAME,
                     "opaque mask after PIAA element 0");
            piaacmcopticalsystem.elemtype[elem] = 1; // opaque mask
            piaacmcopticalsystem.elemarrayindex[elem] = ID;
            piaacmcopticalsystem.elemZpos[elem] =
    ...
    if (piaacmcopticaldesign.PIAAmode == 1)
    {
        ...
        piaacmcopticalsystem.elemtype[elem] = 1; // opaque mask
        {
            ...
            {
                ID =
                    make_disk("piaa1mask", size, size, 0.5 * size, 0.5 * size, piaacmcopticaldesign.r1lim * beamradpix);
            }
            piaacmcopticalsystem.elemarrayindex[elem] = ID;
        }
    ...
    if ((variable_ID("PIAACMC_NOFPM")) == -1)
    {
        ...
        FUNC_CHECK_RETURN(mkFocalPlaneMask("fpmzmap", "piaacmcfpm", piaacmcparams.focmMode, savefpm,
        ...
        FUNC_CHECK_RETURN(save_fits("piaacmcfpm_re", "./testdir/test_piaacmcfpm_re.fits"));

        FUNC_CHECK_RETURN(save_fits("piaacmcfpm_im", "./testdir/test_piaacmcfpm_im.fits"));

        FUNC_CHECK_RETURN(delete_image_ID("piaacmcfpm_re", DELETE_IMAGE_ERRMODE_WARNING));

        FUNC_CHECK_RETURN(delete_image_ID("piaacmcfpm_im", DELETE_IMAGE_ERRMODE_WARNING));
        ...
        piaacmcopticalsystem.elemZpos[elem] = piaacmcopticalsystem.elemZpos[elem - 1]; // plane from which FT is done
    ...
    if (piaacmcopticaldesign.PIAAmode == 1)
    {
        ...
        piaacmcopticalsystem.elemtype[elem] = 1;
        {
            ...
            ID = make_disk("pupoutmask", size, size, 0.5 * size, 0.5 * size,
                           piaacmcopticaldesign.pupoutmaskrad * piaacmcopticaldesign.beamrad /
    ...
    if (piaacmcparams.PIAACMC_save == 1)
    {
        ...
        WRITE_FULLFILENAME(fname, "%s/conjugations.txt", piaacmcparams.piaacmcconfdir);
        fp = fopen(fname, "w");
        if (fp == NULL)
        {
