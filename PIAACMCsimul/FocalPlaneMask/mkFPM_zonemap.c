...
errno_t mkFPM_zonemap(const char *__restrict__ IDname, imageID *outID)
{
    ...
    nbsectorcumul = (uint_fast32_t *)malloc(sizeof(uint_fast32_t) * piaacmcopticaldesign.NBrings);
    ...
    case 1: // rings broken in sectors
        WRITE_FULLFILENAME(fname, "%s/fpm_zonescoord_%d_%03ld.txt", piaacmcparams.piaacmcconfdir,
    ...
    if ((piaacmcparams.PIAACMC_FPMsectors == 0) || (piaacmcparams.PIAACMC_FPMsectors == 1))
    {
    ...
    {
        hexsteppix =
            0.5 * piaacmcopticaldesign.fpmarraysize / piaacmcopticaldesign.NBrings * piaacmcparams.FPMSCALEFACTOR;
        for (hindex = 0; hindex < hindexMax; hindex++)
        {
