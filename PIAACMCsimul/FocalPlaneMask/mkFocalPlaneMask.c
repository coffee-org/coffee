...
*/
errno_t mkFocalPlaneMask(const char *IDzonemap_name, const char *ID_name, int mode, int saveMask, imageID *outID)
{
    DEBUG_TRACE_FSTART();
    ...
    imageID IDsag;
    FUNC_CHECK_RETURN(create_3Dimage_ID("fpmsag", size, size, nblambda, &IDsag));

    imageID IDzone;
    FUNC_CHECK_RETURN(create_3Dimage_ID("fpmzone", size, size, nblambda, &IDzone));
    ...
    printf("===================== Make focal plane mask  %s %s   mode=%d/%ld   [%d %d]\n", IDzonemap_name, ID_name,
           mode, piaacmcopticaldesign.focmNBzone, CentCone, OuterCone);
    ...
    double fpscale = (2.0 * piaacmcopticaldesign.beamrad / piaacmcopticaldesign.pixscale) / piaacmcopticaldesign.size /
                     piaacmcopticaldesign.fpzfactor * piaacmcopticalsystem.lambdaarray[0] * piaacmcopticaldesign.Fratio;
    printf("piaacmcopticaldesign.fpmRad = %g m    fpscale[0] = %g m/pix   mode = %d\n", piaacmcopticaldesign.fpmRad,
           fpscale, mode);

    printf("Allocate memory\n");
    fflush(stdout);
    tarray = (double *)malloc(sizeof(double) * piaacmcopticaldesign.focmNBzone * nblambda);
    if (tarray == NULL)
    {
    ...
    aarray = (double *)malloc(sizeof(double) * piaacmcopticaldesign.focmNBzone * nblambda);
    if (aarray == NULL)
    {
        PRINT_ERROR("malloc returns NULL pointer");
        abort();
    ...
    phaarray = (double *)malloc(sizeof(double) * piaacmcopticaldesign.focmNBzone * nblambda);
    if (phaarray == NULL)
    {
        PRINT_ERROR("malloc returns NULL pointer");
        abort();
    ...
    cosphaarray = (double *)malloc(sizeof(double) * piaacmcopticaldesign.focmNBzone * nblambda);
    if (cosphaarray == NULL)
    {
        PRINT_ERROR("malloc returns NULL pointer");
        abort();
    ...
    sinphaarray = (double *)malloc(sizeof(double) * piaacmcopticaldesign.focmNBzone * nblambda);
    if (sinphaarray == NULL)
    {
        PRINT_ERROR("malloc returns NULL pointer");
        abort();
    ...
    for (int k = 0; k < nblambda; k++)
        for (long zi = 0; zi < piaacmcopticaldesign.focmNBzone; zi++)
        {
            ...
            tarray[piaacmcopticaldesign.focmNBzone * k + zi] = data.image[piaacmcopticaldesign.zonezID].array.D[zi];
            ...
            //fflush(stdout);
            aarray[piaacmcopticaldesign.focmNBzone * k + zi] = data.image[piaacmcopticaldesign.zoneaID].array.D[zi];
            ...
            //
            phaarray[piaacmcopticaldesign.focmNBzone * k + zi] = OpticsMaterials_pha_lambda(
                piaacmcopticaldesign.fpmmaterial_code, tarray[piaacmcopticaldesign.focmNBzone * k + zi],
...
#ifdef HAVE_LIBGOMP
#pragma omp parallel default(shared) private(ii, jj, x, y, r, retmp, imtmp, iii, jjj, ii1, jj1, zi, t, a, fpscale,     \
                                             amp, pha, cospha, sinpha, ttmp, zonetmp)
    {
#pragma omp for
#endif
        for (int k = 0; k < nblambda; k++)
        {
            fpscale = (2.0 * piaacmcopticaldesign.beamrad / piaacmcopticaldesign.pixscale) / piaacmcopticaldesign.size /
                      piaacmcopticaldesign.fpzfactor * piaacmcopticalsystem.lambdaarray[k] *
                      piaacmcopticaldesign.Fratio;
            printf("LAMBDA %3d / %3d = %10.5g m    SCALE = %10.5g m/pix   size=%4ul  rad=%g\n", k, nblambda,
                   piaacmcopticalsystem.lambdaarray[k], fpscale, size, piaacmcopticaldesign.fpmRad);
                ...
                for (uint32_t jj = 0; jj < size; jj++)
                {
                    ...
                    if (OuterCone == 1)
                    {
                        if ((r > 0.9 * piaacmcopticaldesign.fpmRad) &&
                            (r < piaacmcopticaldesign.fpmOuterConeRad)) // outer cone
                        {
                            double t = piaacmcopticaldesign.fpmOuterConeZ * (piaacmcopticaldesign.fpmOuterConeRad - r) /
                                       (piaacmcopticaldesign.fpmOuterConeRad - piaacmcopticaldesign.fpmRad);
                            double pha = OpticsMaterials_pha_lambda(piaacmcopticaldesign.fpmmaterial_code, t,
                                                                    piaacmcopticalsystem.lambdaarray[k]);
                            cospha = cosf(pha);
                            sinpha = sinf(pha);
                            a = 1.0;
                    ...
                    if (r < 1.1 * piaacmcopticaldesign.fpmRad) //     fine sampling
                    {
                        ...
                        for (long iii = 0; iii < NBsubPix; iii++)
                        {
                            for (long jjj = 0; jjj < NBsubPix; jjj++)
                            {
                                ...
                                long ii1 = (long)((0.5 + 0.5 * x / piaacmcopticaldesign.fpmRad *
                                                             piaacmcparams.FPMSCALEFACTOR) *
                                                      piaacmcopticaldesign.fpmarraysize +
                                                  0.5);
                                long jj1 = (long)((0.5 + 0.5 * y / piaacmcopticaldesign.fpmRad *
                                                             piaacmcparams.FPMSCALEFACTOR) *
                                                      piaacmcopticaldesign.fpmarraysize +
                                                  0.5);

                                if ((ii1 > -1) && (ii1 < piaacmcopticaldesign.fpmarraysize) && (jj1 > -1) &&
                                    (jj1 < piaacmcopticaldesign.fpmarraysize))
                                {
                                    if (CentCone == 1)
                                    {
                                        // central cone
                                        if ((r < 0.99 * piaacmcopticaldesign.fpmRad) &&
                                            (r < piaacmcopticaldesign.fpmCentConeRad))
                                        {
                                            t = piaacmcopticaldesign.fpmCentConeZ +
                                                (r / piaacmcopticaldesign.fpmCentConeRad) *
                                                    (0.5 * (piaacmcopticaldesign.fpmminsag +
                                                            piaacmcopticaldesign.fpmmaxsag) -
                                                     piaacmcopticaldesign.fpmCentConeZ);
                                    ...
                                    // Zone number
                                    zi = (long)(data.image[IDz]
                                                    .array.UI16[jj1 * piaacmcopticaldesign.fpmarraysize + ii1]);
                                    if (zi - 1 > data.image[piaacmcopticaldesign.zonezID].md[0].size[0] - 1)
                                    {
                                        printf("ERROR: Zone %d does not exist (image %s has size %ld %ld)   pix %ld "
                                               "%ld   %ld\n",
                                               (int)zi, data.image[piaacmcopticaldesign.zonezID].md[0].name,
                                               (long)data.image[piaacmcopticaldesign.zonezID].md[0].size[0],
                                               (long)data.image[piaacmcopticaldesign.zonezID].md[0].size[1], (long)jj1,
                                               (long)jj1, piaacmcopticaldesign.fpmarraysize);
                                        exit(0);
                                    }
                                    if (zi > 0)
                                    {
                                        t = data.image[piaacmcopticaldesign.zonezID].array.D[zi - 1]; // thickness
                                        a = data.image[piaacmcopticaldesign.zoneaID]
                                                .array.D[zi - 1]; // amplitude transmission
                                        cospha = cosphaarray[piaacmcopticaldesign.focmNBzone * k + zi - 1];
                                        sinpha = sinphaarray[piaacmcopticaldesign.focmNBzone * k + zi - 1];
                                        amp = a;
                                    }
    ...
    if (saveMask == 1)
    {
        ...
        WRITE_FULLFILENAME(fname, "%s/fpm_sagmap2D.%s.fits.gz", piaacmcparams.piaacmcconfdir, piaacmcparams.fnamedescr);
        FUNC_CHECK_RETURN(save_fits("fpmsag", fname));

        /* save zones */
        WRITE_FULLFILENAME(fname, "%s/fpm_zonemap2D.%s.fits.gz", piaacmcparams.piaacmcconfdir,
                           piaacmcparams.fnamedescr);
        FUNC_CHECK_RETURN(save_fits("fpmzone", fname));

        /* save mask transmission */
        imageID IDm;
        FUNC_CHECK_RETURN(create_3DCimage_ID("fpmCA", size, size, nblambda, &IDm));
            ...
            for (uint32_t ii = 0; ii < size; ii++)
                for (uint32_t jj = 0; jj < size; jj++)
                {
                    data.image[IDm].array.CF[k * size2 + jj * size + ii].re =
                        1.0 - data.image[ID].array.CF[k * size2 + jj * size + ii].re;
        ...
        FUNC_CHECK_RETURN(delete_image_ID("fpmCA", DELETE_IMAGE_ERRMODE_WARNING));

        WRITE_FULLFILENAME(fname, "%s/fpm_CAampmap2D.%s.fits.gz", piaacmcparams.piaacmcconfdir,
                           piaacmcparams.fnamedescr);
        FUNC_CHECK_RETURN(save_fits("tfpma", fname));

        WRITE_FULLFILENAME(fname, "%s/fpm_CAphamap2D.%s.fits.gz", piaacmcparams.piaacmcconfdir,
                           piaacmcparams.fnamedescr);
