...
errno_t exec_optimize_fpm_zones()
{
    ...
    piaacmcparams.LINOPT = 1; // perform linear optimization after the switch exits
    ...
    */
    ...
    piaacmcparams.vsize = data.image[IDfpmresp].md[0].size[0]; // number of eval pts x2
    // make an array that holds the resulting light for evaluation point given the FPM solution, for each wavelenth
    //ID =
    FUNC_CHECK_RETURN(create_2Dimage_ID("imvect1", piaacmcparams.vsize, piaacmcopticaldesign.nblambda, NULL));
    ...
    piaacmcparams.dphadz_array = (double *)malloc(sizeof(double) * piaacmcopticaldesign.nblambda);
    ...
    for (long k = 0; k < piaacmcopticaldesign.nblambda; k++)
    {
        ...
        piaacmcparams.dphadz_array[k] =
            OpticsMaterials_pha_lambda(piaacmcopticaldesign.fpmmaterial_code, 1.0, piaacmcopticalsystem.lambdaarray[k]);
        printf("%ld  %g %g\n", k, piaacmcopticalsystem.lambdaarray[k], piaacmcparams.dphadz_array[k]);
    }
    piaacmcparams.outtmp_array =
        (double *)malloc(sizeof(double) * (piaacmcparams.vsize * piaacmcopticaldesign.nblambda +
    ...
    {
        data.image[piaacmcopticaldesign.zonezID].array.D[k] += piaacmcparams.MODampl * (1.0 - 2.0 * ran1());
    }
    ...
    for (long mz = 0; mz < data.image[piaacmcopticaldesign.zonezID].md[0].size[0]; mz++)
    {
        ...
        piaacmcparams.linopt_paramdelta[piaacmcparams.linopt_number_param] = 3.0e-9;
        // max parameter step size
        piaacmcparams.linopt_parammaxstep[piaacmcparams.linopt_number_param] = 1.0e-6;
        // max and min allowed values for parameter
        piaacmcparams.linopt_parammin[piaacmcparams.linopt_number_param] = piaacmcopticaldesign.fpmminsag;
        piaacmcparams.linopt_parammax[piaacmcparams.linopt_number_param] = piaacmcopticaldesign.fpmmaxsag;
        // move on to next parameter
        piaacmcparams.linopt_number_param++;
    }
    piaacmcparams.PIAACMC_FPM_FASTDERIVATIVES = 1; // for fast execution using analytic derivatives
