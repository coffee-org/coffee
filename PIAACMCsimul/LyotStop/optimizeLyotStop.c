 ...
 *
*/
errno_t optimizeLyotStop(const char *__restrict__ IDamp_name, const char *__restrict__ IDpha_name,
                         const char *__restrict__ IDincohc_name, float zmin, float zmax, double throughput, long NBz,
                         long NBmasks, double *outratioval)
{
    DEBUG_TRACE_FSTART();
    DEBUG_TRACEPOINT("FARG %s %s %s %f %f %lf %ld %ld", IDamp_name, IDpha_name, IDincohc_name, zmin, zmax, throughput,
                     NBz, NBmasks);
    ...
    imageID ID_LMintC;
    FUNC_CHECK_RETURN(create_3Dimage_ID("LMintC", xsize, ysize, NBz, &ID_LMintC));
    ...
    for (long l = 0; l < NBz; l++)
    {
        ...
        FUNC_CHECK_RETURN(
            OptSystProp_propagateCube(&piaacmcopticalsystem, 0, IDamp_name, IDpha_name, nameamp, namepha, zprop, 0));
        ...
        for (long k = 0; k < nblambda; k++)
        {
            ...
            for (uint64_t ii = 0; ii < xysize; ii++)
            {
                data.image[ID_LMintC].array.F[l * xsize * ysize + ii] += data.image[IDintgg].array.F[ii];
            }
            //data.image[IDa1].array.F[ii]*data.image[IDa1].array.F[ii]; //data.image[IDintgg].array.F[ii];

            FUNC_CHECK_RETURN(delete_image_ID("retmpimg", DELETE_IMAGE_ERRMODE_WARNING));

            FUNC_CHECK_RETURN(delete_image_ID("imtmpimg", DELETE_IMAGE_ERRMODE_WARNING));

            FUNC_CHECK_RETURN(delete_image_ID("tmpintgg", DELETE_IMAGE_ERRMODE_WARNING));
        }
        ...
        FUNC_CHECK_RETURN(delete_image_ID(nameamp, DELETE_IMAGE_ERRMODE_WARNING));

        FUNC_CHECK_RETURN(delete_image_ID(namepha, DELETE_IMAGE_ERRMODE_WARNING));
    }
...

    {
        ...
        for (long m = 0; m < NBmasks; m++)
        {
            ...
            for (long l = 0; l < NBz; l++)
            {
                ...
                printf("MASK %ld   z(%ld)= %f  ->  %g   ( %g %g) \n", m, l, zarray[l], val, tot2array[l * NBmasks + m],
    ...
    imageID IDlyotmask;
    FUNC_CHECK_RETURN(mkLyotMask("LcombOA", "Lcomb", "LMzonemap", throughput, "LMask", &IDlyotmask));
    ...
    for (long m = 0; m < NBmasks; m++)
    {
        ...
        for (uint32_t ii = 0; ii < xsize; ii++)
            for (uint32_t jj = 0; jj < ysize; jj++)
            {
                ...
                {
                    data.image[IDm].array.F[jj * xsize + ii] = data.image[IDlyotmask].array.F[jj * xsize + ii];
                }
                else
                {
        ...
        if (m == 0)
            for (uint32_t ii = 0; ii < xsize; ii++)
                ...
                {
                    double x =
                        (1.0 * ii - 0.5 * xsize) / (piaacmcopticaldesign.beamrad / piaacmcopticaldesign.pixscale);
                    double y =
                        (1.0 * jj - 0.5 * xsize) / (piaacmcopticaldesign.beamrad / piaacmcopticaldesign.pixscale);
                    double r = sqrt(x * x + y * y);
                    if (r > 1.0)
                    {
    ...
    FUNC_CHECK_RETURN(delete_image_ID("LMzonemap", DELETE_IMAGE_ERRMODE_WARNING));
