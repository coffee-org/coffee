...
errno_t exec_optimize_lyot_stops_shapes_positions()
{
    ...
    // compute the reference on-axis PSF
    {
        double cval = 0.0;
        FUNC_CHECK_RETURN(
            PIAACMCsimul_computePSF(0.0, 0.0, 0, optsyst[0].NBelem, 0, 0, 0, 0, &cval)
        );
    }
    ...
    {
        double cval = 0.0;
        FUNC_CHECK_RETURN(PIAACMCsimul_computePSF(0.0, 0.0, 0, optsyst[0].NBelem, 0, 0, 0, 0, &cval));
    }
    ...
    FUNC_CHECK_RETURN(
        PIAACMCsimul_CA2propCubeInt(fnamea, fnamep, zmin, zmax, NBpropstep, "iprop00", NULL)
    );
    ...
    ///
    FUNC_CHECK_RETURN(optimizeLyotStop_offaxis_min("OAincohc"));
    ...
    FUNC_CHECK_RETURN(
        optimizeLyotStop(fnamea, fnamep, "OAincohc", zmin, zmax, lstransm,
                         NBpropstep, piaacmc[0].NBLyotStop, NULL)
    );
