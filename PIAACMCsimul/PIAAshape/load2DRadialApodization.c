 ...
 * @return errno_t
 */
errno_t load2DRadialApodization(const char *__restrict__ IDapo_name, float beamradpix,
                                const char *__restrict__ IDapofit_name)
{
    ...
    { // CREATE MASK AND CROP INPUT
        imageID IDmask;
        FUNC_CHECK_RETURN(create_2Dimage_ID("fitmaskapo", sizem, sizem, &IDmask));
        ...
        for (uint32_t ii = 0; ii < sizem; ii++)
            for (uint32_t jj = 0; jj < sizem; jj++)
            {
                data.image[ID].array.F[jj * sizem + ii] =
                    data.image[IDin].array.F[(jj + offset) * sizein + (ii + offset)];
    ...
    EXECUTE_SYSTEM_COMMAND("mv %s/eigenv.dat %s/eigenv_APOmodesCos.dat", piaacmcparams.piaacmcconfdir,
    ...
    if (debug == 1) // test fit quality
    {
        ...
        FUNC_CHECK_RETURN(linopt_imtools_image_construct("APOmodesCos", IDapofit_name, "testapofitsol", NULL));
    ...
    FUNC_CHECK_RETURN(delete_image_ID("_apoincrop", DELETE_IMAGE_ERRMODE_WARNING));

    FUNC_CHECK_RETURN(delete_image_ID("fitmaskapo", DELETE_IMAGE_ERRMODE_WARNING));
