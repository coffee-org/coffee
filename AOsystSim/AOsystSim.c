...
{
    if (0 + CLI_checkarg(1, CLIARG_STR_NOT_IMG) + CLI_checkarg(1, CLIARG_STR_NOT_IMG) == 0)
    {
...
{
    if (0 + CLI_checkarg(1, CLIARG_IMG) + CLI_checkarg(2, CLIARG_STR_NOT_IMG) == 0)
    {
...
static errno_t AOsystSim_FPWFS_mkprobes__cli()
{
    if (0 + CLI_checkarg(1, CLIARG_STR_NOT_IMG) + CLI_checkarg(2, CLIARG_STR_NOT_IMG) + CLI_checkarg(3, CLIARG_LONG) +
            CLI_checkarg(4, CLIARG_LONG) + CLI_checkarg(5, CLIARG_FLOAT) + CLI_checkarg(6, CLIARG_FLOAT) +
            CLI_checkarg(7, CLIARG_FLOAT) + CLI_checkarg(8, CLIARG_FLOAT) + CLI_checkarg(9, CLIARG_LONG) ==
        0)
    {
        AOsystSim_FPWFS_mkprobes(
            data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string, data.cmdargtoken[3].val.numl,
            data.cmdargtoken[4].val.numl, data.cmdargtoken[5].val.numf, data.cmdargtoken[6].val.numf,
            data.cmdargtoken[7].val.numf, data.cmdargtoken[8].val.numf, data.cmdargtoken[9].val.numl);
...
{
    if (0 + CLI_checkarg(1, CLIARG_LONG) + CLI_checkarg(2, CLIARG_LONG) + CLI_checkarg(3, CLIARG_LONG) +
            CLI_checkarg(4, CLIARG_LONG) ==
        0)
    {
        AOsystSim_FPWFS_sensitivityAnalysis(data.cmdargtoken[1].val.numl, data.cmdargtoken[2].val.numl,
...
{
    RegisterCLIcommand("AOsimfilt", __FILE__, AOsystSim_simpleAOfilter__cli, "AO simple filtering",
                       "<input WF> <output WF>", "AOsimfilt wfin wfout",
                       "int AOsystSim_simpleAOfilter(const char *IDin_name, const char *IDout_name)");

    RegisterCLIcommand("AOsystsfitpup", __FILE__, AOsystSim_fitTelPup__cli, "fit telescope pupil", "tel pupil file",
                       "AOsystfitpup", "AOsystSim_fitTelPup(const char *ID_name)");

    RegisterCLIcommand("AOsimmkWF", __FILE__, AOsystSim_mkWF__cli, "make WF series for AO simulation",
                       "<configuration file name>", "AOsystmkWF WF.conf", "int AOsystSim_mkWF(const char *CONF_FNAME)");

    RegisterCLIcommand("AOsimPyrWFS", __FILE__, AOsystSim_PyrWFS__cli, "run pyramid WFS for AO simulation",
                       "<configuration file name>", "AOsimPyrWFS PyrWFS.conf",
                       "int AOsystSim_PyrWFS(const char *CONF_FNAME)");

    RegisterCLIcommand("AOsimDM", __FILE__, AOsystSim_DM__cli, "run DM for AO simulation", "<configuration file name>",
                       "AOsimDM PyrWFS.conf", "int AOsystSim_DM(const char *CONF_FNAME)");

    RegisterCLIcommand("AOsimcoroLOWFS", __FILE__, AOsystSim_coroLOWFS__cli, "run coro and LOWFS for AO simulation",
                       "<configuration file name>", "AOsimcoroLOWFS LOWFS.conf",
                       "int AOsystSim_coroLOWFS(const char *CONF_FNAME)");

    RegisterCLIcommand("AOsystsim", __FILE__, AOsystSim_run__cli, "run fake AO system",
                       "<syncmode> <DMindex> <delayus>", "AOsystsim",
                       "int AOsystSim_run(int syncmode, long DMindex, long delayus)");

    RegisterCLIcommand("AOsystexaosim", __FILE__, AOsystSim_extremeAO_contrast_sim, "run extremeAO analysis",
                       "no argument", "AOsystexaosim", "int AOsystSim_extremeAO_contrast_sim()");

    RegisterCLIcommand(
        "AOsystmkABprobes", __FILE__, AOsystSim_FPWFS_mkprobes__cli,
        "make AB probes for focal plane sensing and coherence measurement", "probes params",
        "AOsystmkABprobes prA prB 50 50 0.7 0.1 0.7 0.1 1",
        "int AOsystSim_FPWFS_mkprobes(const char *IDprobeA_name, const char *IDprobeB_name, long dmxsize, long "
        "dmysize, double CPAmax, double CPArmin, double CPArmax, double RMSampl, long modegeom)");

    RegisterCLIcommand("AOsystFPWFSan", __FILE__, AOsystSim_FPWFS_sensitivityAnalysis__cli,
                       "run focal plane WFS sensitivity analysis", "<mapmode> <mode> <optmode> <NBprobes>",
                       "AOsystFPWFSan",
...
int AOsystSim_run(int syncmode, long DMindex, long delayus)
{
    ...
    if (IDdmctrl == -1)
    {
        ...
        create_image_ID(name, 2, dmsizearray, _DATATYPE_FLOAT, 1, 0, 0, &IDdmctrl);
        free(dmsizearray);
    ...
    list_image_ID();
    printf("dmifc = %ld   %ld %ld %ld    %ld %ld\n", IDifc, (long)data.image[IDifc].md[0].size[0],
           (long)data.image[IDifc].md[0].size[1], (long)data.image[IDifc].md[0].size[2], arraysize, arraysize);
            ...
            for (ii = 0; ii < arraysize; ii++)
                for (jj = 0; jj < arraysize; jj++)
                {
                    ...
                    if ((rxi > 0) && (rxi < arraysize - 1) && (ryi > 0) && (ryi < arraysize - 1))
                    {
                        ...
                        data.image[IDifc].array.F[(my * DMsize + mx) * arraysize * arraysize + jj * arraysize + ii] =
                            (1.0 - u) * (1.0 - t) * v00 + (1.0 - u) * t * v10 + u * (1.0 - t) * v01 + u * t * v11;
                    }
                }
        }
    free(imsize);
    ...
    memcpy(data.image[IDout].array.F, data.image[ID].array.F,
           sizeof(float) * data.image[ID].md[0].size[0] * data.image[ID].md[0].size[1] * data.image[ID].md[0].size[2]);
    ...
    for (;;)
    {
        ...
        data.image[IDout].md[0].write = 1;
        memcpy(data.image[IDout].array.F, data.image[ID].array.F,
               sizeof(float) * data.image[ID].md[0].size[0] * data.image[ID].md[0].size[1] *
        ...
        create_image_ID(imdhname, 2, dhsizearray, _DATATYPE_FLOAT, 1, 0, 0, &IDdh);
        data.image[IDdh].md[0].write = 1;
        for (ii = 0; ii < dhxsize; ii++)
            for (jj = 0; jj < dhysize; jj++)
            {
                data.image[IDdh].array.F[jj * (2 * dhxsize) + ii] =
                    data.image[IDre].array.F[(jj + dhyoffset) * arraysize + (ii + dhxoffset)] *
                    data.image[IDdhmask].array.F[jj * dhxsize + ii];
                data.image[IDdh].array.F[jj * (2 * dhxsize) + (ii + dhxsize)] =
                    data.image[IDim].array.F[(jj + dhyoffset) * arraysize + (ii + dhxoffset)] *
                    data.image[IDdhmask].array.F[jj * dhxsize + ii];
            }
...
int AOsystSim_simpleAOfilter(const char *IDin_name, const char *IDout_name)
{
    ...
    create_3Dimage_ID("wfcbuff", sizearray[0], sizearray[1], wfcbuff_size, &IDwfcbuff);
    ...
    create_image_ID("aofiltwfe", 2, sizearray, _DATATYPE_FLOAT, 1, 0, 0, &IDwfe);
    ...
    for (;;)
    {
        ...
        if (data.image[IDin].md[0].cnt0 != cnt0)
        {
            ...
            printf("\r Time : %.6lf    WFSexposure time:  %.6f   [%5ld]    %10.5lf -> %10.5lf ", tnowdouble,
                   tnowdouble - time_wfse0, wfsecnt, WFrms0, WFrms);
            fflush(stdout);
                    ...
                    if (wfcbuff_time[k] < tnowdouble)
                    {
                        ...
                        for (uint64_t ii = 0; ii < sizearray[0] * sizearray[1]; ii++)
                        {
                            data.image[IDdm].array.F[ii] +=
                                data.image[IDaosf_gain].array.F[ii] * data.image[IDwfcbuff].array.F[k * size2 + ii];
                        }
                        wfcbuff_status[k] = 0;
                    }
            }
...
errno_t AOsystSim_extremeAO_contrast_sim()
{
    ...
    exaosimconf[0].Fwfs = zeroptWFS * 1.0e6 * (exaosimconf[0].lambdawfs * lambdaBwfs) *
                          pow(100.0, -0.2 * sourcemag_wfs) * lambdaBwfs * systemEfficiency;
    exaosimconf[0].Fsci = zeroptWFSsci * 1.0e6 * (exaosimconf[0].lambdai * lambdaBsci) *
    ...
    for (exaosimconf[0].alpha_arcsec = 0.010; exaosimconf[0].alpha_arcsec < 2.0; exaosimconf[0].alpha_arcsec += 0.001)
    {
        ...
        if (1) // SHWFS
        {
            ...
            exaosimconf[0].betapWFS =
                1.48 / (exaosimconf[0].f * dsa) * sqrt(1.0 + (dsa * dsa / exaosimconf[0].r0 / exaosimconf[0].r0));
        }

        // aberration amplitude at frequency f [m]
        exaosimconf[0].hf = 0.22 * exaosimconf[0].lambda0 /
                            (pow(exaosimconf[0].f, 11.0 / 6.0) * exaosimconf[0].D * pow(exaosimconf[0].r0, 5.0 / 6.0));
        ...
        for (CN2layer = 0; CN2layer < 20; CN2layer++)
        {
            tmpv1 = cos(M_PI * exaosimconf[0].CN2layer_h[CN2layer] * exaosimconf[0].f * exaosimconf[0].f *
                        exaosimconf[0].lambdai);
            tmpv2 = cos(M_PI * exaosimconf[0].CN2layer_h[CN2layer] * exaosimconf[0].f * exaosimconf[0].f *
                        exaosimconf[0].lambdawfs);
            tmpv3 = sin(M_PI * exaosimconf[0].CN2layer_h[CN2layer] * exaosimconf[0].f * exaosimconf[0].f *
                        exaosimconf[0].lambdai);
            tmpv4 = sin(M_PI * exaosimconf[0].CN2layer_h[CN2layer] * exaosimconf[0].f * exaosimconf[0].f *
                        exaosimconf[0].lambdawfs);
            exaosimconf[0].X += exaosimconf[0].CN2layer_coeff[CN2layer] * tmpv1 * tmpv1;
            exaosimconf[0].Xwfs += exaosimconf[0].CN2layer_coeff[CN2layer] * tmpv2 * tmpv2;
        ...
        tmpA = 2.0 * M_PI * exaosimconf[0].hf * exaosimconf[0].windspeed * exaosimconf[0].f *
               exaosimconf[0].framedelayMult;
        // NOTE: tmpB IS PHOTON NOISE TERM [m]
        tmpB = exaosimconf[0].lambdawfs / M_PI * exaosimconf[0].betapWFS / sqrt(exaosimconf[0].Fwfs * M_PI) /
               exaosimconf[0].D;
        ...
        exaosimconf[0].hfc =
            sqrt(exaosimconf[0].hfca * exaosimconf[0].hfca + exaosimconf[0].hfcb * exaosimconf[0].hfcb);
        ...
        // NOTE: tmpA IS TEMPORAL LAG ERROR [m]
        tmpA = 2.0 * M_PI * exaosimconf[0].hf * exaosimconf[0].Ywfs * exaosimconf[0].windspeed * exaosimconf[0].f *
               exaosimconf[0].framedelayMult;
        // NOTE: tmpB IS PHOTON NOISE TERM [m]
        tmpB = exaosimconf[0].lambdawfs / M_PI * exaosimconf[0].betaaWFS / sqrt(exaosimconf[0].Fwfs * M_PI) /
               exaosimconf[0].D;
        ...
        for (CN2layer = 0; CN2layer < 20; CN2layer++)
        {
            ...
            exaosimconf[0].CC_ORPA += pow(M_PI * tmpv1 / exaosimconf[0].lambdai, 2.0) *
                                      exaosimconf[0].CN2layer_coeff[CN2layer] * exaosimconf[0].Xwfs;
            exaosimconf[0].CC_ARPA += pow(M_PI * tmpv1 / exaosimconf[0].lambdai, 2.0) *
        ...
        //
        exaosimconf[0].twfs_opt_amp = exaosimconf[0].twfs_opt * pow(exaosimconf[0].X / exaosimconf[0].Y, 1.0 / 3.0) *
        ...
        printf("single frequ WF error: %g -> %g  (%g + %g)\n", exaosimconf[0].hf, exaosimconf[0].hfc,
               exaosimconf[0].hfca, exaosimconf[0].hfcb);
        ...
        printf("   WFS  lag = %20g    %20g\n", pow(M_PI * exaosimconf[0].hfca / exaosimconf[0].lambdai, 2.0),
               pow(M_PI * exaosimconf[0].hfca / exaosimconf[0].lambdai, 2.0) / sqrt(tobs / crosstime) * 2.0);
        printf("   WFS noise= %20g    %20g\n", pow(M_PI * exaosimconf[0].hfcb / exaosimconf[0].lambdai, 2.0),
               pow(M_PI * exaosimconf[0].hfcb / exaosimconf[0].lambdai, 2.0) / sqrt(tobs / exaosimconf[0].twfs) * 2.0);
        ...
        */
        ...
        if (initprint == 0)
        {
            ...
            fprintf(fp,
                    "# col #18 : exaosimconf[0].twfs_Aopt Optimal effective WFS exposure time for AMP correction\n");
        ...
        fprintf(fp, "%10.8f %10.8f %10.8f %10.8f   ", exaosimconf[0].X, exaosimconf[0].Y, exaosimconf[0].Xwfs,
                exaosimconf[0].Ywfs);
    ...
    fprintf(fp, "plot [0:0.3][-11:0] \"ContrastTerms.out.txt\" u 1:13 lw 3 w l title \"Residual OPD correction error - "
                "WFS photon noise [CR_{OPHN}]\"");
    fprintf(fp, ", \"ContrastTerms.out.txt\" u 1:14 lw 3 w l title \"Residual OPD correction error - Temporal lag "
                "[CR_{OTEM}]\"");
    fprintf(fp, ", \"ContrastTerms.out.txt\" u 1:4 lw 3 w l title \"Residual Amplitude - Scintillation [CR_{UAMP}]\"");
    fprintf(fp, ", \"ContrastTerms.out.txt\" u 1:16 lw 3 w l title \"Residual Amplitude correction error - WFS photon "
                "noise [CR_{APHN}]\"");
    fprintf(fp, ", \"ContrastTerms.out.txt\" u 1:17 lw 3 w l title \"Residual Amplitude correction error - Temporal "
                "lag [CR_{ATEM}]\"");
    fprintf(fp, ", \"ContrastTerms.out.txt\" u 1:19 lw 3 w l title \"Chromatic OPD - Multiplicative refractivity "
                "[CC_{OMUL}]\"");
    fprintf(fp, ", \"ContrastTerms.out.txt\" u 1:20 lw 3 w l title \"Chromatic Amplitude - Multiplicative refractivity "
                "[CC_{AMUL}]\"");
    fprintf(fp,
            ", \"ContrastTerms.out.txt\" u 1:21 lw 3 w l title \"Chromatic OPD - Fresnel propagation [CC_{OPRO}]\"");
    fprintf(
        fp,
        ", \"ContrastTerms.out.txt\" u 1:22 lw 3 w l title \"Chromatic Amplitude - Fresnel propagation [CC_{APRO}]\"");
    fprintf(fp,
            ", \"ContrastTerms.out.txt\" u 1:23 lw 3 w l title \"Chromatic OPD - Refractive Light Path [CC_{OPRA}]\"");
    fprintf(fp, ", \"ContrastTerms.out.txt\" u 1:24 lw 3 w l title \"Chromatic Amplitude - Refractive Light Path "
                "[CC_{APRA}]\"");
    fprintf(fp, "\n");
    fprintf(fp, "exit\n");
    fprintf(fp, "EOF\n");
...
long AOsystSim_mkTelPupDM(const char *ID_name, long msize, double xc, double yc, double rin, double rout, double pupPA,
...
long AOsystSim_fitTelPup(const char *ID_name, const char *IDtelpup_name)
{
    ...
    printf("%f %f\n", vp10, vp90);
    for (uint64_t ii = 0; ii < data.image[ID].md[0].size[0] * data.image[ID].md[0].size[1]; ii++)
    {
        ...
        for (coeffy = -1.5; coeffy < 1.5; coeffy += 0.05)
        {
            for (uint32_t ii = 0; ii < size; ii++)
                for (uint32_t jj = 0; jj < size; jj++)
                {
                    x = 1.0 * ii / size;
                    y = 1.0 * jj / size;
                    data.image[ID1].array.F[jj * size + ii] = data.image[ID].array.F[jj * size + ii] *
    ...
    for (uint32_t ii = 0; ii < size; ii++)
        for (uint32_t jj = 0; jj < size; jj++)
        {
            ...
            data.image[ID].array.F[jj * size + ii] =
                data.image[ID].array.F[jj * size + ii] * (1.0 + coeffx1 * (x - 0.5)) * (1.0 + coeffy1 * (y - 0.5));
        }
    ...
    for (iter = 0; iter < NBiter; iter++)
    {
        ...
        for (spiderPA = spiderPA_min; spiderPA < spiderPA_max + eps; spiderPA += (spiderPA_max - spiderPA_min) / nbstep)
            for (spideroffset = spideroffset_min; spideroffset < spideroffset_max + eps;
                        ...
                        for (xc = xc_min; xc < xc_max + eps; xc += (xc_max - xc_min) / nbstep)
                            for (yc = yc_min; yc < yc_max + eps; yc += (yc_max - yc_min) / nbstep)
                                     ...
                                     pupPA += (pupPA_max - pupPA_min) / nbstep)
                                {
                                    ...
                                    AOsystSim_mkTelPupDM("testpup", size, xc, yc, rin, rout, pupPA, spiderPA,
                                                         spideroffset, spiderthick, stretchx);
    ...
    AOsystSim_mkTelPupDM(IDtelpup_name, size, xc1, yc1, rin, rout, pupPA1, spiderPA1, spideroffset1, spiderthick,
                         stretchx1);
...
int AOsystSim_mkWF(const char *CONF_FNAME)
{
    ...
    sizearray[0] = 1;
    sizearray[1] = 1;
    create_image_ID(OUTPHYSTIME, 2, sizearray, _DATATYPE_FLOAT, 1, 0, 0, &IDphystime);
    phystime = 0;
    ...
    if (OUT0STREAM > 0)
    {
        create_image_ID(OUT0STREAMNAMEOPD, 2, sizearray, _DATATYPE_FLOAT, 1, 0, 0, &IDopd0);
        COREMOD_MEMORY_image_set_createsem(OUT0STREAMNAMEOPD, 10);
    }
    else
    {
        create_image_ID(OUT0STREAMNAMEOPD, 2, sizearray, _DATATYPE_FLOAT, 0, 0, 0, &IDopd0);
    }

    if (OUT0STREAM > 0) // create amp stream if OUT0STREAM=1 or 2, but will only update it if OUT0STREAM=2
    {
        create_image_ID(OUT0STREAMNAMEAMP, 2, sizearray, _DATATYPE_FLOAT, 1, 0, 0, &IDamp0);
        COREMOD_MEMORY_image_set_createsem(OUT0STREAMNAMEAMP, 10);
    }
    else
    {
        create_image_ID(OUT0STREAMNAMEAMP, 2, sizearray, _DATATYPE_FLOAT, 0, 0, 0, &IDopd0);
    }
    ...
    if (DM0MODE > 0)
    {
        if (OUT1STREAM > 0)
        {
            create_image_ID(OUT1STREAMNAMEOPD, 2, sizearray, _DATATYPE_FLOAT, 1, 0, 0, &IDopd1);
            COREMOD_MEMORY_image_set_createsem(OUT1STREAMNAMEOPD, 10);
        }
        else
        {
            create_image_ID(OUT1STREAMNAMEOPD, 2, sizearray, _DATATYPE_FLOAT, 0, 0, 0, &IDopd1);
        }

        printf("CREATED %s stream  %ld %ld   [%d]\n", OUT1STREAMNAMEOPD, (long)sizearray[0], (long)sizearray[1],
               OUT1STREAM); //TEST

        if (OUT1STREAM > 0) // create amp stream if OUT0STREAM=1 or 2, but will only update it if OUT0STREAM=2
        {
            create_image_ID(OUT1STREAMNAMEAMP, 2, sizearray, _DATATYPE_FLOAT, 1, 0, 0, &IDamp1);
            COREMOD_MEMORY_image_set_createsem(OUT1STREAMNAMEAMP, 10);
        }
        else
        {
            create_image_ID(OUT1STREAMNAMEAMP, 2, sizearray, _DATATYPE_FLOAT, 0, 0, 0, &IDopd1);
        }
    }
    ...
    if (DM0MODE > 0)
    {
        ...
        if ((data.image[IDdm0opd].md[0].size[0] != ARRAYSIZE) || (data.image[IDdm0opd].md[0].size[1] != ARRAYSIZE))
        {
            printf("ERROR: stream %s has wrong size: is %ld x %ld, should be %ld x %ld\n", DM0NAME,
                   (long)data.image[IDdm0opd].md[0].size[0], (long)data.image[IDdm0opd].md[0].size[1], ARRAYSIZE,
                   ARRAYSIZE);
            exit(0);
        }
    }
    ...
    for (;;)
    {
        ...
        {
            WRITE_FULLFILENAME(wf_fname, "%s/%s%08ld.%09ld.pha.fits", WFDIR, wfin_PREFIX, k,
                               (long)(1.0e12 * LAMBDA + 0.5));
            printf("Loading WF file name : %s\n", wf_fname);
            sprintf(wfimname_pha, "wf%08ld_pha", k);
            load_fits(wf_fname, wfimname_pha, 1, &IDwf0);

            WRITE_FULLFILENAME(wf_fname, "%s/%s%08ld.%09ld.amp.fits", WFDIR, wfin_PREFIX, k,
                               (long)(1.0e12 * LAMBDA + 0.5));
            printf("Loading WF file name : %s\n", wf_fname);
            sprintf(wfimname_amp, "wf%08ld_amp", k);
        ...
        if (image_ID(wfimname_pha) == -1)
        {
            WRITE_FULLFILENAME(wf_fname, "%s/%s%08ld.%09ld.pha.fits", WFDIR, wfin_PREFIX, knext,
                               (long)(1.0e12 * LAMBDA + 0.5));
            printf("Loading WF file name : %s\n", wf_fname);
                        ...
                        if (data.image[IDampmask].array.F[(jj1 - off1) * ARRAYSIZE + (ii1 - off1)] > 1.0e-6)
                        {
                            ...
                            for (long i = 0; i < PIXBINFACTOR; i++)
                                for (long j = 0; j < PIXBINFACTOR; j++)
                                {
                                    ...
                                    opd = OPDMFACT *
                                          ((1.0 - alpha) * data.image[ID0].array.F[kk0 * wfin_WFsize * wfin_WFsize +
                                                                                   jj * wfin_WFsize + ii] +
                                           alpha * data.image[ID1].array.F[kk1 * wfin_WFsize * wfin_WFsize +
                                                                           jj * wfin_WFsize + ii]) /
                                          (2.0 * M_PI) * LAMBDA * 1.0e6;
                                    amp =
                                        ((1.0 - alpha) * data.image[ID0amp].array.F[kk0 * wfin_WFsize * wfin_WFsize +
                                                                                    jj * wfin_WFsize + ii] +
                                         alpha * data.image[ID1amp]
                                                     .array.F[kk1 * wfin_WFsize * wfin_WFsize + jj * wfin_WFsize + ii]);
                            ...
                            data.image[IDbin_amp].array.F[jj1 * csize + ii1] =
                                sqrt(re * re + im * im) / PIXBINFACTOR / PIXBINFACTOR;
            ...
            for (long ii = iistart; ii < iiend; ii++)
                for (long jj = jjstart; jj < jjend; jj++)
                {
                    ...
                    data.image[IDopd0].array.F[jj * ARRAYSIZE + ii] =
                        OPDMFACT *
                        ((1.0 - alpha) *
                             data.image[ID0].array.F[kk0 * wfin_WFsize * wfin_WFsize + jj1 * wfin_WFsize + ii1] +
                         alpha * data.image[ID1].array.F[kk1 * wfin_WFsize * wfin_WFsize + jj1 * wfin_WFsize + ii1]);
                }
        }
        else
        {
                    ...
                    for (long ii = iistart; ii < iiend; ii++)
                        for (long jj = jjstart; jj < jjend; jj++)
                        {
                            ...
                            data.image[IDamp0].array.F[jj * ARRAYSIZE + ii] =
                                AMPMFACT * data.image[IDampmask].array.F[jj * ARRAYSIZE + ii] *
                                ((1.0 - alpha) * data.image[ID0amp].array.F[kk0 * wfin_WFsize * wfin_WFsize +
                                                                            jj1 * wfin_WFsize + ii1] +
                                 alpha * data.image[ID1amp]
                                             .array.F[kk1 * wfin_WFsize * wfin_WFsize + jj1 * wfin_WFsize + ii1]);
                        }
                }
                else
                {
        ...
        if (DM0MODE > 0)
        {
            ...
            for (uint64_t ii = 0; ii < ARRAYSIZE * ARRAYSIZE; ii++)
            {
                data.image[IDopd1].array.F[ii] =
                    data.image[IDopd0].array.F[ii] - 2.0 * data.image[IDdm0opd].array.F[ii];
            }
            memcpy((char *)data.image[IDamp1].array.F, (char *)data.image[IDamp0].array.F,
...
int AOsystSim_WFSsim_Pyramid(const char *inWFc_name, const char *outWFSim_name, double modampl, long modnbpts)
{
    ...
    for (pmodpt = 0; pmodpt < PYRMOD_nbpts; pmodpt++)
    {
        ...
        if ((IDpyramp == -1) || (IDpyrpha == -1))
        {
            ...
            imsize[0] = arraysize;
            imsize[1] = arraysize;
            create_image_ID(pnamea, 2, imsize, _DATATYPE_FLOAT, 0, 0, 0, &IDpyramp);
            create_image_ID("pyrpha0", 2, imsize, _DATATYPE_FLOAT, 0, 0, 0, &IDpyrpha);
            free(imsize);
    ...
    if (ID_outWFSim == -1)
    {
        ...
        imsize[0] = arraysize;
        imsize[1] = arraysize;
        create_image_ID(outWFSim_name, 2, imsize, _DATATYPE_FLOAT, 1, 0, 0, &ID_outWFSim);
    ...
    if (ID_outWFSim_tmp == -1)
    {
        ...
        imsize[0] = arraysize;
        imsize[1] = arraysize;
        create_image_ID("outpwfsimtmp", 2, imsize, _DATATYPE_FLOAT, 1, 0, 0, &ID_outWFSim_tmp);
        free(imsize);
    }
    ...
    if (ID_inWFccp == -1)
    {
        ...
        imsize[0] = arraysize;
        imsize[1] = arraysize;
        create_image_ID("pyrwfcin", 2, imsize, _DATATYPE_COMPLEX_FLOAT, 0, 0, 0, &ID_inWFccp);
        free(imsize);
    }
    ...
    for (pmodpt = 0; pmodpt < PYRMOD_nbpts; pmodpt++)
    {
        ...
        for (uint64_t ii = 0; ii < arraysize2; ii++)
        {
            data.image[ID_outWFSim_tmp].array.F[ii] +=
                data.image[IDa].array.F[ii] * data.image[IDa].array.F[ii] / PYRMOD_nbpts;
        }
...
int AOsystSim_PyrWFS_mkCONF(const char *fname)
{
    ...
    fprintf(fp, "PYRMODAMP                4.0               # Modulation radius [l/D]\n");
    fprintf(fp, "PYRMODNBPT               8                 # number of pyramid modulation points\n");
...
int AOsystSim_PyrWFS(const char *CONF_FNAME)
{
    ...
    for (pmodpt = 0; pmodpt < PYRMODNBPT; pmodpt++)
    {
        ...
        for (uint32_t ii = 0; ii < ARRAYSIZE; ii++)
            for (uint32_t jj = 0; jj < ARRAYSIZE; jj++)
            {
                ...
                data.image[IDpyr_pha].array.F[pmodpt * ARRAYSIZE * ARRAYSIZE + jj * ARRAYSIZE + ii] =
                    M_PI * PUPPIXDIAM * PYRPUPSEP / ARRAYSIZE * (fabs(x) + fabs(y));
    ...
    if (OUTMODE == 0)
    {
        create_image_ID(OUTSTREAMNAME, 2, sizearray, _DATATYPE_FLOAT, 1, 0, 0, &IDout);
        COREMOD_MEMORY_image_set_createsem(OUTSTREAMNAME, 10);

        sizearray[0] = ARRAYSIZE;
        sizearray[1] = ARRAYSIZE;
        create_image_ID(OUTINSTSTREAMNAME, 2, sizearray, _DATATYPE_FLOAT, 1, 0, 0, &IDoutinst);
        COREMOD_MEMORY_image_set_createsem(OUTINSTSTREAMNAME, 10);
    }
    else
    {
        create_image_ID(OUTSTREAMNAME, 2, sizearray, _DATATYPE_FLOAT, 0, 0, 0, &IDout);
        create_image_ID(OUTINSTSTREAMNAME, 2, sizearray, _DATATYPE_FLOAT, 0, 0, 0, &IDoutinst);
    }
    free(sizearray);
    ...
    while (k < kmax)
    {
        ...
        for (uint32_t ii = 0; ii < wfinsize; ii++)
            for (uint32_t jj = 0; jj < wfinsize; jj++)
            {
                ...
                data.image[IDpuppha].array.F[jj1 * ARRAYSIZE + ii1] =
                    2.0 * M_PI * (1.0e-6 * data.image[IDinOPD].array.F[jj * wfinsize + ii]) / LAMBDA;
        ...
        for (uint64_t ii = 0; ii < ARRAYSIZE * ARRAYSIZE; ii++)
        {
            data.image[IDfocamp].array.F[ii] =
                data.image[IDfoca].array.F[ii] * data.image[IDpyr_amp].array.F[pmodpt * ARRAYSIZE * ARRAYSIZE + ii];
            data.image[IDfocpha].array.F[ii] =
                data.image[IDfocp].array.F[ii] + data.image[IDpyr_pha].array.F[pmodpt * ARRAYSIZE * ARRAYSIZE + ii];
        }
...
int AOsystSim_DM(const char *CONF_FNAME)
{
    ...
    INMODE = read_config_parameter_long(CONF_FNAME, "INMODE");
    read_config_parameter(CONF_FNAME, "INSTREAMNAMEDM",
                          INSTREAMNAMEDM); // IDinDM
    read_config_parameter(CONF_FNAME, "INFITSFILENAMEDM", INFITSFILENAMEDM);
    read_config_parameter(CONF_FNAME, "INTRIGSTREAMNAME",
    ...
    if (OUTMODE == 0)
    {
        create_image_ID(OUTSTREAMNAMEDM, 2, sizearray, _DATATYPE_FLOAT, 1, 0, 0, &IDout);
        COREMOD_MEMORY_image_set_createsem(OUTSTREAMNAMEDM, 10);
    }
    else
    {
        create_image_ID(OUTSTREAMNAMEDM, 2, sizearray, _DATATYPE_FLOAT, 0, 0, 0, &IDout);
    }

    create_image_ID("tmpDMshape", 2, sizearray, _DATATYPE_FLOAT, 0, 0, 0, &IDout_tmp);
    free(sizearray);
            ...
            for (uint32_t ii = 0; ii < ARRAYSIZE; ii++)
                for (uint32_t jj = 0; jj < ARRAYSIZE; jj++)
                {
                    ...
                    if ((rxi > 0) && (rxi < ARRAYSIZE - 1) && (ryi > 0) && (ryi < ARRAYSIZE - 1))
                    {
                        ...
                        data.image[IDifc].array.F[(my * DMsize + mx) * ARRAYSIZE * ARRAYSIZE + jj * ARRAYSIZE + ii] =
                            (1.0 - u) * (1.0 - t) * v00 + (1.0 - u) * t * v10 + u * (1.0 - t) * v01 + u * t * v11;
                        if (fabs(data.image[IDifc].array.F[(my * DMsize + mx) * ARRAYSIZE * ARRAYSIZE + jj * ARRAYSIZE +
                                                           ii]) > dmif_limit)
                        {
                            NBdmifcarray++;
                        }
    ...
    while (k < kmax)
    {
        ...
        memcpy((char *)data.image[IDdmdispC1].array.F, (char *)data.image[IDdmdispC].array.F,
               sizeof(float) * DMsize * DMsize * NBTSAMPLES);
        memcpy((char *)data.image[IDdmdispC].array.F,
               (char *)data.image[IDdmdispC1].array.F + sizeof(float) * DMsize * DMsize,
        ...
        for (kk = kkstart; kk < NBTSAMPLES; kk++)
            for (uint64_t ii = 0; ii < DMsize * DMsize; ii++)
            {
                data.image[IDdmdispC].array.F[kk * DMsize * DMsize + ii] =
                    ...
                    alpha * (data.image[IDinDM].array.F[ii] - data.image[IDdmdispC].array.F[kk * DMsize * DMsize + ii]);
            }
        ...
        for (kk = 0; kk < NBdmifcarray; kk++)
        {
            data.image[IDout_tmp].array.F[dmifcarray_iijj[kk]] +=
                data.image[IDdmdispC].array.F[dmifcarray_act[kk]] * dmifcarray_value[kk];
        }
...
int AOsystSim_coroLOWFS(const char *CONF_FNAME)
{
    ...
    INMODE = read_config_parameter_long(CONF_FNAME, "INMODE");
    read_config_parameter(CONF_FNAME, "INAMPSTREAMNAME",
                          INAMPSTREAMNAME); // IDinAMP
    read_config_parameter(CONF_FNAME, "INOPDSTREAMNAME",
    ...
    if (OUTMODE == 0)
    {
        create_image_ID(OUTLOWFSSTREAMNAME, 2, sizearray, _DATATYPE_FLOAT, 1, 0, 0, &IDoutLOWFS);
        COREMOD_MEMORY_image_set_createsem(OUTLOWFSSTREAMNAME, 10);
    }
    else
    {
        create_image_ID(OUTLOWFSSTREAMNAME, 2, sizearray, _DATATYPE_FLOAT, 0, 0, 0, &IDoutLOWFS);
    }

    create_image_ID("aosim_imlowfs", 2, sizearray, _DATATYPE_FLOAT, 1, 0, 0, &IDimlowfs);

    sizearray[0] = ARRAYSIZE;
    sizearray[1] = ARRAYSIZE;

    create_image_ID("aosim_foc1_amp", 2, sizearray, _DATATYPE_FLOAT, 1, 0, 0, &IDfoc1a);
    create_image_ID("aosim_foc1_pha", 2, sizearray, _DATATYPE_FLOAT, 1, 0, 0, &IDfoc1p);

    create_image_ID("aosim_pup1t_amp", 2, sizearray, _DATATYPE_FLOAT, 1, 0, 0, &IDpup1ta);
    create_image_ID("aosim_pup1t_pha", 2, sizearray, _DATATYPE_FLOAT, 1, 0, 0, &IDpup1tp);

    create_image_ID("aosim_pup1r_amp", 2, sizearray, _DATATYPE_FLOAT, 1, 0, 0, &IDpup1ra);
    create_image_ID("aosim_pup1r_pha", 2, sizearray, _DATATYPE_FLOAT, 1, 0, 0, &IDpup1rp);
    ...
    create_2Dimage_ID("aosim_imcamlowfstmp", ARRAYSIZE, ARRAYSIZE, &IDimcamlowfstmp);
    ...
    while (k < kmax)
    {
        ...
        for (uint32_t ii = 0; ii < xsizein; ii++)
            for (uint32_t jj = 0; jj < ysizein; jj++)
            {
                ...
                data.image[IDwfp].array.F[jj1 * ARRAYSIZE + ii1] =
                    1.0 * M_PI * data.image[IDinOPD].array.F[jj * xsizein + ii] * 1e-6 / LAMBDA;
            }
        ...
        for (uint64_t ii = 0; ii < ARRAYSIZE2; ii++)
        {
            ...
            data.image[IDpup1rp].array.F[ii] = data.image[IDpup1p].array.F[ii] + data.image[ID_LS_rpha].array.F[ii] +
                                               2.0 * M_PI * data.image[ID_lowfsopd].array.F[ii] * 1.0e-6 / LAMBDA;
        }
        ...
        for (uint32_t ii = 0; ii < OUTLOWFSARRAYSIZE; ii++)
            for (uint32_t jj = 0; jj < OUTLOWFSARRAYSIZE; jj++)
            {
                ...
                data.image[IDimlowfs].array.F[OUTLOWFSARRAYSIZE * jj + ii] =
                    ...
                    data.image[IDfoclowfsa].array.F[jj1 * ARRAYSIZE + ii1];
                data.image[IDimcamlowfstmp].array.F[OUTLOWFSARRAYSIZE * jj + ii] +=
                    data.image[IDimlowfs].array.F[OUTLOWFSARRAYSIZE * jj + ii];
            }
        ...
        printf("%8ld  TIME: %16f  %16f   %16f/%16f\n", k, data.image[IDphystime].array.F[0], LOWFScamstarttime,
               data.image[IDphystime].array.F[0] - LOWFScamstarttime, LOWFSCAMETIME);
        fflush(stdout);
                             ...
                             double totFlux, double DMgainErr, double RON, double CnoiseFloor)
{
            ...
            for (CPAx = CPAxmin; CPAx < CPAxmax; CPAx += CPAstep)
                for (CPAy = CPAymin; CPAy < CPAymax; CPAy += CPAstep)
                {
                    data.image[IDwfA].array.F[jj * size + ii] +=
                        probeAmultcoeff * probeamp * CPAstep * CPAstep * cos(M_PI * (x * CPAx + y * CPAy));
                    data.image[IDwfB].array.F[jj * size + ii] += probeBmultcoeff * probeamp * CPAstep * CPAstep *
    ...
    for (pr = 0; pr < NBprobesG; pr++)
    {
        ...
        for (ii = 0; ii < size * size; ii++)
        {
            data.image[IDwf].array.F[ii] = data.image[IDwf0].array.F[ii] + coeffA * data.image[IDwfA].array.F[ii] +
    ...
    for (pr = 0; pr < NBprobesG; pr++)
    {
        for (ii = 0; ii < size; ii++)
            for (jj = 0; jj < size; jj++)
            {
                ...
                {
                    data.image[IDpsfC].array.F[pr * size * size + jj1 * size + ii1] +=
                        contrast * data.image[IDtmp].array.F[pr * size * size + jj * size + ii];
                }
                ...
                {
                    data.image[IDpsfC].array.F[pr * size * size + jj1 * size + ii1] +=
                        0.3 * contrast * data.image[IDtmp].array.F[pr * size * size + jj * size + ii];
                }
                ...
                {
                    data.image[IDpsfC].array.F[pr * size * size + jj1 * size + ii1] +=
                        0.1 * contrast * data.image[IDtmp].array.F[pr * size * size + jj * size + ii];
                }
                ...
                {
                    data.image[IDpsfC].array.F[pr * size * size + jj1 * size + ii1] +=
                        0.03 * contrast * data.image[IDtmp].array.F[pr * size * size + jj * size + ii];
                }
                ...
                {
                    data.image[IDpsfC].array.F[pr * size * size + jj1 * size + ii1] +=
                        0.01 * contrast * data.image[IDtmp].array.F[pr * size * size + jj * size + ii];
                }
            }
    }
        ...
        for (ii1 = 0; ii1 < xsize; ii1++)
            for (jj1 = 0; jj1 < ysize; jj1++)
            {
                ...
                data.image[ID].array.F[pr * xsize * ysize + jj1 * xsize + ii1] =
                    data.image[IDpsfC].array.F[pr * size * size + jj * size + ii];
    ...
    for (pr = 0; pr < NBprobesG; pr++)
        for (ii1 = 0; ii1 < xsize; ii1++)
            for (jj1 = 0; jj1 < ysize; jj1++)
            {
                data.image[ID1].array.F[pr * xsize * ysize + jj1 * xsize + ii1] =
                    data.image[ID].array.F[pr * xsize * ysize + jj1 * xsize + ii1] / peak;
                ...
                data.image[IDnoise].array.F[pr * xsize * ysize + jj1 * xsize + ii1] =
                    sqrt(val + RON * RON) / peak; // assuming photon noise + readout noise
    ...
    for (pr = 0; pr < NBprobesG; pr++)
    {
        ...
        for (ii = 0; ii < size; ii++)
            ...
            {
                data.image[ID].array.F[jj * size + ii] =
                    data.image[IDpsfC].array.F[pr * size * size + jj * size + ii] / peak;
            }
                             ...
                             double CPAmax, double CPArmin, double CPArmax, double RMSampl, long modegeom)
{
    ...
    for (uint32_t ii = 0; ii < dmxsize; ii++)
        for (uint32_t jj = 0; jj < dmysize; jj++)
        {
            data.image[ID].array.F[(jj + (imsize - dmysize) / 2) * imsize + (ii + (imsize - dmxsize) / 2)] =
...
int AOsystSim_FPWFS_sensitivityAnalysis(int mapmode, int mode, int optmode, int NBprobes)
{
    ...
    for (mapz = 0; mapz < mapzsize; mapz++)
    {
        ...
        if (imSimulMode == 1)
        {
            IDpsfC = AOsystSim_FPWFS_imsimul(probeampl, 30.0, 30.0, contrast, WFerr, totFlux, probe_noise_prop, RON,
            ...
            for (pr = 1; pr < NBprobes; pr++)
            {
                for (uint32_t ii = 0; ii < data.image[IDpsfC].md[0].size[0]; ii++)
                    ...
                    {
                        ave += data.image[IDpsfC]
                                   .array.F[pr * data.image[IDpsfC].md[0].size[0] * data.image[IDpsfC].md[0].size[1] +
        ...
        if (initmap == 0)
        {
            if (mapmode > 0)
            {
                ...
                create_3Dimage_ID("WFSsol_ptre_in", mapxsize, mapysize, mapzsize, &IDmap_ptre_in);
                create_3Dimage_ID("WFSsol_ptim_in", mapxsize, mapysize, mapzsize, &IDmap_ptim_in);
            ...
            for (kx = 0; kx < mapxsize; kx++)
                for (ky = 0; ky < mapysize; ky++)
                {
                    ...
                    for (pr = 0; pr < NBprobes; pr++)
                    {
                        ...
                        if ((mapmode == 0) &&
                            (((kx == kxtest) && (ky == kytest)) || ((kx == kxtest + 1) && (ky == kytest))))
                        {
                            printf("M0    NO NOISE:  probe %3d  normalized flux = %8.5lf  (%g ph)\n", pr,
                                   (double)(probe_mflux[pr] / Cflux), (double)probe_mflux[pr]);
                        ...
                        {
                            probe_nmflux[pr] =
                                data.image[IDpsfC].array.F[pr * mapxsize * mapysize + ky * mapxsize + kx] /
                                data.image[IDprobampC].array.F[ky * mapxsize + kx]; // unit : normalized contrast
                            probe_nmnoise[pr] =
                                data.image[IDpsfCnoise].array.F[pr * mapxsize * mapysize + ky * mapxsize + kx] /
                    ...
                    do
                    {
                        ...
                        status = gsl_multimin_test_size(optsize, 1.0 / pow(10.0,
                    ...
                    if (mapmode > 0)
                    {
                        ...
                        data.image[IDmap].array.F[mapz * mapxsize * mapysize + ky * mapxsize + kx] =
                            Iflux_best * sqrt(data.image[IDprobampC].array.F[ky * mapxsize + kx]);
                        data.image[IDmap_ptre].array.F[mapz * mapysize * mapxsize + ky * mapxsize + kx] =
                            ptre_best * sqrt(data.image[IDprobampC].array.F[ky * mapxsize + kx]);
                        data.image[IDmap_ptim].array.F[mapz * mapysize * mapxsize + ky * mapxsize + kx] =
                            ptim_best * sqrt(data.image[IDprobampC].array.F[ky * mapxsize + kx]);
                        data.image[IDmap_ptre_in].array.F[mapz * mapysize * mapxsize + ky * mapxsize + kx] =
                            ptre * sqrt(data.image[IDprobampC].array.F[ky * mapxsize + kx]);
                        data.image[IDmap_ptim_in].array.F[mapz * mapysize * mapxsize + ky * mapxsize + kx] =
                            ptim * sqrt(data.image[IDprobampC].array.F[ky * mapxsize + kx]);
                        data.image[IDmap_Iflux].array.F[mapz * mapysize * mapxsize + ky * mapxsize + kx] =
                            Iflux_best * data.image[IDprobampC].array.F[ky * mapxsize + kx];
                        if (NBoptVar > 3)
                        {
                            data.image[IDmap_are].array.F[mapz * mapysize * mapxsize + ky * mapxsize + kx] =
                                are_best * sqrt(data.image[IDprobampC].array.F[ky * mapxsize + kx]);
                        }
                        if (NBoptVar > 4)
                        {
                            data.image[IDmap_aim].array.F[mapz * mapysize * mapxsize + ky * mapxsize + kx] =
                                aim_best * sqrt(data.image[IDprobampC].array.F[ky * mapxsize + kx]);
        ...
        if ((mapz > 2) && (mapmode > 0))
        {
            ...
            for (kx = 0; kx < mapxsize; kx++)
                for (ky = 0; ky < mapysize; ky++)
                {
                    ...
                    for (kz = 0; kz < mapz; kz++)
                    {
                        ...
                        rms += data.image[IDmap_Iflux].array.F[kz * mapysize * mapxsize + ky * mapxsize + kx] *
                               data.image[IDmap_Iflux].array.F[kz * mapysize * mapxsize + ky * mapxsize + kx];
                    }
                    ave /= mapz;
                    rms /= mapz;
                    rms -= ave * ave;
            ...
            if (imSimulMode == 0)
            {
                ...
                {
                    create_2Dimage_ID("WFSsol_Iflux_rmsn1", mapxsize, mapysize, &IDmap_Iflux_rmsn1);
                }
                ...
                for (kx = 0; kx < mapxsize; kx++)
                    for (ky = 0; ky < mapysize; ky++)
                    {
                        ...
                        data.image[IDmap_Iflux_rmsn].array.F[ky * mapxsize + kx] =
                            data.image[IDmap_Iflux_rms].array.F[ky * mapxsize + kx] / sqrt(1.0 / FLUXph);
                        data.image[IDmap_Iflux_rmsn1].array.F[ky * mapxsize + kx] =
                            data.image[IDmap_Iflux_rms].array.F[ky * mapxsize + kx] / sqrt(1.0 / FLUXph) /
                ...
                for (kx = 0; kx < mapxsize; kx++)
                    for (ky = 0; ky < mapysize; ky++)
                    {
                        ...
                        for (kz = 0; kz < mapz; kz++)
                        {
                            dx = data.image[IDmap_ptre].array.F[kz * mapysize * mapxsize + ky * mapxsize + kx] -
                                 data.image[IDmap_ptre_in].array.F[kz * mapysize * mapxsize + ky * mapxsize + kx];
                            dy = data.image[IDmap_ptim].array.F[kz * mapysize * mapxsize + ky * mapxsize + kx] -
                                 data.image[IDmap_ptim_in].array.F[kz * mapysize * mapxsize + ky * mapxsize + kx];
                            val = dx * dx + dy * dy;
                            ave += val;
                            rms += val * val;
                        }
                ...
                for (kx = 0; kx < mapxsize; kx++)
                    for (ky = 0; ky < mapysize; ky++)
                    {
                        data.image[IDmap_CA_rmsn].array.F[ky * mapxsize + kx] =
                            data.image[IDmap_CA_rms].array.F[ky * mapxsize + kx] * sqrt(FLUXph);
                    }
